...
function scheduler(): void {
    ...
    $head = getTplAdminTableHead([_TITLE, _SCHEDULER_NEXTRUN, _SCHEDULER_RESULT, _SCHEDULER_PRIO, _STATUS, [_FUNCTIONS, 'nosort']]);
    $rows = '';
    foreach ($jobs as $job) {
        $name = $job['name'];
        ...
        $tip .= getTplAdminTipLine(_TYPE, $type);
        $tip .= getTplAdminTipLine(_STATUS, $stat);
        if ($sched !== '') $tip .= getTplAdminTipLine(_SCHEDULER_SCHED, $sched);
        $tip .= getTplAdminTipLine(_SCHEDULER_LASTRUN, $last);
        $tip .= getTplAdminTipLine(_SCHEDULER_LAST_OK, $lastok);
        $tip .= getTplAdminTipLine(_SCHEDULER_NEXTRUN, $nextr);
        $tip .= getTplAdminTipLine(_SCHEDULER_TRIGGER, ($trig !== '' ? $trig : _NO));
        $tip .= getTplAdminTipLine(_SCHEDULER_RUNTIME, (string)$time);
        $tip .= getTplAdminTipLine(_SCHEDULER_FAILS, (string)$fail);
        if ($note !== '') $tip .= getTplAdminTipLine(_DESCRIPTION, $note);
        $title = $job['title'];

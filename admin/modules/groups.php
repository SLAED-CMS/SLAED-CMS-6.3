    ...
    if ($db->getSqlRowCount($result) > 0) {
        ...
        while ([$grid, $grname, $description, $points, $extra, $rank, $color] = $db->getSqlRow($result)) {
            ...
            $rows .= getTplAdminTableRow($tpl->getHtmlFrag('admin-groups-list-row', [
                'actions_html' => $acts,
                'extra_text' => $extra,
                'group_html' => getTplAdminTitleTip(_DESCRIPTION.': '.$description).getTplAdminColorLabel($color, $grname),
                'id_text' => (string)$grid,
...
function add(): void {
    ...
    setHead();
    $cont = getTplAdminNavi(['ops' => ['name=groups', 'name=groups&amp;op=add', 'name=groups&amp;op=points', 'name=groups&amp;op=info'], 'tabs' => [_HOME, _ADD, _POINTS, _INFO], 'tab' => 1]);
...
function points(): void {
    global $afile, $conf;
    setHead();
    $cont = getTplAdminNavi(['ops' => ['name=groups', 'name=groups&amp;op=add', 'name=groups&amp;op=points', 'name=groups&amp;op=info'], 'tabs' => [_HOME, _ADD, _POINTS, _INFO], 'tab' => 2]);
...
function info(): void {
    $cont = getTplAdminNavi(['ops' => ['name=groups', 'name=groups&amp;op=add', 'name=groups&amp;op=points', 'name=groups&amp;op=info'], 'tabs' => [_HOME, _ADD, _POINTS, _INFO], 'tab' => 3]);

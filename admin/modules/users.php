...
function users_save(): void {
	...
	$xawidth = (!intval($awidth)) ? 100 : $awidth;
	$xaheight = (!intval($aheight)) ? 100 : $aheight;
	$xuser_t = (!$user_t) ? 2592000 : intval($user_t * 86400);
	$xanum = (!intval($anum)) ? 50 : $anum;
	$xanump = (!intval($anump)) ? 10 : $anump;
...
function users_new_del(): void {
	...
	if ($id) {
		$db->sql_query('DELETE FROM '.$prefix.'_users_temp WHERE user_id = :id', ['id' => $id]);
	}

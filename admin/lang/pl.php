<?php
# Author: Eduard Laas
# 2005 - 2026 SLAED
# License: MIT
# Website: slaed.net

define('_ACESS','Prawa dostępu');
define('_ACESSF','Prawa forum');
define('_ACESSI','Możesz ustawić prawa dostępu dla wielu opcji, jest to tylko możliwe dla grup.');
define('_ACTIVATE2','Aktywacja?');
define('_ACTMULTILINGUAL','Aktywacja multilingual właściwości?');
define('_ACTUSEFLAGS','Wyświetlane flagi zamiast menu?');
define('_ACTUSELANG','Automatycznie przełączanie język witryny pod kraju użytkownika?');
define('_ADDAMAIL','Powiadom administratorów o nowych dokumentach pz?');
define('_ADDCATEGORY','Dodaj kategorię');
define('_ADDCONST','Uwzględniona językowych stałych. Wartości językowych stałych dostępne są w dziale «Języki».');
define('_ADDNEWBLOCK','Dodaj blok');
define('_ADDNEWFILEBLOCK','Dodaj plików blok');
define('_ADDRESS','Adres');
define('_ADDSUBCATEGORY','Dodaj pod-kategorii');
define('_ADIR','Katalog obrazów w katalogu głównym strony');
define('_ADMCOL','Liczba kolumn po wyświetleniu modułów panelu administratora');
define('_ADMINEMAIL','E-Mail admin');
define('_ADMINFOEDIT','Aktywacja redaktor działu informacji?');
define('_ADMININFO','Notatki admin');
define('_ADMININFODES','Informacja, która będzie dostępna dla wszystkich autorów. Wyświetlanie tej informacji odbywa się w górnej części panelu administratora systemu.');
define('_ADMINLOGIN','Logowanie administracyjne');
define('_ADMINLOGIN_NEW','Instalacja systemu administracji');
define('_ADMIN_SESSION','Nazwa sesje dla administratorów');
define('_AFTEREXPIRATION','Po upływie');
define('_AKTIVE','Aktywne');
define('_ALLOWANONPOST','Pozwól анонимам publikacji?');
define('_ALLMODULES','Wszystkie moduły');
define('_ALLUSER','Wszystkich użytkowników');
define('_ALREADYOPTIMIZED','Nie wymaga');
define('_ALWAYS','Zawsze');
define('_ANEWSLETTER','Tylko użytkownicy biorący udział');
define('_ANONIMP','Tylko anonimowych użytkowników');
define('_APOSTMOD','Z próbnego moderatora');
define('_APOSTNOMOD','Bez sprawdzania moderatora');
define('_ASC','W porządku rosnącym');
define('_ASIZE','Maksymalny rozmiar pliku obrazka w bajtach');
define('_ASSOTOPIC','Podobne tematy');
define('_ASSOTOPICI','Podczas korzystania z tej opcji, materiał będzie dodatkowo wyświetlane w tych kategoriach.');
define('_ATYPE','Dopuszczalne formaty obrazu');
define('_AUDIENCE','Publiczność');
define('_AUPLOAD','Zezwolić na pobieranie z avatara na serwer?');
define('_AUTH_DELETE','usuwanie');
define('_AUTH_EDIT','zmiana');
define('_AUTH_MOD','moderuj');
define('_AUTH_POST','tworzyć');
define('_AUTH_READ','czytać');
define('_AUTH_REPLY','odpowiedzi');
define('_AUTH_VIEW','oglądać');
define('_BANNED','Blokowanie');
define('_BANNED_IP','Zablokowane Adresy IP');
define('_BANNED_USERS','Zablokowani użytkownicy');
define('_BANNER','Banner');
define('_BANNERDOWN','Dolny banner');
define('_BANNERUP','Górny baner');
define('_BASCOL','Liczba kolumn w druku publikacji');
define('_BLOCK','Blok');
define('_BLOCKFILE','Plików blok');
define('_BLOCKFILE2','Plik');
define('_BLOCKHTML','HTML blok');
define('_BLOCKRSS','RSS blok');
define('_BLOCKS','Zespoły i banery');
define('_BLOCKSYSTEM','System');
define('_BLOCKS_MOD','Wyświetlanie bloków w module');
define('_BLOCKS_MOD0','Aktywacja lewe i prawe zespoły');
define('_BLOCKS_MOD1','Aktywować tylko lewe bloki');
define('_BLOCKS_MOD2','Aktywować tylko prawe zespoły');
define('_BLOCKS_MOD3','Wyłączyć lewy i prawy zespoły');
define('_BLOCKS_MODC0','Aktywacja środkowe górne i dolne zespoły');
define('_BLOCKS_MODC1','Aktywować tylko środkowe górne zespoły');
define('_BLOCKS_MODC2','Aktywować tylko dolne centralne zespoły');
define('_BLOCKS_MODC3','Dezaktywować środkowe górne i dolne zespoły');
define('_BLOCK_ALL','We wszystkich modułach');
define('_BLOCK_VIEW','Wyświetl blok w modułach');
define('_BOTSACT','Pokazuj boty wyszukiwarek w bloku użytkowników?');
define('_BOTSINFO','Lista опознаваемых systemem boty wyszukiwarek. Aby dodać nowy bot użyj następującego formatu zapisu: "unikalna linia z user-agent"="nazwa bota do wyświetlania na stronie". Przykład: googlebot=Google Bot');
define('_BOTSLIST','Lista boty wyszukiwarek');
define('_BOTSOPT','Roboty indeksujące');
define('_BOTSSITE','Lista stron w wyszukiwarkach');
define('_B_FEDIT','Plik o tej nazwie już istnieje w systemie. Zwróć uwagę, po zapisaniu, plik bloku będzie w pełni edytować, zawartość tego pliku zostaną zastąpione istotne.');
define('_CACESSI','Prawa forum należy zainstalować tylko w przypadku jeśli tworzona kategoria należy do części forum, w przeciwnym razie pozostaw domyślne.');
define('_CACHE','Buforowanie');
define('_CACHEINFO','Steruje także buforem treści przetworzonych przez parser. Druga opcja ogranicza tylko bufor stron, nie tę warstwę.');
define('_CACHEBROW','Czas pamięci podręcznej przeglądarki, w dniach (0 - wyłączone)');
define('_CACHELOCK','Przebudowa cache przez jeden proces?');
define('_CACHELOCKINFO','Po wygaśnięciu cache tylko jeden proces przebudowuje stronę; pozostałe chwilowo otrzymują poprzednią wersję. Zmniejsza szczytowe obciążenie przy dużym ruchu. Zalecane dla projektów o dużym ruchu.');
define('_CACHECSS','Buforować style motywy?');
define('_CACHECLEAR','Wyczyść całą pamięć podręczną');
define('_CACHESCRIPT','Buforować skrypty?');
define('_CACHETIME','Czas życia bufora, w sekundach');
define('_CACHETIMEINFO','Przez ten czas gotowa strona oraz spakowane style i skrypty są świeże, potem powstają na nowo. Zaplanowane czyszczenie usuwa pliki starsze niż ta wartość razy 24, ale nie wcześniej niż po dobie.');
define('_CACHE_1','Strona główna i listy materiałów');
define('_CACHE_2','Tylko strony głównej');
define('_CAN','Mogą');
define('_CATDRAGSORT','Przeciągnij ikonę „Pozycja”, aby sortować kategorie wspólnego rodzica w module. Podkategorie są przenoszone z rodzicem. Kliknięcie nagłówka zwija grupę.');
define('_CDEFIS','Symbol rozdzielający kategorii');
define('_CEDITT','Okres jej edycji przez użytkownika, w minutach');
define('_CENSOR','Zakazane słowa');
define('_CENSORMODE','Sposób cenzury');
define('_CENSORREPLACE','Wymiana zakazanych słów na');
define('_CENSORS','Cenzura');
define('_CENTERBLOCK','Centralna jednostka');
define('_CENTERDOWN','Na środku na dole');
define('_CENTERUP','Na środku u góry');
define('_CLEAN','Wyczyść');
define('_CLICABLE','Konwersja adresu hiperłącza?');
define('_CLICABLEINFO','Dotyczy przetwarzania treści tekstowych.');
define('_CLIENTSM','Klienci modułu');
define('_COLOR','Kolor');
define('_COMLETTER','Maksymalna dozwolona liczba znaków w słowie');
define('_COMMENTS_DRIFT','Liczniki komentarzy nie zgadzają się z dyskusjami. Materiały: %d.');
define('_COMMENTS_SYNC','Napraw liczniki');
define('_COMMENTS_FIXED','Naprawione liczniki: %d.');
define('_CONFIG_BLOCKED','Przywracanie nie może samo rozstrzygnąć, powód: %s. Sprawdź ręcznie pliki operacji w storage/backup/config.');
define('_CONFIG_CLEAN','Nie znaleziono przerwanej operacji konfiguracji, konfiguracja jest spójna.');
define('_CONFIG_OP','Operacja');
define('_CONFIG_PENDING','Operacja konfiguracji została przerwana. Nowe zapisy konfiguracji są odrzucane do czasu jej przywrócenia.');
define('_CONFIG_PHASE','Faza');
define('_CONFIG_RESTORE','Przywróć konfigurację');
define('_CONFIG_RESTORED','Konfiguracja została pomyślnie przywrócona.');
define('_CONFIG_VERDICT','Migawka do zastosowania');
define('_NODE','Treści');
define('_NODE_STALE','Typ Node %s został w międzyczasie zmieniony przez inny zapis, nic nie zapisano. Odśwież stronę i powtórz zmianę.');
define('_NODE_BAD','Typ Node %s odrzucił zmianę, nic nie zapisano.');
define('_NODE_PROOF','O migawce do zastosowania decyduje baza danych na podstawie wersji typu Node %s.');
define('_NODE_MANAGE','Node: typy i ich ustawienia');
define('_NODE_SETUP','Instalacja nie ukończyła tych sekcji Node: %s - sekcja nie została utworzona, nie została włączona lub pozostała bez materiału startowego. Przyczyna jest zapisana w dzienniku witryny.');
define('_NODE_START_TITLE','Witamy w SLAED CMS');
define('_NODE_START_INTRO','Strona jest zainstalowana i gotowa do pracy. Sekcje wiadomości, stron, pytań i odpowiedzi, plików, linków, multimediów, dokumentacji i inne zostały utworzone ze standardowych profili Node i są już włączone.');
define('_NODE_START_BODY','Sekcje konfiguruje się w panelu administratora na karcie „Typy” modułu Node: lista, funkcje, zasoby i integracje każdej sekcji. Tam też nową sekcję tworzy się z profilu, klonuje, eksportuje i importuje. Tę wiadomość można zmienić lub usunąć.');
define('_CONFINES','Należy podać wartość «0» do wycofania wszystkich ograniczeń.');
define('_CONST','Stała język');
define('_CREATEBLOCK','Nowy blok');
define('_CREATEUSERDATA','Utwórz użytkownika?');
define('_CSEND','Minimalny czas między publikacjami, w sekundach');
define('_CSSCOMP','Kompresji style?');
define('_CSSDIR','Katalogu stylów');
define('_CSSDIRINFO','Należy podać tylko w katalogu, w którym przechowywane są style. System określi niezbędne pliki css sama. Wartość <b>[theme]</b> - będzie wymieniony na aktualne motyw automatycznie.');
define('_CSSENC','Przekonwertować obrazy stylów binarny tekst?');
define('_CSSHEAD','Wyświetlanie stylów w nagłówku strony?');
define('_CTRLINFO','Do wyboru dwóch i więcej opcji przytrzymaj klawisz «Ctrl lub Strg».');
define('_CUSTOM','Standard');
define('_C_11','Wyświetla liczbę powiązanych publikacji');
define('_C_13','Ilość w liście');
define('_C_15','Rasa w kategorii zawartych w niej podkategorie?');
define('_C_17','Wyświetla datę publikacji?');
define('_C_18','Wyświetla ilość odsłon?');
define('_C_19','Wyświetl najlepsze i najpopularniejsze?');
define('_C_20','Pozwól wyszukiwać alfabetycznie?');
define('_C_21','Aktywacja ocena użytkownika?');
define('_C_22','Pozwól oceniać tylko po szczegółowym przeglądzie?');
define('_C_23','Wyświetl związane publikacji?');
define('_C_32','Rasa opis kategorii?');
define('_C_33','Ilość publikacji na stronę użytkownika');
define('_C_34','Ilość publikacji na stronę admina');
define('_C_35','Maksymalna liczba numerów stron użytkowników');
define('_C_36','Maksymalna liczba numerów stron admin');
define('_DAILY','Codziennie');
define('_DATABASE','Baza danych');
define('_DBCOLL','Porównanie');
define('_DBFREE','Koszty');
define('_DBINFO','Przy użyciu standardowego prefiksu nie jest konieczne, na miejsce można go używać <b>{prefix}</b>. W tym przypadku zmienna <b>{prefix}</b> zostanie automatycznie zastąpiony z prefiksem. Możesz również użyć zmiennych <b>{engine}</b>, <b>{charset}</b> i <b>{collate}</b>, aby automatycznie zastosować żądany silnik tabeli, zestaw znaków i sortowanie podczas tworzenia lub aktualizowania tabel bazy danych.');
define('_DBWARN','W przypadku nieprawidłowego żądania możliwy błąd w działaniu bazy danych, więc korzystać z tej funkcji jest zalecane tylko dla zaawansowanych użytkowników systemu.');
define('_DB_DELETE','Usuwanie tabel bazy');
define('_DB_INSTALL','Instalacja tabele bazy');
define('_DB_SYNC','Synchronizacja «PHP timezone» Czas «MySQL timezone»?');
define('_DB_UPDATE','Aktualizacja tabel bazy');
define('_DB_MODE','Tryb');
define('_DB_BLOCKS','Bloki');
define('_DB_ERRORS','Błędy');
define('_DB_STOP','Blok zatrzymania');
define('_DB_SQL','Zapytanie');
define('_DB_PARSE','Sprawdź parser');
define('_DB_PARSEMODE','Sprawdzenie');
define('_DB_RUNMODE','Wykonanie');
define('_DEAKTIVE','Nieaktywne');
define('_DEVMODE','Tryb deweloperski?');
define('_DEFIS','Symbol rozdzielający титлы i nagłówki');
define('_DELSETUP','W celu zwiększenia bezpieczeństwa należy usunąć lub zmienić nazwę pliku instalacyjnego systemu setup.php w głównym katalogu strony!');
define('_DENYING','Zdradzony wybranych modułów');
define('_DESC','W porządku malejącym');
define('_DRAGSORT','Przeciągnij ikonę w kolumnie „Pozycja” w obrębie grupy. Wolne bloki tworzą osobną grupę i używają [block=id]; pozycja ani kolejność ich nie dotyczą.');
define('_DIR','Katalog');
define('_DIRDEF','Katalog otwierany domyślnie');
define('_EDFILEA','Całkowita liczba plików dla admina');
define('_EDFILEG','Całkowita liczba plików dla gościa');
define('_EDFILEU','Całkowita liczba plików dla użytkownika');
define('_EDITBLOCK','Edytuj blok');
define('_EDITORUSER','Edytor użytkownika');
define('_EDITVOTE','Edycja ankieta');
define('_EFUNC','Plik ingerencji w jądro systemu');
define('_EFUNCN','Jądro systemu');
define('_EHEAD','Plik wdrożenia w czapkę systemu');
define('_EHEADN','Czapka systemu');
define('_EHT','Plik konfiguracji reguł transformacji Rewrite na poziomie serwera');
define('_EHTN','Serwera Rewrite');
define('_EINFO','Tutaj możesz wkleić odpowiedni kod, który będzie położenie w jądrze systemu. Zwykle plik ten jest używany do implementacji dodatkowych funkcji rozszerzających funkcjonalność systemu.');
define('_EINFO2','Tutaj możesz wkleić odpowiedni kod, który zostanie osadzony w head systemu. Zwykle plik ten jest używany do implementacji java skryptów i znaczników meta.');
define('_EINFO4','Tutaj możesz zmienić lub uzupełnić standardowe zasady przekształceń adresowych wierszy przeglądarki na serwerze poziomie. Opcja ta jest obsługiwana tylko w przypadku zainstalowanego na serwerze Mod Rewrite.');
define('_EINFO5','Tutaj możesz zmienić lub dodać plik tekstowy w katalogu głównym witryny, który odnotował specjalne instrukcje dla wyszukiwarek. Instrukcje te mogą zabronić indeksowania niektóre sekcje lub strony w miejscu, wskazują na prawidłowe «lustrzanego odbicia» domeny polecić robota wyszukiwarki obserwować pewien odstęp czasu między dokumentami pobierania z serwera itp W tym pliku można określić parametry strony indeksowania dla wszystkich robotów na raz, i dla każdej wyszukiwarki indywidualnie.');
define('_EINFOPHP','Używając standardowej składni PHP, z wyjątkiem ustalania otwarcie <b>&lt;?php</b> i zamykanie <b>?&gt;</b> znacznik, system zrobi to sam. Jeżeli wpiszesz zły kod, ewentualnej awarii systemu, więc korzystać z tej funkcji jest zalecane tylko dla zaawansowanych użytkowników.');
define('_EROB','Firmware i parametr wskaźnik dla wyszukiwarek');
define('_ERON','Zasady robotów');
define('_EROBSTD','Standard');
define('_ESAVED','Zapisano do pliku');
define('_ERRORPERM','nie ma odpowiednich uprawnień do zapisu na serwerze.<br>Zaznacz właściwe atrybuty');
define('_ERROR_SUBCAT','Nie znaleziono ani jednej kategorii do modułu: %1$s. Do dalszej pracy tego wydziału, należy utworzyć przynajmniej jedną podstawową kategorię.');
define('_EXECUTE','Wykonać operację');
define('_EXPIRATION','Czas pracy, w bezpłatny');
define('_FAVOR_ACT','Aktywacja ulubionych?');
define('_FAVOR_UMAX','Maksymalna dopuszczalna ilość');
define('_FIELD','Dodatkowe pole');
define('_FIELDAREA','Pole tekstowe');
define('_FIELDDATE','Data: rok-miesiąc-dzień');
define('_FIELDIN','Obowiązkowo wypełnić');
define('_FIELDINFO','Nazwa techniczna pola i klucz opcji są zapisywane z każdą wartością i nie można ich później zmienić: najpierw mała litera łacińska, potem litery, cyfry lub podkreślenie, najwyżej 32 znaki. Tytuł zaczynający się od podkreślenia wskazuje stałą językową. Granice liczą znaki tekstu, ograniczają liczbę lub datę albo liczą wybrane opcje listy z kilkoma wartościami. Wyłączone pole jest ukryte i zachowuje swoje wartości; usunięte traci je przy następnym zapisie właściciela.');
define('_FIELDINPUT','Jeden wiersz');
define('_FIELDS','Dodatkowe pola');
define('_FIELDS_BOOL','Przełącznik: tak lub nie');
define('_FIELDS_INT','Liczba całkowita');
define('_FIELDS_DECIMAL','Liczba dziesiętna');
define('_FIELDS_KEY','Nazwa techniczna');
define('_FIELDS_DEFAULT','Wartość domyślna');
define('_FIELDS_MULTI','Zezwól na kilka wartości');
define('_FIELDS_LOW','Dolna granica');
define('_FIELDS_TOP','Górna granica');
define('_FIELDS_SCALE','Cyfry po przecinku');
define('_FIELDS_ITEMS','Opcje');
define('_FIELDS_NOMARK','Aktualizacja danych 6.3 nie została wykonana: pola dodatkowe są zamknięte do zapisu.');
define('_FIELDS_BAD','Definicje zostały odrzucone, pierwszy błąd: %s');
define('_FIELDSELECT','Lista z wyborem');
define('_FIELDTIME','Data: rok-miesiąc-dzień godzina:minuty');
define('_FILENAME','Nazwa pliku');
define('_FILENAMEIN','W przypadku korzystania z plików bloku jest zapisywana zawartość tekstowa zostanie zignorowane.');
define('_FILE_M','Całkowita liczba plików');
define('_FILE_S','Całkowity rozmiar plików');
define('_FIX','Sortowanie');
define('_FIXED','Zablokować');
define('_FLY_FIX','Wolny stały blok');
define('_F_8','Pozwól dodawać pliki?');
define('_F_9','Zezwól gościom dodawać pliki?');
define('_GDWIDTH','Szerokość obrazu podglądu');
define('_GENPREF','Podstawowe');
define('_GEO_IP','Wykrywać kraj odwiedzającego po IP?');
define('_GEOLOCATION','Geolokalizacja');
define('_GEOIP_ANON','Anonimizować IP przed zapisaniem?');
define('_GEOIP_ASN','ASN');
define('_GEOIP_ASNDB','Baza ASN');
define('_GEOIP_CACHE','Czas pamięci podręcznej GeoIP, w sekundach');
define('_GEOIP_CONTINENT','Kontynent');
define('_GEOIP_COUNTRYDB','Baza krajów');
define('_GEOIP_COUNTRYNAME','Nazwa kraju');
define('_GEOIP_ORG','Organizacja');
define('_GEOIP_PROVIDER','Dostawca');
define('_GEOIP_STORE','Zapisywać pełny adres IP?');
define('_GEOIP_TESTIP','Test IP');
define('_FOUND','Znaleziono');
define('_NOTFOUND','Nie znaleziono');
define('_PARAMETERS','Parametry');
define('_VALUE','Wartość');
define('_GROUPSI','Weź użytkownika do specjalnej grupy może tylko administrator systemu. Dodać nowego członka do grupy, można w dziale «Użytkownicy». Funkcja jest dostępna podczas tworzenia nowej lub edycji istniejącego użytkownika.');
define('_GRSINFO','Jeśli zdecydujesz, minimalna liczba punktów zostanie zignorowany.');
define('_GTIME','Globalny czas systemowy');
define('_HOMCAT','Wyświetl kategorii na głównej?');
define('_HOMEPAGE','Witryny');
define('_HOURLY','Co godzinę');
define('_ICON','Ikona');
define('_ICONPICK','Wybierz ikonę');
define('_INFLY','Wolny blok');
define('_INFLYINFO','Wolne bloki — wyświetlane tagiem [block=id]');
define('_INFOCATDEL','Usunięcie kategorii jest niemożliwe tak długo, jak to jest związane zawartość modułu materiałów lub zawiera inne podkategorie.');
define('_INHOME','Tylko na głównej');
define('_INQUIRY','Zapytanie do bazy danych');
define('_IN_ID','Identyczne wejścia');
define('_IN_URL','Adresy wejść');
define('_IP_ID','Identyczne IP o których mowa');
define('_IP_LINK','Link do strony sprawdzanie adresu IP');
define('_IP_CIDR','Zakres IP (CIDR)');
define('_IP_CIDR_TIP','Użyj notacji CIDR, na przykład: 203.0.113.0/24, 2001:db8::/64. Użyj przecinka bez spacji jako separatora.');
define('_IP_REF','IP o których mowa');
define('_LADATE','Ustaw bieżącą datę');
define('_LANGCOUNT','Liczba nowych pól stałych językowych');
define('_LANGKEY','API-Key');
define('_LANGTR','Język, który będzie używany tłumaczenie');
define('_LANG','Edytor języków');
define('_LEFT','Po lewej stronie');
define('_LEFTBLOCK','Lewy blok');
define('_LIVE_UPD','Automatyczne odświeżanie bloków na żywo (sekundy, 0 = wył.)');
define('_LHOME','Publikować na głównej');
define('_LNFIX','Zdjąć zabezpieczenia');
define('_LNHOME','Zdjąć publikacji na głównej');
define('_LOCATION','Lokalizacja');
define('_LOGINF','Bezpośrednia forma logowania');
define('_LOGINFL','Typ pozdrowienia anonimowych użytkowników');
define('_LOGINL','Link do logowania i rejestracji');
define('_LOGO','Instalacja logo witryny');
define('_MAILOPT','Poczta');
define('_MAILTEMP','Podstawowy wzór pocztowy');
define('_MAILTEMPINFO','<b>[text]</b> - Treść, która będzie подставлено systemem');
define('_MAIL_BLOCK','Zabronione adresy E-Mail');
define('_MAIL_INFO','<b>[time]</b> - Czas pozostały do zakończenia blokady<br><b>[info]</b> - Powód blokady');
define('_MAIL_PASS_INFO','<b>[login]</b> - Nazwa użytkownika w projekcie<br><b>[pass]</b> - Hasło użytkownika na projekcie');
define('_MAIL_SEND','Napisał wiadomość na adres E-Mail użytkownika wysłana!');
define('_MAIL_SENDE','Powiadomienie na E-Mail użytkownika?');
define('_MAIL_TEXT','Tekst wiadomości');
define('_MAIL_TRANS','Transport');
define('_MAIL_FROMNAME','Nazwa nadawcy');
define('_MAIL_NAMEINFO','Puste — zostanie użyta nazwa witryny');
define('_MAIL_FROMMAIL','Adres nadawcy');
define('_MAIL_MAILINFO','Puste — zostanie użyty adres administratora');
define('_MAIL_REPLYTO','Adres do odpowiedzi');
define('_MAIL_REPLYINFO','Puste — zostanie użyty nadawca każdej wiadomości');
define('_MAIL_HOST','Serwer SMTP');
define('_MAIL_PORT','Port SMTP');
define('_MAIL_SECURE','Szyfrowanie');
define('_MAIL_AUTH','Uwierzytelnianie');
define('_MAIL_PASSINFO','Pozostaw puste, aby zachować zapisane hasło');
define('_MAIL_TIMEOUT','Limit czasu połączenia, sekundy');
define('_MAIL_SENDPATH','Ścieżka do Sendmail');
define('_MAIL_PATHINFO','Tylko ścieżka do programu, bez argumentów');
define('_MAIL_TEST','Wiadomość testowa');
define('_MAIL_TESTINFO','Najpierw zapisz ustawienia. Pusty adres wyśle wiadomość na adres administratora');
define('_MAIL_TESTOK','Transport przyjął wiadomość testową');
define('_MAIL_TESTWAIT','Wiadomość jest w kolejce, ale ten przebieg nic nie wysłał — sprawdź limit wysyłki');
define('_MAIL_NOSSL','Brak rozszerzenia openssl, połączenia szyfrowane są niedostępne');
define('_MAIL_NOPROC','proc_open jest wyłączony na tym hostingu, Sendmail jest niedostępny');
define('_MAIL_BADMAIL','Nieprawidłowy adres e-mail');
define('_MAIL_BATCH','Wiadomości na jedno uruchomienie');
define('_MAIL_RATE','Wiadomości na minutę, 0 wyłącza limit');
define('_MAIL_TRIES','Próby dostarczenia');
define('_MAIL_BACKOFF','Odstęp przed ponowieniem, sekundy');
define('_MAIL_KEEP','Przechowywanie dostarczonych wiadomości, dni');
define('_MAIL_KEEPBULK','Przechowywanie dostarczonych mailingów, dni');
define('_MAIL_VERIFY','Sprawdzaj adresy przed wysyłką');
define('_MAIL_VERIFYI','Składnia, zapisane odrzucenia i istnienie domeny. Niedostępne sprawdzenie jest pomijane, a list i tak wychodzi');
define('_MAIL_DNSTTL','Przechowuj wynik sprawdzenia domeny, sekund');
define('_MAIL_PEND','W kolejce');
define('_MAIL_HELD','Wstrzymane');
define('_MAIL_ACCEPT','Przyjęte przez transport');
define('_MAIL_FAILED','Odrzucone');
define('_MAIL_RETRY','Wyślij ponownie');
define('_MAIL_NODRAIN','Zadanie harmonogramu "maildrain" jest wyłączone — listy będą się gromadzić i nigdy nie wyjdą');
define('_MAIL_STALE','Kolejka dawno nie była opróżniana, ostatni przebieg:');
define('_MAP_AUTO','Automatyczne generowanie i aktualizacja pliku mapy?');
define('_MAP_AUTO_T','Czas między aktualizacjami pliku mapy, w godzinach');
define('_MAP_DAT_C','Podać datę ostatniej zmiany kategorii?');
define('_MAP_DAT_H','Podać datę ostatniej zmiany głównej?');
define('_MAP_DAT_M','Podać datę ostatniej modyfikacji modułów?');
define('_MAP_DAT_P','Podać datę ostatniej modyfikacji publikacji?');
define('_MAP_FR_C','Częstotliwość odświeżania kategorii modułów');
define('_MAP_FR_H','Częstotliwość aktualizacji strony głównej');
define('_MAP_FR_M','Częstotliwość aktualizacji modułów serwisu');
define('_MAP_FR_P','Częstotliwość aktualizacji publikacji w modułach');
define('_MAP_GEN_C','Generować linki do kategorii?');
define('_MAP_GEN_H','Wygenerować link na stronie?');
define('_MAP_GEN_M','Generować linki na moduły?');
define('_MAP_GEN_P','Generować linki do publikacji?');
define('_MAP_PR_C','Priorytet kategorii modułów');
define('_MAP_PR_H','Priorytet strony głównej');
define('_MAP_PR_M','Priorytet modułów serwisu');
define('_MAP_PR_P','Pierwszeństwo publikacji w modułach');
define('_MAP_SITE','Tworzenie typową strukturę mapy do oglądania na miejscu?');
define('_MAP_XSL','Użyć szablon XSL do pliku mapy?');
define('_MASSMAIL','Wszyscy użytkownicy');
define('_MATCHANY','Filtrowanie słów w tekście');
define('_MESSAGE_BOX','Wyświetlanie wiadomości na stronie głównej?');
define('_MODULES','Moduły');
define('_MODULESADMIN','Panel do zarządzania modułami');
define('_MODADMINPANEL','Administracyjne i systemowe moduły panelu');
define('_MODSITEPANEL','Użytkownika i publiczne moduły witryny');
define('_MONTHLY','Co miesiąc');
define('_MULTI','Zgoda na możliwość wyboru kilku opcji?');
define('_MULTILINGUAL','Językowe');
define('_MVIEW','Szczegółowy przegląd');
define('_NAME_BLOCK','Zabro');
define('_NAME_ID','Identyczne nazwy o których mowa');
define('_NAME_REF','Nazwy, o których mowa');
define('_NEVER','Nigdy');
define('_NEWSLETTER','Newsletter');
define('_NEW_USER','Nowi użytkownicy');
define('_NLABORT','Udział twardych odrzuceń zatrzymujący wysyłkę, procent');
define('_NLABORTI','Wysyłka zatrzymuje się, gdy udział odrzuceń adresatów osiągnie tę wartość');
define('_NLACTIVE','Użytkownicy obecni w ostatnich dniach');
define('_NLBOUNCE','Odrzuceń, zanim adres wypadnie z wysyłek masowych');
define('_NLBOUNCEI','Listy transakcyjne na ten adres nadal są wysyłane');
define('_NLBREAK','Okno oceny odrzuceń, listów');
define('_NLBREAKI','Udział liczony jest po ostatnich wynikach wysyłki i oceniany dopiero, gdy jest ich tyle');
define('_NLBUSY','Nie można usunąć wysyłki: jej listy są jeszcze w kolejce');
define('_NLCANARY','Wielkość partii próbnej');
define('_NLCANARYI','0 wyłącza partię próbną');
define('_NLCANMIN','Najmniejsza grupa odbiorców z partią próbną');
define('_NLCANMINI','Mniejsza grupa wysyłana jest w całości, bez próby');
define('_NLCLONE','Skopiuj do nowej wysyłki');
define('_NLDAYS','Okno aktywności, dni');
define('_NLDAYSI','Używane tylko przez grupę wybraną po ostatniej wizycie');
define('_NLEND','Wysłane');
define('_NLFAILS','Odrzuceń');
define('_NLFREE','Wyślij resztę odbiorców');
define('_NLLOCKED','Rozpoczęta wysyłka nie jest edytowana. Zapis utworzy nową wysyłkę z tym tekstem');
define('_NLNOAUDIT','Nie wybrano odbiorców albo wybór już nie istnieje');
define('_NLNOJOB','Zadanie harmonogramu "newsletter" jest wyłączone — wysyłki nie zostaną przygotowane');
define('_NLNOTE','Powód zatrzymania');
define('_NLQUEUE','Kolejka');
define('_NLPROGRESS','Przygotowano');
define('_NLRATE','odrzuceń');
define('_NLSTCANAR','Partia próbna');
define('_NLSTDONE','Zakończona');
define('_NLSTDRAFT','Szkic');
define('_NLSTHELD','Czeka na decyzję');
define('_NLSTLIST','Lista adresów poprzedniej wersji');
define('_NLSTOP','Zatrzymaj wysyłkę');
define('_NLSTOPPED','wysyłka zatrzymana ręcznie');
define('_NLSTPREP','Przygotowanie');
define('_NLSTQUEUE','W kolejce');
define('_NLSTRUN','Wysyłana');
define('_NLSTSTOP','Zatrzymana');
define('_NLUSER','użytkownicy');
define('_NLWHERE','Biuletyn otrzymają');
define('_NOALINKP','Zakaz automatycznej konwersji linków');
define('_NOAUM','Akcja ta nie ma zastosowania do głównego administratora i adminów odpowiednich modułów.');
define('_NOKOMA','Jako separatora należy używać przecinkami bez spacji.');
define('_NOLINKP','Zakaz publikowania linków');
define('_NONUMVALUE','Określonej właściwości punktów nie jest liczbą!');
define('_NO_MAIL_REG','Rejestracja bez potwierdzenia na E-Mail?');
define('_NO_SICHT','Niewiem w bloku modułów');
define('_NULLPOINTS','Wyzeruj ustawienia');
define('_OAUTHACT','Aktywować autoryzację OAuth przez usługi zewnętrzne?');
define('_OAUTHGACT','Aktywować logowanie przez Google?');
define('_OAUTHGID','Google Client ID');
define('_OAUTHGKEY','Google Client Secret');
define('_OAUTHLIST','Powiązania OAuth');
define('_OAUTHMACT','Aktywować logowanie przez Microsoft?');
define('_OAUTHMID','Microsoft Client ID');
define('_OAUTHMKEY','Microsoft Client Secret');
define('_OPCL','Minimalizuj / Maksymalizuj');
define('_OPTIMIZE','Optymalizacja');
define('_OPTIMIZED','Zoptymalizowany');
define('_OPTTEXT','Optymalizacja przebudowuje tabele i aktualizuje wewnętrzne statystyki, co czasami może poprawić wydajność. W nowoczesnych tabelach InnoDB funkcja ta jest potrzebna tylko po dużych zmianach danych (np. masowym usuwaniu), a nie regularnie.');
define('_PARTNERS','Partnerzy');
define('_PASSWDLEN','Minimalna długość hasła');
define('_PERMISSIONS','Uprawnienia');
define('_PHPSETUP','Wersja PHP zainstalowany na Twoim serwerze nie spełnia minimalne wymagania systemu, powinna być nie niższa PHP %1$s!');
define('_POINTSNEEDED','Minimalna wymagana ilość punktów');
define('_POINTS_DIFF','Zmiana salda, ze znakiem');
define('_POINTS_NOTE','Powód zmiany');
define('_POINTS_BADDIFF','Zmiana salda musi być liczbą całkowitą od -1000000 do 1000000 różną od zera i wymaga powodu.');
define('_POINTS_BADSAVE','Saldo nie zostało zmienione: system punktów odrzucił korektę.');
define('_POINTS_RESETMORE','Resetowanie nie zostało zakończone. Wyślij formularz ponownie, aby kontynuować.');
define('_POINTS_JOURNAL','Dziennik punktów');
define('_POINTS_NOMARK','Aktualizacja danych 6.3 nie została wykonana: punkty są zamknięte do zapisu.');
define('_POINTS_SOURCE','Źródło');
define('_POINTS_SCOPE','Kontekst');
define('_POSITION','Pozycja');
define('_PRACT','Aktywacja prywatne wiadomości?');
define('_PREFERENCES','Konfiguracji');
define('_PRINM','Maksymalna liczba przychodzących wiadomości');
define('_PRMAIL','Powiadamiaj użytkownika o nowych wiadomościach?');
define('_PROFACT','Aktywacja przeglądanie profilu tylko dla zarejestrowanych użytkowników?');
define('_PRSAVEM','Maksymalna liczba zapisanych wiadomości');
define('_PRSELF','Wyłącz wysyłanie wiadomości do siebie?');
define('_PUBHOME','Pokaż na stronie głównej?');
define('_PUTINAHOME','Moduł ładowany na stronie głównej admin');
define('_PUTINHOME','Moduł ładowany na stronie głównej');
define('_PUTINHOMEINFO','W przypadku wybrania kilku opcji, moduły będą wyświetlane w tryb losowy.');
define('_RATINGS_GUESTS','Pozwolić gościom oceniać?');
define('_RATINGS_NOMARK','Aktualizacja danych 6.3 nie została wykonana: oceny są zamknięte do zapisu.');
define('_RATINGS_BADDAYS','Odstęp musi być całkowitą liczbą dni, zero lub więcej.');
define('_RATINGS_VOTES','Dziennik ocen');
define('_RATINGS_ANNUL','Anuluj');
define('_RATINGS_REASON','Powód anulowania');
define('_RATINGS_DONE','Ocena została anulowana.');
define('_RATINGS_ACTOR','Uczestnik');
define('_RATINGS_TARGET','Cel');
define('_REFER','Aktywacja statystyki przejścia z innych stron?');
define('_REFERB','Rejestrowanie przejścia tylko z wyszukiwarek internetowych?');
define('_REFERERS','Przejścia');
define('_REFER_T','Czas zachowania przejścia do innych stron, jest bezpłatny');
define('_REFINFO','Ma znaczenie tylko w przypadku korzystania z kanału RSS.');
define('_REFRESHTIME','Czas aktualizacji');
define('_REF_ID','Identyczne przejścia');
define('_REF_URL','Adresy przejścia');
define('_REGACT','Włączyć rejestrację nowych użytkowników?');
define('_REPAIR','Naprawy');
define('_REPLACE','Wymiana słów');
define('_REPLACEIN','Słowo zostanie zastąpione przez wpisana zawartość tekstową.');
define('_REPLACEINFO','W treści wymiany dopuszczalne wykorzystania, jak zwykły tekst, jak i HTML. Aby wyłączyć wymiany należy podać wartość «0» w słowie i zawartości pola.');
define('_REPLACE_FIELD','Wymiana');
define('_REPTEXT','W nowoczesnych tabelach InnoDB odzyskiwanie danych po awarii odbywa się automatycznie. Funkcja naprawy powinna być używana tylko w przypadku faktycznych błędów tabel.');
define('_REWRITE_MOD','Aktywacja konwersja linków w Rewrite Mod?');
define('_FORCESSL','Zawsze używaj HTTPS (przekieruj z http)?');
define('_FORCEHOST','Jedna domena (przekieruj www na główną)?');
define('_RIGHT','Po prawej stronie');
define('_RIGHTBLOCK','Prawy blok');
define('_ROWS','Wierszy');
define('_RSSACT','Aktywować wyjście z kanałów na stronach serwisu');
define('_RSSC','Kanał RSS');
define('_RSSDESC','Standardowe kanały, to Swoje kanały, które zostaną dodane do kodu czapki strony do wyświetlania informacji z innych witryn, przeglądarkach lub innymi odpowiednimi programami. Zewnętrzne kanały, to kanały innych stron, które zostaną dodane do Twojej listy kanałów modułu, do bezpośredniego przeglądania użytkowników witryny. W celu wyłączenia kanału wprowadź wartość «0» w polach nazwa i adres.');
define('_RSSFAIL','Problemy z RSS plikiem, sprawdź link!');
define('_RSSFILE','Link do kanału RSS');
define('_RSSHOME','Standardowy kanał');
define('_RSSINFO','Podczas korzystania z kanału RSS, jest zapisywana zawartość tekstowa zostanie zignorowane.');
define('_RSSLINESINFO','Wybierz domyślny, podaj adres lub po prostu wybierz z listy do transmisji kanału RSS.');
define('_RSSMAX','Maksymalna liczba domyślnie');
define('_RSSMIN','Minimalna ilość domyślnie');
define('_RSSSITE','Zewnętrzny kanał');
define('_RSSUSE','Aktywacja przeglądanie kanałów RSS?');
define('_RULACT','Potwierdzenie zasad podczas rejestracji użytkowników?');
define('_SCRIPTASIN','Asynchroniczne ładowanie skryptów?');
define('_SCRIPTBOT','Przesuń w dół skryptów stronie?');
define('_SCRIPTFILE','Pliki skryptów');
define('_SCRIPTFILEINFO','Podawać bezpośrednio pliki skryptów.');
define('_SCRIPTHEAD','Wyświetlanie treści skryptów w nagłówku strony?');
define('_SEARCHLETINFO','Aktywnie szukać modułu, jak również w funkcji prosi użytkownika podczas wyszukiwania w module osobistych wiadomości i panelu administracyjnego.');
define('_USERSEARCHTIP','Wpisz co najmniej %d znaki, aby zobaczyć podpowiedzi.');
define('_SEARCHLIMIT','Maksymalna długość tekstu do wyszukiwania (znaki)');
define('_SEARCHLIMITINFO','Ogranicza liczbę znaków przekazywanych do zapytania wyszukiwania, aby chronić bazę danych przed przeciążeniem.');
define('_SEARCHCLEAR','Usuń statystyki wyszukiwania');
define('_SEARCHCLEARINFO','Ta akcja usuwa wszystkie zapisane zapytania wyszukiwania ze statystyk wyszukiwania.');
define('_SEARCHEMPTY','Usuń puste zapytania');
define('_SEARCHBYMOD','Usuń według modułu');
define('_SEARCHBYDAY','Usuń starsze niż');
define('_SEARCHLAST','Ostatnie zapytanie');
define('_SEARCHTOP','Top zapytania');
define('_SEARCHTOPMOD','Top moduł');
define('_SEARCHTOTAL','Wszystkie zapytania');
define('_SEARCHUNIQUE','Unikalne zapytania');
define('_SEARCHNUM','Liczba wyników wyszukiwania na stronę');
define('_SEC_ADMIN_IP','Dozwolone zakresy IP administratora (CIDR)');
define('_SEC_AUTH_INFO','Włączenie dodatkowego uwierzytelniania HTTP (login/hasło) znacząco podnosi poziom bezpieczeństwa systemu i zmniejsza ryzyko nieautoryzowanego dostępu do panelu administracyjnego. Uwierzytelnianie odbywa się na poziomie serwera WWW i działa jeszcze przed załadowaniem CMS, co pozwala skutecznie blokować zautomatyzowane ataki oraz próby odgadywania hasła. Funkcja jest obsługiwana przez większość nowoczesnych serwerów WWW (Apache, Nginx, IIS) i zwykle konfigurowana przez ustawienia serwera lub plik .htaccess. W razie potrzeby skonsultuj wymagane ustawienia ze swoim dostawcą hostingu.');
define('_SEC_AUTH_OK',"Dodatkowy login i hasło zostały poprawnie skonfigurowane. Aby je wyłączyć lub zmienić, otwórz plik: config/security.php i ustaw wartości zmiennych na: 'login' => '' oraz 'password' => ''");
define('_SEC_COOKIE','Nazwa Cookies do zablokowanych użytkowników');
define('_SEC_DB','Automatyczne kopiowanie bazy danych?');
define('_SEC_HACK_STAT','Prowadzenie statystyk ataków?');
define('_SEC_LOG','Prowadzenie statystyk całej napływających informacji?');
define('_SEC_LOGIN','Dodatkowy login administratora');
define('_SEC_LOG_A','Prowadzenie statystyk wejść adminów?');
define('_SEC_LOG_D','Prowadzenie statystyk modyfikacji plików?');
define('_SEC_DUMP_SKIP','Lista wykluczeń do kontroli struktury plików');
define('_SEC_DUMP_SKIP_INFO','Jedna ścieżka względna w wierszu, katalogi mogą kończyć się znakiem /, np. .git/ lub vendor/cache/');
define('_SEC_LOG_DB','Odstęp czasu zapasowej bazy danych, w odległości krótkiego');
define('_SEC_LOG_DS','Czas skanowania plików na modyfikacje, w odległości krótkiego');
define('_SEC_LOG_SIZE','Maksymalny rozmiar plików statystyki, w bajtach');
define('_SEC_LOG_U','Prowadzenie statystyk wejść użytkowników?');
define('_SEC_MAIL_D_SEND','Informacja o modyfikacji plików na E-Mail?');
define('_SEC_MAIL_SEND','Informacja o atakach na E-Mail?');
define('_SEC_MAIL_W_SEND','Informacja o blokowanych działaniach na E-Mail?');
define('_SEC_PASS','Login i hasło');
define('_SEC_PASSWORD','Opcjonalne hasło administratora');
define('_SEC_REF_POST','Wyłącz przyjmować zmienne POST od innych stron, źródeł?');
define('_SEC_STAT','Włączyć dziennik błędów?');
define('_SEC_STAT_A','Dziennik logowań administratorów');
define('_SEC_STAT_DB','Dziennik zmian bazy danych');
define('_SEC_STAT_DUM','Struktura plików');
define('_SEC_STAT_DUML','Dziennik zmodyfikowanych plików');
define('_SEC_STAT_ERROR_D','Dziennik błędów dynamicznych');
define('_SEC_STAT_ERROR_FILE','Dziennik błędów plików');
define('_SEC_STAT_ERROR_S','Dziennik błędów statycznych');
define('_SEC_STAT_ERROR_SQL','Dziennik błędów bazy danych');
define('_SEC_STAT_HACK','Dziennik ataków');
define('_SEC_STAT_LOG','Dziennik przychodzących żądań');
define('_SEC_STAT_U','Dziennik logowań użytkowników');
define('_SEC_STAT_WARN','Dziennik zabronionych działań');
define('_SEC_URL_GET','Zakazu podejmowania przez linki GET?');
define('_SEC_URL_POST','Zablokować przyjmowanie linki z POST?');
define('_SEC_VIEW','Komunikaty o błędach PHP');
define('_SEC_VIEW_1','Zgłoś wszystkie błędy z wyjątkiem powiadomień');
define('_SEC_VIEW_2','Zgłoś wszystkie błędy');
define('_SEC_WARN_BLOCK','Zablokuj napastników?');
define('_SEC_WARN_STAT','Prowadzenie statystyk czynów zabronionych?');
define('_SELLANGUAGE','Język standardowy');
define('_SELTHEME','Zezwól na zmianę tematy?');
define('_SEO','SEO');
define('_SESSION','Aktywacja sesji odwiedzających?');
define('_SESS_T','Czas pracy sesji, minut');
define('_SFLOD_T','Przerwa między testami atakującego, w sekundach');
define('_SFLOOD','Ochrona przed atakami przepełnienia, typu «Flood Atack», «DDoS Atack»');
define('_SFLOOD_1','Sprawdzać przesyłanie za pomocą zmiennej POST');
define('_SFLOOD_2','Sprawdzać przesyłanie za pomocą zmiennych GET i POST');
define('_SFLOOD_3','Sprawdzić wszystkie przychodzące i wymaganych informacji');
define('_SHOWINMENU','Zrobić widoczny w bloku moduły?');
define('_ADMINLOGO','Instalacja logo panelu administratora');
define('_ADMINLOGOP','Logo panelu administratora');
define('_SITELOGO','Logo strony');
define('_SITE_CLOSE','Zamknąć stronę internetową na profilaktykę?');
define('_SMAIL','Otrzymywać e-maile i powiadomienia?');
define('_SORT','Sortowanie');
define('_SORTORDER','Kolejność');
define('_SORTE','Sortuj');
define('_STATISTIC','Statystyki');
define('_STATACT','Aktywacja statystyki?');
define('_STATBET','Odległość między belkami');
define('_STATFROM','Pokaż statystyki za');
define('_STATGR','Harmonogram wizyt w tygodniu');
define('_STATSHI','Szerokość belki');
define('_STYLES','Style');
define('_SUBCATEGORY','Pod-kategoria');
define('_ACCESS','Dostęp');
define('_SUPERUSER','Pełny dostęp');
define('_SWORD','Treść zapytania');
define('_SYNTAX','Podświetlanie składni kodu');
define('_SYNTAXP','Standardowa metoda PHP');
define('_SYNTAXPN','Standardowa metoda PHP z numeracją');
define('_SYNTAXSH','Biblioteka Syntax Highlighter');
define('_SYSTEM','System');
define('_TABLE','Pozycja na liście');
define('_TEMPLATE','Szablon');
define('_TEMPLATES','Szablony');
define('_TIME','Czas podawany jest w bezpłatny');
define('_TIMENL','Zajęło czasu');
define('_TIME_DB','Wyświetl szybkość generowania strony?');
define('_TIME_ID','Jednoczesne przejścia');
define('_TIME_REF','Czas przejścia');
define('_TOTALFREE','Ogólne koszty');
define('_TOTALSPACE','Całkowity rozmiar bazy danych');
define('_TPFOR','Wzór dla formatu');
define('_TPFORM','Formaty załączników z szablonem wyświetlania');
define('_TPFORMIN','Dla których rozszerzeń karta Szablony pokazuje edytor szablonu wyświetlania. Po przecinku, bez kropki i bez spacji. Obsługiwane:');
define('_TPHEIGHT','Domyślna wysokość załącznika, wstawiana w [height]');
define('_TPINFO','<b>[src]</b> - Link do pliku<br><b>[tsrc]</b> - Link do pliku podglądu<br><b>[width]</b> - Szerokość aplikacji<br><b>[twidth]</b> - Szerokość podglądu<br><b>[height]</b> - Wysokość aplikacji<br><b>[theight]</b> - Wysokość podglądu<br><b>[align]</b> - Położenie w stosunku do treści<br><b>[title]</b> - Nagłówek aplikacji<br><b>[243]</b> - Cudzysłowy<br><b>[rel]</b> - atrybut Opcjonalny');
define('_TPWIDTH','Domyślna szerokość załącznika, wstawiana w [width]');
define('_UGROUP','Dla użytkowników składających się w grupie');
define('_GROUPS','Grupy');
define('_UNIQUE','Unikalne');
define('_UPDATE_POINTS','Aktywacja liczenie punktów użytkowników?');
define('_UPLOADS','Pobierania');
define('_URLS','Linki');
define('_USEREXIST','Użytkownik już istnieje!');
define('_USERIPCHECK','Określenia użytkownika na ostatni Adres IP?');
define('_USERSCOUNT','Uczestników');
define('_USERSHOMENUM','Umożliwić użytkownikom zmieniać liczbę postów?');
define('_USER_COOKIE','Nazwa Cookies dla użytkowników');
define('_USER_COOKIE_T','Czas przechowywania plików Cookies przez użytkownika, jest bezpłatny');
define('_USES','Zastosowanie');
define('_VANSW','Liczba możliwych odpowiedzi');
define('_VARIABLES','Aktywacja rozszerzeń naprawy');
define('_VAR_VIEW','Widoczność funkcje debugowania');
define('_VBLOCK','Wyświetlanie bloku ankietach');
define('_VCLOSED','Wyłączyć głosowanie');
define('_VDEACT','Dezaktywować ankieta');
define('_VIEWCAT','Wyświetlanie kategorii w szczegółowym widoku?');
define('_VIEWPRIV','Kto może zobaczyć?');
define('_VIEWSITE','Zobacz na stronie');
define('_VLASTACT','Ostatnio aktywny');
define('_VLASTCLO','Ostatnio ukończony');
define('_VOTING_TIME','Zakres udział w rankingu jest bezpłatny');
define('_VPRIVAT','Rasa przycisk prywatną wiadomość do użytkownika?');
define('_VPROFIL','Rasa przycisk przejść do profilu użytkownika?');
define('_VRANACT','Przypadkowe aktywny');
define('_VRANCLO','Przypadkowe ukończony');
define('_VWEB','Rasa przycisk przejścia na stronie użytkownika?');
define('_WEEKLY','Co tydzień');
define('_WEIGHT','Położenie');
define('_WORD','Słowo');
define('_XSL_INFO','Plik: %1$s jest mechanizm arkusze stylów, specjalnie przystosowany do pracy z XML plikiem mapy witryny dla wyszukiwarek i zapewnia wizualne konwersja.');
define('_YEARLY','Raz w roku');
define('_ADMIN_FILE','Nazwa pliku do wprowadzenia do działu administracyjnego');
define('_ADMIN_FILE_INFO','Ze względów bezpieczeństwa zmień domyślną nazwę pliku admin.php. Zmień jego nazwę na jakąkolwiek wymyślisz, na przykład:%1$s.php i podaj nową nazwę, ale bez końcówki «.php» w naszym przypadku: %1$s');
define('_SMODULE','Użyj wyszukiwania w modułach');
define('_ASEARCH','Aktywować analizator wyszukiwarek?');
define('_LTITLE','Używać długich tytułów?');
define('_ADESC','Automatyczne generowanie opisu?');
define('_DLETTER','Maksymalna liczba znaków w opisie');
define('_RSEP','Separator URL');
define('_TSEP','Separator tytułów dla adresów SEO');
define('_SEOTITLE','Dodać tytuł?');
define('_SEOCTITLE','Dodać tytuł kategorii?');
define('_OGRAPH','Włączyć Open Graph?');
define('_OGRAPHT','Szablon Open Graph');
define('_SCHEMA','Włączyć Schema.org?');
define('_SCHEMAT','Szablon Schema.org');
define('_SEO_TPL_INVALID','Nieprawidłowy szablon SEO');
define('_TPLVARS','<b>[site]</b> - Nazwa strony<br><b>[loc]</b> - URL strony<br><b>[title]</b> - Tytuł SEO (artykuł + kategoria + moduł + strona)<br><b>[desc]</b> - Opis<br><b>[img]</b> - Obraz<br><b>[type]</b> - Typ treści<br><b>[url]</b> - URL<br><b>[homeurl]</b> - Strona główna<br><b>[logo]</b> - Logo<br><b>[time]</b> - Data publikacji<br><b>[mtime]</b> - Data modyfikacji<br><b>[ctitle]</b> - Kategoria<br><b>[headline]</b> - Nagłówek artykułu (tylko tekst, bez sufiksów)<br><b>[author]</b> - Autor artykułu');
define('_ADMINPASSKEEP','Pozostaw puste, aby zachować bieżące hasło');
define('_ADMINSELFDEL','Nie można usunąć bieżącego konta administratora');
define('_ADMINSELFSUPER','Nie można odebrać pełnego dostępu bieżącemu kontu administratora');
define('_ADMINLASTSUPER','Nie można usunąć ani zdegradować ostatniego administratora z pełnym dostępem');
define('_SEARCHAUTO','Automatyczne dodawanie zgodnych modulow');
define('_SEARCHAUTOINFO','Dodaje do wyszukiwania wszystkie aktywne moduly ze zgodna struktura tabeli');
define('_SEARCHAUTODONE','Liczba aktywowanych modulow');
define('_SEARCHENABLED','Juz aktywne');
define('_SEARCHREADY','Dostepne do wlaczenia');
define('_SEARCHINVALID','Niezgodne');
define('_SEARCHTYPE','Typ');
define('_SEARCHFIELDS','Pola wyszukiwania');
define('_SEARCHEDIT','Sciezka edycji');
define('_SEARCHREASON','Powod');
define('_SEARCHSPECIAL','Specjalny');
define('_SEARCHSIMPLE','Auto-kompatybilny');
define('_SEARCHNOTABLE','Nie znaleziono tabeli');
define('_SEARCHMISSCOLS','Wymagane pola');
define('_SEARCHNOFIELDS','Wymagane jedno z: intro, body, url');
define('_SEARCHADDSEL','Dodaj wybrane');
define('_SEARCHADDALL','Dodaj wszystkie zgodne');
define('_SCHEDULER','Scheduler');
define('_SCHEDULER_LASTRUN','Ostatnie uruchomienie');
define('_SCHEDULER_LAST_OK','Ostatni sukces');
define('_SCHEDULER_NEXTRUN','Nastepne uruchomienie');
define('_SCHEDULER_TRIGGER','Trigger');
define('_SCHEDULER_RUNTIME','Czas trwania');
define('_SCHEDULER_FAILS','Bledy');
define('_SCHEDULER_SCHED','Harmonogram');
define('_SCHEDULER_SYSTEM','Systemowy');
define('_SCHEDULER_CUSTOM','Wlasny');
define('_SCHEDULER_PRIO','Priorytet');
define('_SCHEDULER_PRIOTIP','Im mniejsza liczba, tym wczesniej zadanie zostanie uruchomione.');
define('_SCHEDULER_LOCK','Maks. czas wykonania, sek.');
define('_SCHEDULER_UNLOCK','Odblokuj');
define('_SCHEDULER_RUN','Uruchom teraz');
define('_SCHEDULER_JOBKEY','Klucz zadania');
define('_SCHEDULER_URL','Docelowy URL');
define('_SCHEDULER_UNLOCKD','Blokada schedulera usunieta');
define('_SCHEDULER_RUNNING','Dziala');
define('_SCHEDULER_RESULT','Wykonanie');
define('_SCHEDULER_IDLE','Oczekuje');
define('_SCHEDULER_CRASH','Awaria');
define('_SCHEDULER_OFF','Wyłączone');
define('_SCHEDULER_URLINFO','Wzgledne URL sa rozwiazywane wzgledem adresu glownego strony');
define('_SCHEDULER_SYSINFO','Zadania systemowe mozna tutaj stroic przed usunieciem starych triggerow');
define('_SCHEDULER_MANUAL','Zezwol na reczny start');
define('_SCHEDULER_CRONFMT','Format cron: minuta godzina dzien miesiac dzien_tygodnia. Przyklady: 0 2 * * * | 0 3 * * 1 | 15 1 1 * *');
define('_SCHEDULER_WARN_DB','Automatyczna kopia zapasowa bazy danych jest wyłączona, więc zadanie schedulera nie zostanie uruchomione.');
define('_SCHEDULER_WARNLOG','Rejestrowanie skanowania plików jest wyłączone, więc zadanie schedulera nie zostanie uruchomione.');
define('_SCHEDULER_WARN_GO','Przejdź do ustawień bezpieczeństwa');
define('_SCHEDULER_LIMIT','Elementow na uruchomienie');
define('_MONITOR_TIME_AGO','%d sek.');
define('_MONITOR_TIME_NOW','Teraz');

# Captcha (ALTCHA)
define('_CAPTCHA','Captcha');
define('_CAPTCHA_ACTIVE','Włącz captcha');
define('_CAPTCHA_PROVIDER','Dostawca captcha');
define('_CAPTCHA_REGISTER','Chroń rejestrację');
define('_CAPTCHA_CONTACT','Chroń formularz kontaktowy');
define('_CAPTCHA_COMMENTS','Chroń komentarze i formularze treści');
define('_CAPTCHA_LOGIN_USER','Captcha przy logowaniu użytkownika');
define('_CAPTCHA_LOGIN_ADMIN','Captcha przy logowaniu administratora');
define('_CAPTCHA_LOGIN_HINT','Kiedy captcha pojawia się w formularzu logowania: nigdy, dopiero po kilku nieudanych próbach lub przy każdym logowaniu.');
define('_CAPTCHA_NEVER','Nigdy');
define('_CAPTCHA_AFTERFAIL','Po nieudanych próbach');
define('_CAPTCHA_ALWAYS','Zawsze');
define('_CAPTCHA_DIFFICULTY','Trudność');
define('_CAPTCHA_DIFFICULTY_HINT','Wyższa trudność zwiększa koszt proof-of-work dla odwiedzającego.');
define('_CAPTCHA_LOW','Niska');
define('_CAPTCHA_NORMAL','Normalna');
define('_CAPTCHA_HIGH','Wysoka');
define('_CAPTCHA_TTL','Czas życia wyzwania (sekundy)');
define('_CAPTCHA_TTL_HINT','Jak długo wydane wyzwanie pozostaje ważne.');
define('_SECRET_REGEN','Wygeneruj nowy klucz bezpieczeństwa');
define('_SECRET_REGEN_HINT','Generuje nowy globalny klucz przy zapisie i unieważnia wszystkie tokeny CSRF, wyzwania captcha oraz zaciemnione nazwy pól.');
define('_CAPTCHA_STORE_WARN','Katalog storage/captcha nie jest zapisywalny. ALTCHA nie może wygenerować sekretu ani weryfikować rozwiązań.');
define('_UPLOADS_SYSTEM','Pliki systemowe');
define('_UPLOADS_ROOT','Witryna');
define('_UPLOADS_TREE','Drzewo witryny');
define('_UPLOADS_PROPS','Właściwości');
define('_UPLOADS_UP','Poziom wyżej');
define('_UPLOADS_FILTER','Filtr w tym katalogu');
define('_UPLOADS_TILES','Kafelki');
define('_UPLOADS_PATH','Ścieżka');
define('_UPLOADS_FULL','Pełna ścieżka');
define('_UPLOADS_DIMS','Rozdzielczość');
define('_UPLOADS_OPEN','Otwórz');
define('_UPLOADS_GRANTS','Dostępne');
define('_UPLOADS_BROWSE','przeglądanie');
define('_UPLOADS_PREVIEW','podgląd');
define('_UPLOADS_UPLOAD','wysyłanie');
define('_UPLOADS_DOWNLOAD','pobieranie');
define('_UPLOADS_EDIT','edycja tekstu');
define('_UPLOADS_CREATE','tworzenie pliku');
define('_UPLOADS_MKDIR','tworzenie katalogu');
define('_UPLOADS_RENAME','zmiana nazwy');
define('_UPLOADS_COPY','kopiowanie');
define('_UPLOADS_MOVE','przenoszenie');
define('_UPLOADS_DELETE','usuwanie');
define('_UPLOADS_COMPRESS','archiwizacja');
define('_UPLOADS_EMPTY','Katalog jest pusty');
define('_UPLOADS_EMPTYTXT','Nie ma tu jeszcze plików ani podkatalogów.');
define('_UPLOADS_NOFIND','Nic nie znaleziono');
define('_UPLOADS_NOFINDTXT','Żaden obiekt tego katalogu nie pasuje do filtru.');
define('_UPLOADS_CRIT','Plik uczestniczy w pracy witryny: błąd w nim zatrzymuje witrynę.');
define('_UPLOADS_VERSION','Wersja');
define('_UPLOADS_LINES','Wiersze');
define('_UPLOADS_VERHINT','Wersja jest wydawana przy otwarciu; przy zapisie serwer ją porównuje i odrzuca zapis, jeśli plik zmienił ktoś inny');
define('_UPLOADS_CRITASK','Plik uczestniczy w pracy witryny: błąd w nim zatrzymuje witrynę. Zapisać zmiany? Operacja trafi do dziennika.');
define('_UPLOADS_STALE','Plik zmienił się po otwarciu, dlatego nic nie zapisano. Ponowny zapis nadpisze zmianę innego autora.');
define('_UPLOADS_NOEDIT','Tego pliku nie można otworzyć w edytorze źródeł.');
define('_UPLOADS_LEAVE','Zmiana nie została zapisana. Wyjść z edytora i ją stracić?');
define('_UPLOADS_TORENAME','Zmień nazwę');
define('_UPLOADS_TOCOPY','Kopiuj');
define('_UPLOADS_TOMOVE','Przenieś');
define('_UPLOADS_TOZIP','Spakuj do ZIP');
define('_UPLOADS_NEWFILE','Nowy plik');
define('_UPLOADS_NEWDIR','Nowy katalog');
define('_UPLOADS_TAKEN','Obiekt o tej nazwie już istnieje w tym katalogu.');
define('_UPLOADS_FILLED','Usuwany jest tylko pusty katalog: ten wciąż zawiera obiekty.');
define('_UPLOADS_BADDEST','Cel znajduje się wewnątrz samego obiektu.');
define('_UPLOADS_CRITDEL','Plik uczestniczy w pracy witryny: po usunięciu witryna może się zatrzymać. Operacja trafi do dziennika.');
define('_UPLOADS_DIRS','Katalogi wysyłania');
define('_UPLOADS_ADDR','Adres');
define('_UPLOADS_PERMS','Prawa dostępu');
define('_UPLOADS_USER','Właściciel na serwerze');
define('_UPLOADS_OWNER','Właściciel');
define('_UPLOADS_OWNSITE','Administracja');
define('_UPLOADS_LINKWARN','Zmiana nazwy i przeniesienie nie aktualizują odnośników w już opublikowanych materiałach: stary adres przestanie się otwierać.');
define('_UPLOADS_MARK','Zaznacz obiekt');
define('_UPLOADS_MARKALL','Zaznacz wszystkie obiekty');
define('_UPLOADS_MARKED','Zaznaczono');
define('_UPLOADS_UNMARK','Usuń zaznaczenia');
define('_UPLOADS_MANYDEL','Usunąć zaznaczone obiekty?');
define('_UPLOADS_LOAD','Wyślij plik');
define('_UPLOADS_DROP','Przeciągnij pliki tutaj lub kliknij');
define('_UPLOADS_QUEUE','Wysyłanie');
define('_UPLOADS_STOP','Anuluj wysyłanie');
define('_UPLOADS_TOPACK','Spakuj do jednego archiwum');
define('_UPLOADS_FAIL','Lista nie dotarła');
define('_UPLOADS_FAILTXT','Serwer nie odpowiedział na żądanie katalogu. Dotychczasowa zawartość pozostaje.');

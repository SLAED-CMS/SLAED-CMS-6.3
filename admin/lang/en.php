<?php
# Author: Eduard Laas
# 2005 - 2026 SLAED
# License: MIT
# Website: slaed.net

define('_ACESS','Access rights');
define('_ACESSF','The rights to the forum');
define('_ACESSI','You can set access rights for several teams, this is only possible for groups.');
define('_ACTIVATE2','Activate?');
define('_ACTMULTILINGUAL','Activate Multilingual features?');
define('_ACTUSEFLAGS','Display flags instead of a dropdown box?');
define('_ACTUSELANG','Automatically switch the language of the site to the country of the visitor?');
define('_ADDAMAIL','Notify administrator of new additions?');
define('_ADDCATEGORY','Add a New Category');
define('_ADDCONST','Provision for the use of language constants. The values of the language constants are available in the «Languages».');
define('_ADDNEWBLOCK','Add Block');
define('_ADDNEWFILEBLOCK','Add Block File');
define('_ADDRESS','Address');
define('_ADDSUBCATEGORY','Add under-category');
define('_ADIR','Path under your root dir');
define('_ADMCOL','The number of columns in the output modules the admin panel');
define('_ADMINEMAIL','Administrator Email');
define('_ADMINFOEDIT','Activate the editor of the Department of information?');
define('_ADMININFO','Note of the manager');
define('_ADMININFODES','The information which will be accessible to all authors. Display of the given information is made in the top part of the panel of the manager of system.');
define('_ADMINLOGIN','Administration System Login');
define('_ADMINLOGIN_NEW','To create the manager of a site');
define('_ADMIN_SESSION','Name sessions for managers');
define('_AFTEREXPIRATION','After Expiration');
define('_AKTIVE','Active');
define('_ALLOWANONPOST','Allow Anonymous to Post?');
define('_ALLMODULES','All modules');
define('_ALLUSER','All users');
define('_ALREADYOPTIMIZED','Already optimized');
define('_ALWAYS','Always');
define('_ANEWSLETTER','A Newsletter to subscribed users only');
define('_ANONIMP','Only anonymous users');
define('_APOSTMOD','With control of moderator');
define('_APOSTNOMOD','Without control by moderator');
define('_ASC','Ascending');
define('_ASIZE','Maximal size of a file of the image, in bytes');
define('_ASSOTOPIC','Associated Topics');
define('_ASSOTOPICI','If you use this option, the material is displayed in addition to the above categories.');
define('_ATYPE','Admissible picture-sizes');
define('_AUDIENCE','Audience');
define('_AUPLOAD','To resolve loading avatars on a server?');
define('_AUTH_DELETE','delete');
define('_AUTH_EDIT','edit');
define('_AUTH_MOD','moderate');
define('_AUTH_POST','create');
define('_AUTH_READ','read');
define('_AUTH_REPLY','reply');
define('_AUTH_VIEW','look');
define('_BANNED','Blocking');
define('_BANNED_IP','Blocked IP');
define('_BANNED_USERS','Blocked users');
define('_BANNER','Banner');
define('_BANNERDOWN','Bottom banner');
define('_BANNERUP','Top banner');
define('_BASCOL','The number of columns in the output of publications');
define('_BLOCK','Block');
define('_BLOCKFILE','Block File');
define('_BLOCKFILE2','File');
define('_BLOCKHTML','Block HTML');
define('_BLOCKRSS','Block RSS');
define('_BLOCKS','Blocks');
define('_BLOCKSYSTEM','System');
define('_BLOCKS_MOD','Display of blocks in the module');
define('_BLOCKS_MOD0','Activate the left and right blocks');
define('_BLOCKS_MOD1','Activate only the left blocks');
define('_BLOCKS_MOD2','Activate only the right blocks');
define('_BLOCKS_MOD3','Deactivate the left and right blocks');
define('_BLOCKS_MODC0','Activate the central top and bottom blocks');
define('_BLOCKS_MODC1','Activate only the central top blocks');
define('_BLOCKS_MODC2','Activate only the central bottom blocks');
define('_BLOCKS_MODC3','Deactivate the central top and bottom blocks');
define('_BLOCK_ALL','In all modules');
define('_BLOCK_VIEW','Display the block in modules');
define('_BOTSACT','To show search boats in the block of users?');
define('_BOTSINFO','List recognized system of search bots. For addition of a new bot use a following format of record: "a unique line from user-agent"="a name of a boat for display to a site". An example: googlebot=Google Bot');
define('_BOTSLIST','List of search bots');
define('_BOTSOPT','Crawlers');
define('_BOTSSITE','List of the sites of search engines');
define('_B_FEDIT','A file with this name already exists in the system. Note that after you save the file, block will be fully edited, the contents of this file will be replaced with actual.');
define('_CACESSI','The rights of the forum should be set up only in case if produced by category belongs to the module of the forum, otherwise, leave the default.');
define('_CACHE','Сache');
define('_CACHEINFO','Also gates the cache of parsed content. The second option limits page caching only, not that layer.');
define('_CACHEBROW','Browser cache duration, in days (0 - off)');
define('_CACHELOCK','Single-flight cache rebuild?');
define('_CACHELOCKINFO','When a cached page expires only one worker rebuilds it; others briefly receive the previous version. Reduces peak load under high traffic. Recommended for high-traffic projects.');
define('_CACHECSS','To cache the styles of the theme?');
define('_CACHECLEAR','Clear all cache');
define('_CACHESCRIPT','To cache the scripts?');
define('_CACHETIME','Cache lifetime, in seconds');
define('_CACHETIMEINFO','How long a built page and the bundled styles and scripts stay fresh before they are created again. The scheduled cleanup removes files older than this value times 24, and never sooner than a day.');
define('_CACHE_1','Home page and the material lists');
define('_CACHE_2','Only the main page');
define('_CAN','May');
define('_CATDRAGSORT','Drag the "Position" icon to reorder sibling categories per module. Subcategories follow their parent. Click the module header to collapse the group.');
define('_CDEFIS','The symbol that separates category');
define('_CEDITT','The period you are editing a message the user, in minutes');
define('_CENSOR','The forbidden words');
define('_CENSORMODE','Censor Mode');
define('_CENSORREPLACE','Replace Censored Words with');
define('_CENSORS','Censure');
define('_CENTERBLOCK','Center Block');
define('_CENTERDOWN','Center Down');
define('_CENTERUP','Center Up');
define('_CLEAN','Clear');
define('_CLICABLE','Convert addresses into hyperlinks?');
define('_CLICABLEINFO','Applies when processing text content.');
define('_CLIENTSM','Clients of the module');
define('_COLOR','Color');
define('_COMLETTER','As much as possible admissible quantity of signs in a word');
define('_COMMENTS_DRIFT','Comment counters disagree with the discussions below them. Targets: %d.');
define('_COMMENTS_SYNC','Repair counters');
define('_COMMENTS_FIXED','Counters repaired: %d.');
define('_CONFIG_BLOCKED','The restore cannot decide by itself, reason: %s. Check the operation files in storage/backup/config manually.');
define('_CONFIG_CLEAN','No interrupted configuration operation was found, the configuration is consistent.');
define('_CONFIG_OP','Operation');
define('_CONFIG_PENDING','A configuration operation was interrupted. New configuration saves are refused until it is restored.');
define('_CONFIG_PHASE','Phase');
define('_CONFIG_RESTORE','Restore configuration');
define('_CONFIG_RESTORED','The configuration was restored successfully.');
define('_CONFIG_VERDICT','Snapshot to apply');
define('_NODE','Content');
define('_NODE_STALE','The Node type %s was changed by another save meanwhile, nothing was written. Reload the page and repeat the change.');
define('_NODE_BAD','The Node type %s refused the change, nothing was written.');
define('_NODE_PROOF','The database decides which snapshot applies, by the version of the Node type %s.');
define('_NODE_MANAGE','Node: types and their settings');
define('_NODE_SETUP','The installation did not finish these Node sections: %s - a section was not created, not switched on or left without its starter material. The reason is written to the site log.');
define('_NODE_START_TITLE','Welcome to SLAED CMS');
define('_NODE_START_INTRO','The site is installed and ready to work. The sections for news, pages, questions and answers, files, links, media, documentation and others have been created from the standard Node profiles and are already switched on.');
define('_NODE_START_BODY','Sections are set up in the administration panel on the "Types" tab of the Node module: the list, features, resources and integrations of every section. There a new section is also created from a profile, cloned, exported and imported. This news can be edited or deleted.');
define('_CONFINES','Specify a value of «0» to remove all limitations.');
define('_CONST','Language constant');
define('_CREATEBLOCK','Create Block');
define('_CREATEUSERDATA','Create a normal user?');
define('_CSEND','The minimum period of time between publications, in seconds');
define('_CSSCOMP','Compress styles?');
define('_CSSDIR','Directory of styles');
define('_CSSDIRINFO','Specify only the directory where are stored the styles. The system will determine the necessary css file itself. <B>[theme]</b> - will be replaced with the current theme automatically.');
define('_CSSENC','Convert image styles to binary text?');
define('_CSSHEAD','Output the styles in the header?');
define('_CTRLINFO','For a choice of two and more variants keep a key «Ctrl or Strg».');
define('_CUSTOM','Standard');
define('_C_11','Displayed quantity connected');
define('_C_13','Quantity in the list');
define('_C_15','Deduce subcategories?');
define('_C_17','To display date of the publication?');
define('_C_18','To display quantity of viewings?');
define('_C_19','To display the best and popular?');
define('_C_20','To resolve search alphabetically?');
define('_C_21','To allow to estimate?');
define('_C_22','To allow to estimate only at detailed viewing?');
define('_C_23','To display connected?');
define('_C_32','To deduce the description of a category?');
define('_C_33','The number of publications on the homepage of the user');
define('_C_34','The number of publications on the page administrator');
define('_C_35','The maximum number of page numbers of users');
define('_C_36','The maximum number of page numbers administrator');
define('_DAILY','Daily');
define('_DATABASE','Database');
define('_DBCOLL','Comparison');
define('_DBFREE','Space Saved');
define('_DBINFO','The use of a standard prefix is not necessarily required. You can use the variable <b>{prefix}</b> instead. In this case, <b>{prefix}</b> will be automatically replaced with your table prefix. You can also use the variables <b>{engine}</b>, <b>{charset}</b>, and <b>{collate}</b> to automatically apply the desired table engine, character set, and collation when creating or updating database tables.');
define('_DBWARN','In case of incorrect request can fail in the database, so using this function, it is recommended that only experienced users of the system.');
define('_DB_DELETE','Removal of tables of a database');
define('_DB_INSTALL','Installation of tables of a database');
define('_DB_SYNC','Synchronize time «PHP timezone» and «MySQL timezone»?');
define('_DB_UPDATE','Updating of tables of a database');
define('_DB_MODE','Mode');
define('_DB_BLOCKS','Blocks');
define('_DB_ERRORS','Errors');
define('_DB_STOP','Stop block');
define('_DB_SQL','Query');
define('_DB_PARSE','Parse check');
define('_DB_PARSEMODE','Parse');
define('_DB_RUNMODE','Execute');
define('_DEAKTIVE','Inactive');
define('_DEVMODE','Developer Mode?');
define('_DEFIS','Symbol separating titles and headings');
define('_DELSETUP','With a view of increase of safety it is necessary to remove or rename a file of installation of system setup.php in the basic directory of a site!');
define('_DENYING','Denying of noted modules');
define('_DESC','Descending');
define('_DRAGSORT','Drag the "Position" icon to reorder blocks within a group. Free blocks form a separate group and use [block=id]; position and order do not apply to them.');
define('_DIR','Directory');
define('_DIRDEF','The directory opened by default');
define('_EDFILEA','The total number of files to the administrator of the');
define('_EDFILEG','The total number of files for the guest');
define('_EDFILEU','The total number of files for the user');
define('_EDITBLOCK','Edit Block');
define('_EDITORUSER','User editor');
define('_EDITVOTE','Edit poll');
define('_EFUNC','File of introduction in a kernel of system');
define('_EFUNCN','The core of the system');
define('_EHEAD','File the implementation of the head system');
define('_EHEADN','Head system');
define('_EHT','File of configurations of rules of transformations URL at a server level');
define('_EHTN','Server rewrite');
define('_EINFO','Here you can insert the code necessary to you which will be placed in a kernel of system.');
define('_EINFO2','Here you can insert the code necessary to you which will be introduced in head systems. As a rule, the given file use for introduction JavaScripts, meta tags in head.');
define('_EINFO4','Here you can change or add standard rules of transformations of address lines of a browser at a server level. Enabled it is supported only in case of established on your server Mod Rewrite.');
define('_EINFO5','Here you can change or add a text file in the root directory of the site, which recorded special instructions for search engines. These instructions may prohibit indexing certain sections or pages on the site, point to the correct «mirroring» the domain to recommend the search robot to observe a certain time interval between downloading documents from the server, etc. In this file, you can specify the parameters indexing site for all the robots at once, and for each search engine individually.');
define('_EINFOPHP','Using the standard syntax of PHP, except for setting the opening <b>&lt;?php</b> and closing <b>?&gt;</b> tag, the system will do it myself. If you enter the wrong code, possible failure of the system, so use this feature is only recommended for advanced users.');
define('_EROB','Firmware and parameter index for search engines');
define('_ERON','Rules for robots');
define('_EROBSTD','Standard');
define('_ESAVED','Saved to file');
define('_ERRORPERM','has no necessary sanctions for record on a server.<br>Establish necessary attributes');
define('_ERROR_SUBCAT','Not found any category for the module: %1$s. For the further work of this Department should create at least one major category.');
define('_EXECUTE','To execute action');
define('_EXPIRATION','Time of work in the day');
define('_FAVOR_ACT','Activate favorites?');
define('_FAVOR_UMAX','The maximum');
define('_FIELD','Additional field');
define('_FIELDAREA','Textarea');
define('_FIELDDATE','Date: year-month-day');
define('_FIELDIN','Is obligatory to filling');
define('_FIELDINFO','The technical name of a field and the key of an option are stored with every value and cannot be changed later: a lowercase Latin letter first, then letters, digits or an underscore, 32 characters at most. A title that starts with an underscore names a language constant. The limits count characters of a text, bound a number or a date, or count the chosen options of a list with several values. Switching a field off hides it and keeps its values; deleting it drops its values the next time their owner is saved.');
define('_FIELDINPUT','One line');
define('_FIELDS','Additional fields');
define('_FIELDS_BOOL','Switch: yes or no');
define('_FIELDS_INT','Whole number');
define('_FIELDS_DECIMAL','Decimal number');
define('_FIELDS_KEY','Technical name');
define('_FIELDS_DEFAULT','Default value');
define('_FIELDS_MULTI','Allow several values');
define('_FIELDS_LOW','Lower limit');
define('_FIELDS_TOP','Upper limit');
define('_FIELDS_SCALE','Digits after the point');
define('_FIELDS_ITEMS','Options');
define('_FIELDS_NOMARK','The 6.3 data update has not run: extra fields are closed for writing.');
define('_FIELDS_BAD','The definitions were refused, first error: %s');
define('_FIELDSELECT','The list with the choice of');
define('_FIELDTIME','Date: year-month-day hours:minutes');
define('_FILENAME','Filename');
define('_FILENAMEIN','In the case of file block of the input text content will be ignored.');
define('_FILE_M','The total number of files');
define('_FILE_S','General size of files');
define('_FIX','Sorting');
define('_FIXED','Fixed');
define('_FLY_FIX','Free fixed block');
define('_F_8','To allow to add files?');
define('_F_9','To allow to add to visitors files?');
define('_GDWIDTH','Width of the image of preliminary viewing');
define('_GENPREF','General');
define('_GEO_IP','Detect visitor country by IP?');
define('_GEOLOCATION','Geolocation');
define('_GEOIP_ANON','Anonymize IP before storage?');
define('_GEOIP_ASN','ASN');
define('_GEOIP_ASNDB','ASN DB');
define('_GEOIP_CACHE','GeoIP cache TTL, seconds');
define('_GEOIP_CONTINENT','Continent');
define('_GEOIP_COUNTRYDB','Country DB');
define('_GEOIP_COUNTRYNAME','Country name');
define('_GEOIP_ORG','Organization');
define('_GEOIP_PROVIDER','Provider');
define('_GEOIP_STORE','Store full IP address?');
define('_GEOIP_TESTIP','Test IP');
define('_FOUND','Found');
define('_NOTFOUND','Not found');
define('_PARAMETERS','Parameters');
define('_VALUE','Value');
define('_GROUPSI','Take the user to a special group can only by the system administrator. Add a new member of the group can be in the «Users». The function is available when you create a new or edit an existing user.');
define('_GRSINFO','If you choose, the minimum number of points will be ignored.');
define('_GTIME','The global system time');
define('_HOMCAT','Display category on the home?');
define('_HOMEPAGE','Your site');
define('_HOURLY','Hourly');
define('_ICON','Icon');
define('_ICONPICK','Choose icon');
define('_INFLY','Free block');
define('_INFLYINFO','Free blocks — output with the [block=id] tag');
define('_INFOCATDEL','Deleting a category is impossible as long as it has tied the content of materials module or it includes other subcategories.');
define('_INHOME','In Home');
define('_INQUIRY','Inquiry in a database');
define('_IN_ID','Identical inputs');
define('_IN_URL','Addresses of inputs');
define('_IP_ID','Identical IP passed');
define('_IP_LINK','The link to a site of check IP of the address');
define('_IP_CIDR','IP range (CIDR)');
define('_IP_CIDR_TIP','Use CIDR notation, for example: 203.0.113.0/24, 2001:db8::/64. Use a comma without spaces as separator.');
define('_IP_REF','IP passed');
define('_LADATE','Set the current date');
define('_LANGCOUNT','The number of new fields of linguistic constants');
define('_LANGKEY','API-Key');
define('_LANGTR','Language, which will be used translation');
define('_LANG','Language editor');
define('_LEFT','Left');
define('_LEFTBLOCK','Left Block');
define('_LIVE_UPD','Live blocks auto-refresh (seconds, 0 = off)');
define('_LHOME','Publish on the main');
define('_LNFIX','Remove the lock');
define('_LNHOME','Remove a publication on the main');
define('_LOCATION','Location');
define('_LOGINF','The direct form of an input');
define('_LOGINFL','Type of a greeting of anonymous visitors');
define('_LOGINL','The link to an input and registration');
define('_LOGO','Installation of a trade mark of a site');
define('_MAILOPT','Mail');
define('_MAILTEMP','The basic post pattern');
define('_MAILTEMPINFO','<b>[text]</b> - The maintenance which will be substituted by system');
define('_MAIL_BLOCK','Forbidden E-Mail addresses');
define('_MAIL_INFO','<b>[time]</b> - Lasting time up to the end of the blockage<br><b>[info]</b> - The reason of blocking');
define('_MAIL_PASS_INFO','<b>[login]</b> - Login of the user on the project<br><b>[pass]</b> - Password of the user on the project');
define('_MAIL_SEND','The written message on E-Mail the user is sent!');
define('_MAIL_SENDE','To send the notice on E-Mail the user?');
define('_MAIL_TEXT','The text from the message');
define('_MAIL_TRANS','Transport');
define('_MAIL_FROMNAME','Sender name');
define('_MAIL_NAMEINFO','Empty falls back to the site name');
define('_MAIL_FROMMAIL','Sender address');
define('_MAIL_MAILINFO','Empty falls back to the administrator address');
define('_MAIL_REPLYTO','Reply-To address');
define('_MAIL_REPLYINFO','Empty falls back to the sender of each message');
define('_MAIL_HOST','SMTP host');
define('_MAIL_PORT','SMTP port');
define('_MAIL_SECURE','Encryption');
define('_MAIL_AUTH','Authentication');
define('_MAIL_PASSINFO','Leave empty to keep the stored password');
define('_MAIL_TIMEOUT','Connection timeout, seconds');
define('_MAIL_SENDPATH','Sendmail path');
define('_MAIL_PATHINFO','The path to the binary only, without arguments');
define('_MAIL_TEST','Test message');
define('_MAIL_TESTINFO','Save the settings first. An empty address sends to the administrator address');
define('_MAIL_TESTOK','The transport accepted the test message');
define('_MAIL_TESTWAIT','The message is queued, but this run sent nothing — check the rate limit');
define('_MAIL_NOSSL','The openssl extension is missing, encrypted connections are unavailable');
define('_MAIL_NOPROC','proc_open is disabled on this host, Sendmail is unavailable');
define('_MAIL_BADMAIL','Invalid mail address');
define('_MAIL_BATCH','Messages per drain run');
define('_MAIL_RATE','Messages per minute, 0 disables the limit');
define('_MAIL_TRIES','Delivery attempts');
define('_MAIL_BACKOFF','Retry delay, seconds');
define('_MAIL_KEEP','Keep delivered mail, days');
define('_MAIL_KEEPBULK','Keep delivered mailings, days');
define('_MAIL_VERIFY','Check addresses before a mailing');
define('_MAIL_VERIFYI','Syntax, recorded failures and domain resolution. A check the host cannot run is skipped and the message is sent');
define('_MAIL_DNSTTL','Keep the domain verdict, seconds');
define('_MAIL_PEND','Pending');
define('_MAIL_HELD','Held');
define('_MAIL_ACCEPT','Accepted by the transport');
define('_MAIL_FAILED','Failed');
define('_MAIL_RETRY','Send again');
define('_MAIL_NODRAIN','The scheduler job "maildrain" is off — mail will pile up and never leave');
define('_MAIL_STALE','The queue has not been drained for a while, last run:');
define('_MAP_AUTO','Automatic generation and updating of the map file?');
define('_MAP_AUTO_T','The time interval between updates of the map file in hours');
define('_MAP_DAT_C','To specify the last modified date categories?');
define('_MAP_DAT_H','To specify the last modified date of home?');
define('_MAP_DAT_M','To specify the last modified date modules?');
define('_MAP_DAT_P','To specify the last modified date of publications?');
define('_MAP_FR_C','The update frequency of categories of modules');
define('_MAP_FR_H','Frequency of updates the main page');
define('_MAP_FR_M','The update frequency of the modules of the site');
define('_MAP_FR_P','The update frequency of publications in modules');
define('_MAP_GEN_C','Generate links to the category?');
define('_MAP_GEN_H','Generate a link to the home?');
define('_MAP_GEN_M','Generate the links for the modules?');
define('_MAP_GEN_P','Generate links to publications?');
define('_MAP_PR_C','The priority of categories of modules');
define('_MAP_PR_H','The priority of the main page');
define('_MAP_PR_M','The priority of the modules of the site');
define('_MAP_PR_P','The priority of publications in modules');
define('_MAP_SITE','Create a regular structure card to view on the website?');
define('_MAP_XSL','Use the design template for the XSL map file?');
define('_MASSMAIL','Massive newsletter to all users');
define('_MATCHANY','Match anywhere in the text');
define('_MESSAGE_BOX','Display messages on the main page?');
define('_MODULES','Modules');
define('_MODULESADMIN','Modules Administration');
define('_MODADMINPANEL','Administrative and system control panel modules');
define('_MODSITEPANEL','User and public site modules');
define('_MONTHLY','Monthly');
define('_MULTI','To enable the selection of multiple options?');
define('_MULTILINGUAL','Multilingual');
define('_MVIEW','Detailed view');
define('_NAME_BLOCK','The forbidden names');
define('_NAME_ID','Identical names passed');
define('_NAME_REF','Names passed');
define('_NEVER','Never');
define('_NEWSLETTER','Newsletter');
define('_NEW_USER','New users');
define('_NLABORT','Hard failure rate that aborts a mailing, percent');
define('_NLABORTI','The mailing stops once the share of recipient failures reaches this value');
define('_NLACTIVE','Users seen within the last days');
define('_NLBOUNCE','Failures before an address leaves bulk audiences');
define('_NLBOUNCEI','Transactional mail to that address is still attempted');
define('_NLBREAK','Failure evaluation window, messages');
define('_NLBREAKI','The rate is counted over the last results of the mailing and evaluated only once that many exist');
define('_NLBUSY','The mailing cannot be deleted: its messages are still in the queue');
define('_NLCANARY','Canary batch size');
define('_NLCANARYI','0 switches the canary off');
define('_NLCANMIN','Smallest audience that gets a canary');
define('_NLCANMINI','A smaller audience is sent whole, without sampling');
define('_NLCLONE','Copy into a new mailing');
define('_NLDAYS','Activity window, days');
define('_NLDAYSI','Used only by the audience based on the last visit');
define('_NLEND','Sent');
define('_NLFAILS','Failed');
define('_NLFREE','Send the rest of the audience');
define('_NLLOCKED','A started mailing is not edited. Saving creates a new mailing with this text');
define('_NLNOAUDIT','No audience is selected, or it no longer exists');
define('_NLNOJOB','The scheduler job "newsletter" is off — mailings will not be prepared');
define('_NLNOTE','Why it stopped');
define('_NLQUEUE','Queue');
define('_NLPROGRESS','Prepared');
define('_NLRATE','failed');
define('_NLSTCANAR','Canary');
define('_NLSTDONE','Done');
define('_NLSTDRAFT','Draft');
define('_NLSTHELD','Held');
define('_NLSTLIST','Address list of the previous version');
define('_NLSTOP','Abort the mailing');
define('_NLSTOPPED','the mailing was aborted by an operator');
define('_NLSTPREP','Preparing');
define('_NLSTQUEUE','Queued');
define('_NLSTRUN','Running');
define('_NLSTSTOP','Aborted');
define('_NLUSER','users');
define('_NLWHERE','Dispatch will receive');
define('_NOALINKP','Prohibition of the automatic conversion of links');
define('_NOAUM','This action does not apply to the chief administrator and administrators of the respective modules.');
define('_NOKOMA','As a delimiter use a comma, without whitespace.');
define('_NOLINKP','Prohibition of publication of the references');
define('_NONUMVALUE',"The value of the Points isn't numeric. Go back and fix it.");
define('_NO_MAIL_REG','Registration without confirmation on E-Mail?');
define('_NO_SICHT','Invisible in the block of modules');
define('_NULLPOINTS','Reset options');
define('_OAUTHACT','Activate OAuth authorization via external services?');
define('_OAUTHGACT','Activate sign-in via Google?');
define('_OAUTHGID','Google Client ID');
define('_OAUTHGKEY','Google Client Secret');
define('_OAUTHLIST','OAuth links');
define('_OAUTHMACT','Activate sign-in via Microsoft?');
define('_OAUTHMID','Microsoft Client ID');
define('_OAUTHMKEY','Microsoft Client Secret');
define('_OPCL','Coagulation / Deployment');
define('_OPTIMIZE','Optimization');
define('_OPTIMIZED','Optimized');
define('_OPTTEXT','Rebuilding tables updates statistics and internal structures, which may improve performance in certain cases. With modern InnoDB storage engines this action is only required after major data changes (e.g., mass deletions) and not on a regular basis.');
define('_PARTNERS','Partners');
define('_PASSWDLEN','Minimum users password length');
define('_PERMISSIONS','Permissions');
define('_PHPSETUP','Version PHP established on your server mismatches minimal requirements of system, it should be not below PHP %1$s!');
define('_POINTSNEEDED','Points Needed');
define('_POINTS_DIFF','Change of the balance, with a sign');
define('_POINTS_NOTE','Reason of the change');
define('_POINTS_BADDIFF','The change of the balance must be a whole number from -1000000 to 1000000 other than zero, and it needs a reason.');
define('_POINTS_BADSAVE','The balance was not changed: the points subsystem refused the correction.');
define('_POINTS_RESETMORE','The reset is not finished. Submit the form again to continue it.');
define('_POINTS_JOURNAL','Points journal');
define('_POINTS_NOMARK','The 6.3 data update has not been run: points are closed for writing.');
define('_POINTS_SOURCE','Source');
define('_POINTS_SCOPE','Context');
define('_POSITION','Position');
define('_PRACT','Activate personal messages?');
define('_PREFERENCES','Preferences');
define('_PRINM','The maximum number of incoming messages');
define('_PRMAIL','Notify the user about new messages?');
define('_PROFACT','Activate viewing profile only for registered users?');
define('_PRSAVEM','The maximum number of saved messages');
define('_PRSELF','To disable sending messages of himself?');
define('_PUBHOME','To show on the main page?');
define('_PUTINAHOME','The module loaded on the main page of the manager');
define('_PUTINHOME','The module loaded on the main page');
define('_PUTINHOMEINFO','In case of a choice of several variants, modules will be displayed in a casual mode.');
define('_RATINGS_GUESTS','Allow guests to rate?');
define('_RATINGS_NOMARK','The 6.3 data update has not been run: ratings are closed for writing.');
define('_RATINGS_BADDAYS','The interval has to be a whole number of days, zero or more.');
define('_RATINGS_VOTES','Rating journal');
define('_RATINGS_ANNUL','Annul');
define('_RATINGS_REASON','Reason of the annulment');
define('_RATINGS_DONE','The vote is annulled.');
define('_RATINGS_ACTOR','Voter');
define('_RATINGS_TARGET','Target');
define('_REFER','To activate statistics of transitions from other sites?');
define('_REFERB','Fix the transitions only from the search sites?');
define('_REFERERS','Referers');
define('_REFER_T','Time of preservation of transitions from other sites, in days');
define('_REFINFO','Matters only at use RSS of the channel.');
define('_REFRESHTIME','Refresh Time');
define('_REF_ID','Identical transitions');
define('_REF_URL','Addresses of transitions');
define('_REGACT','To activate registration of new users?');
define('_REPAIR','Repair');
define('_REPLACE','Replacing the words');
define('_REPLACEIN','The word will be replaced with the entered text content.');
define('_REPLACEINFO','In the content of the replacement of the permitted use, as plain text, and HTML. To disable replacement specify a value of «0» in the word and the content of the field.');
define('_REPLACE_FIELD','Replacing');
define('_REPTEXT','With modern InnoDB tables, most issues are automatically recovered after an unexpected shutdown. The repair function should only be used when real table errors are detected.');
define('_REWRITE_MOD','Activate transformation of links - Mod rewrite?');
define('_FORCESSL','Always use HTTPS (redirect from http)?');
define('_FORCEHOST','Single site domain (redirect www to main)?');
define('_RIGHT','Right');
define('_RIGHTBLOCK','Right Block');
define('_ROWS','Lines');
define('_RSSACT','To activate a conclusion of channels to pages of a site');
define('_RSSC','RSS channel');
define('_RSSDESC','Standard feeds, this is Your channels, which will be added in the code caps site to view other sites, browsers or other relevant programmes. Third-party channels, the channels of other sites, which will be added to Your list of the channels, to directly view the visitors of Your site. To disable a channel, specify a value of «0» in the fields name and address.');
define('_RSSFAIL','The problem with the RSS file, check the link!');
define('_RSSFILE','URL of RSS file');
define('_RSSHOME','Standard channel');
define('_RSSINFO','At use RSS of the channel, the entered text maintenance will be ignored.');
define('_RSSLINESINFO','Select Custom and write the URL or just select a Site from the list to grab RSS news headlines.');
define('_RSSMAX','Maximum quantity by default');
define('_RSSMIN','Minimum quantity by default');
define('_RSSSITE','Foreign channel');
define('_RSSUSE','Activate Headlines Reader?');
define('_RULACT','To demand acknowledgement of rules at registration of users?');
define('_SCRIPTASIN','Asynchronous loading of scripts?');
define('_SCRIPTBOT','The side scripts downward transmitted?');
define('_SCRIPTFILE','Script files');
define('_SCRIPTFILEINFO','Directly include script files.');
define('_SCRIPTHEAD','To display the contents of the scripts in the header?');
define('_SEARCHLETINFO','Actively search for the module, as well as in the function prompts the user when searching in the module personal messages and admin panel.');
define('_USERSEARCHTIP','Type at least %d characters to see suggestions.');
define('_SEARCHLIMIT','Maximum text length for search (characters)');
define('_SEARCHLIMITINFO','Limits the number of characters passed to the search query to protect the database from overload.');
define('_SEARCHCLEAR','Delete search statistics');
define('_SEARCHCLEARINFO','This action deletes all saved search queries from the search statistics.');
define('_SEARCHEMPTY','Delete empty queries');
define('_SEARCHBYMOD','Delete by module');
define('_SEARCHBYDAY','Delete older than');
define('_SEARCHLAST','Last query');
define('_SEARCHTOP','Top queries');
define('_SEARCHTOPMOD','Top module');
define('_SEARCHTOTAL','Total queries');
define('_SEARCHUNIQUE','Unique queries');
define('_SEARCHNUM','Quantity of results of search on page');
define('_SEC_ADMIN_IP','Allowed administrator IP ranges (CIDR)');
define('_SEC_AUTH_INFO','Enabling additional HTTP authentication (login/password) significantly increases system security and reduces the risk of unauthorized access to the admin panel. Authentication is performed at the web server level and is applied before the CMS is loaded, which helps effectively block automated attacks and password guessing attempts. This feature is supported by most modern web servers (Apache, Nginx, IIS) and is usually configured through server settings or the .htaccess file. If needed, check the required settings with your hosting provider.');
define('_SEC_AUTH_OK',"Additional login and password have been configured successfully. To disable or change them, open the file: config/security.php and set the variables to: 'login' => '' and 'password' => ''");
define('_SEC_COOKIE','Cookies for the blocked visitors');
define('_SEC_DB','To activate automatic copying a database?');
define('_SEC_HACK_STAT','To conduct statistics of attacks?');
define('_SEC_LOG','To conduct statistics of all acting information?');
define('_SEC_LOGIN','Additional login of manager');
define('_SEC_LOG_A','To conduct statistics of inputs of managers?');
define('_SEC_LOG_D','To conduct statistics of updatings of files?');
define('_SEC_DUMP_SKIP','Exclude list for file structure scan');
define('_SEC_DUMP_SKIP_INFO','One relative path per line, directories may optionally end with /, for example .git/ or vendor/cache/');
define('_SEC_LOG_DB','Time interval of copying of a database, in minutes');
define('_SEC_LOG_DS','Time of check of files for updatings, in minutes');
define('_SEC_LOG_SIZE','The maximal size of files of statistics, in bytes');
define('_SEC_LOG_U','To conduct statistics of inputs of users?');
define('_SEC_MAIL_D_SEND','To inform on updating files on E-Mail?');
define('_SEC_MAIL_SEND','To inform on attacks on E-Mail?');
define('_SEC_MAIL_W_SEND','To inform on the forbidden actions on E-Mail?');
define('_SEC_PASS','Login and password');
define('_SEC_PASSWORD','Additional password of manager');
define('_SEC_REF_POST','Disable take the POST variables from other sites, sources?');
define('_SEC_STAT','Enable error log?');
define('_SEC_STAT_A','Admin login log');
define('_SEC_STAT_DB','Database changes log');
define('_SEC_STAT_DUM','Structure of files');
define('_SEC_STAT_DUML','Modified files log');
define('_SEC_STAT_ERROR_D','Runtime error log');
define('_SEC_STAT_ERROR_FILE','File error log');
define('_SEC_STAT_ERROR_S','Static error log');
define('_SEC_STAT_ERROR_SQL','SQL error log');
define('_SEC_STAT_HACK','Attack log');
define('_SEC_STAT_LOG','Incoming requests log');
define('_SEC_STAT_U','User login log');
define('_SEC_STAT_WARN','Forbidden actions log');
define('_SEC_URL_GET','To forbid transfer of references through GET?');
define('_SEC_URL_POST','To forbid transfer of references through POST?');
define('_SEC_VIEW','PHP error messages');
define('_SEC_VIEW_1','Report all errors except notifications');
define('_SEC_VIEW_2','Report all errors');
define('_SEC_WARN_BLOCK','To block attacking?');
define('_SEC_WARN_STAT','To conduct statistics of the forbidden actions?');
define('_SELLANGUAGE','Select the Language for your Site');
define('_SELTHEME','To resolve change of a subject of registration?');
define('_SEO','SEO');
define('_SESSION','To activate sessions of visitors?');
define('_SESS_T','Operating time of sessions, in minutes');
define('_SFLOD_T','Interval between inquiries attacking, in seconds');
define('_SFLOOD','Protection against attacks of overflow, type «Flood Atack», «DDoS Atack»');
define('_SFLOOD_1','To check transfer on means of variable POST');
define('_SFLOOD_2','To check transfer on means of variable POST and GET');
define('_SFLOOD_3','To check all entering and required information');
define('_SHOWINMENU','Visible in Modules Block?');
define('_ADMINLOGO','Admin panel logo installation');
define('_ADMINLOGOP','Admin panel logo');
define('_SITELOGO','Site Logo');
define('_SITE_CLOSE','Close the site on prevention?');
define('_SMAIL','Reports and receive alerts?');
define('_SORT','Sort method');
define('_SORTORDER','Order');
define('_SORTE','To sort');
define('_STATISTIC','Statistics');
define('_STATACT','Statistics activate?');
define('_STATBET','The distance between the bars');
define('_STATFROM','Show Statistics');
define('_STATGR','The schedule of visits from day');
define('_STATSHI','The width of the bar');
define('_STYLES','Styles');
define('_SUBCATEGORY','Subcategory');
define('_ACCESS','Access');
define('_SUPERUSER','Full access');
define('_SWORD','The text of the query');
define('_SYNTAX','Syntax highlighting of code');
define('_SYNTAXP','A standard method for PHP');
define('_SYNTAXPN','The standard method of PHP with the numbering');
define('_SYNTAXSH','Library Syntax Highlighter');
define('_SYSTEM','System');
define('_TABLE','Table');
define('_TEMPLATE','Template');
define('_TEMPLATES','Templates');
define('_TIME','Time, is underlined in day');
define('_TIMENL','Has borrowed time');
define('_TIME_DB','To display speed of generation of page?');
define('_TIME_ID','Simultaneous transitions');
define('_TIME_REF','Time of transitions');
define('_TOTALFREE','General overhead charge');
define('_TOTALSPACE','General size of a database');
define('_TPFOR','Template format');
define('_TPFORM','Attachment formats that have an output template');
define('_TPFORMIN','Which extensions the Templates tab shows an output template editor for. Comma separated, without a dot and without spaces. Supported:');
define('_TPHEIGHT','Default attachment height, substituted into [height]');
define('_TPINFO','<b>[src]</b> - Link to file<br><b>[tsrc]</b> - The reference to the file preview<br><b>[width]</b> - The width of application<br><b>[twidth]</b> - Width preview<br><b>[height]</b> - The height of the annex<br><b>[theight]</b> - Height preview<br><b>[align]</b> - The location in relation to the content<br><b>[title]</b> - The title application<br><b>[quot]</b> - Quotation marks<br><b>[rel]</b> - Optional attribute');
define('_TPWIDTH','Default attachment width, substituted into [width]');
define('_UGROUP','Users Group');
define('_GROUPS','Users Groups');
define('_UNIQUE','Unique');
define('_UPDATE_POINTS','To activate calculation of points of users?');
define('_UPLOADS','Uploads');
define('_URLS','Links');
define('_USEREXIST','the User already exists!');
define('_USERIPCHECK','To define the user on the last IP to the Address?');
define('_USERSCOUNT','Participants');
define('_USERSHOMENUM','To allow to change to users number of publications?');
define('_USER_COOKIE','Name Cookies for users');
define('_USER_COOKIE_T','Time of preservation Cookies of the user, in days');
define('_USES','Use');
define('_VANSW','The number of variants of the answer');
define('_VARIABLES','Enabling debug extensions');
define('_VAR_VIEW','Visibility debugging functions');
define('_VBLOCK','Display in a block of polls');
define('_VCLOSED','Turn off vote');
define('_VDEACT','Deactivate the survey');
define('_VIEWCAT','Display category for a detailed view?');
define('_VIEWPRIV','Who can View This?');
define('_VIEWSITE','View on site');
define('_VLASTACT','Last active');
define('_VLASTCLO','The last is completed');
define('_VOTING_TIME','Interval between votings of the user, in days');
define('_VPRIVAT','Private Message button show?');
define('_VPROFIL','User profile button show?');
define('_VRANACT','Random active');
define('_VRANCLO','Random is completed');
define('_VWEB','Button on the website of the user show?');
define('_WEEKLY','Weekly');
define('_WEIGHT','Weight');
define('_WORD','Word');
define('_XSL_INFO','File: %1$s represents a mechanism for style sheets, specially adapted to work with the XML file of the site map for search engines and provides its visual transformation.');
define('_YEARLY','Yearly');
define('_ADMIN_FILE','Name of the file to enter the administration department');
define('_ADMIN_FILE_INFO','With a view of safety change the standard name at a file admin.php. Rename it that you have thought up, for example: %1$s.php also specify its new name, but without the termination «.php» in our case: %1$s');
define('_SMODULE','Use search in modules');
define('_ASEARCH','Activate the search engine analyser?');
define('_LTITLE','Use long titles?');
define('_ADESC','Automatic description generation?');
define('_DLETTER','Maximum number of characters in description');
define('_RSEP','URL separator');
define('_TSEP','Title separator for SEO URLs');
define('_SEOTITLE','Add title?');
define('_SEOCTITLE','Add category title?');
define('_OGRAPH','Enable Open Graph?');
define('_OGRAPHT','Open Graph Template');
define('_SCHEMA','Enable Schema.org?');
define('_SCHEMAT','Schema.org Template');
define('_SEO_TPL_INVALID','Invalid SEO template');
define('_TPLVARS','<b>[site]</b> - Site name<br><b>[loc]</b> - Page URL<br><b>[title]</b> - SEO title (article + category + module + site)<br><b>[desc]</b> - Description<br><b>[img]</b> - Image<br><b>[type]</b> - Content type<br><b>[url]</b> - URL<br><b>[homeurl]</b> - Homepage<br><b>[logo]</b> - Logo<br><b>[time]</b> - Publish date<br><b>[mtime]</b> - Modified date<br><b>[ctitle]</b> - Category<br><b>[headline]</b> - Article headline (text only, no suffixes)<br><b>[author]</b> - Article author');
define('_ADMINPASSKEEP','Leave empty to keep the current password');
define('_ADMINSELFDEL','You cannot delete the current administrator account');
define('_ADMINSELFSUPER','You cannot remove full access from your current administrator account');
define('_ADMINLASTSUPER','You cannot delete or demote the last administrator with full access');
define('_SEARCHAUTO','Auto add compatible modules');
define('_SEARCHAUTOINFO','Adds all active modules with a compatible table structure to search');
define('_SEARCHAUTODONE','Activated modules count');
define('_SEARCHENABLED','Already enabled');
define('_SEARCHREADY','Available to enable');
define('_SEARCHINVALID','Incompatible');
define('_SEARCHTYPE','Type');
define('_SEARCHFIELDS','Search fields');
define('_SEARCHEDIT','Edit route');
define('_SEARCHREASON','Reason');
define('_SEARCHSPECIAL','Special');
define('_SEARCHSIMPLE','Auto-compatible');
define('_SEARCHNOTABLE','Table not found');
define('_SEARCHMISSCOLS','Required fields');
define('_SEARCHNOFIELDS','Needs one of: intro, body, url');
define('_SEARCHADDSEL','Add selected');
define('_SEARCHADDALL','Add all compatible');
define('_SCHEDULER','Scheduler');
define('_SCHEDULER_LASTRUN','Last run');
define('_SCHEDULER_LAST_OK','Last success');
define('_SCHEDULER_NEXTRUN','Next run');
define('_SCHEDULER_TRIGGER','Trigger');
define('_SCHEDULER_RUNTIME','Duration');
define('_SCHEDULER_FAILS','Fails');
define('_SCHEDULER_SCHED','Schedule');
define('_SCHEDULER_SYSTEM','System');
define('_SCHEDULER_CUSTOM','Custom');
define('_SCHEDULER_PRIO','Priority');
define('_SCHEDULER_PRIOTIP','The lower the number, the earlier the job runs.');
define('_SCHEDULER_LOCK','Max execution time, sec.');
define('_SCHEDULER_UNLOCK','Unlock');
define('_SCHEDULER_RUN','Run now');
define('_SCHEDULER_JOBKEY','Job key');
define('_SCHEDULER_URL','Target URL');
define('_SCHEDULER_UNLOCKD','Scheduler lock cleared');
define('_SCHEDULER_RUNNING','Running');
define('_SCHEDULER_RESULT','Execution');
define('_SCHEDULER_IDLE','Idle');
define('_SCHEDULER_CRASH','Crashed');
define('_SCHEDULER_OFF','Disabled');
define('_SCHEDULER_URLINFO','Relative URLs are resolved against the site home URL');
define('_SCHEDULER_SYSINFO','System jobs can be tuned here and later migrated away from legacy triggers');
define('_SCHEDULER_MANUAL','Allow manual start');
define('_SCHEDULER_CRONFMT','Cron format: minute hour day month weekday. Examples: 0 2 * * * | 0 3 * * 1 | 15 1 1 * *');
define('_SCHEDULER_WARN_DB','Automatic database backup is disabled, so the scheduler job will not run.');
define('_SCHEDULER_WARNLOG','File scan logging is disabled, so the scheduler job will not run.');
define('_SCHEDULER_WARN_GO','Go to Security settings');
define('_SCHEDULER_LIMIT','Items per run');
define('_MONITOR_TIME_AGO','%d sec. ago');
define('_MONITOR_TIME_NOW','Now');

# Captcha (ALTCHA)
define('_CAPTCHA','Captcha');
define('_CAPTCHA_ACTIVE','Enable captcha');
define('_CAPTCHA_PROVIDER','Captcha provider');
define('_CAPTCHA_REGISTER','Protect registration');
define('_CAPTCHA_CONTACT','Protect contact form');
define('_CAPTCHA_COMMENTS','Protect comments and content forms');
define('_CAPTCHA_LOGIN_USER','Captcha at user login');
define('_CAPTCHA_LOGIN_ADMIN','Captcha at administrator login');
define('_CAPTCHA_LOGIN_HINT','When the captcha appears on the login form: never, only after several failed attempts, or on every login.');
define('_CAPTCHA_NEVER','Never');
define('_CAPTCHA_AFTERFAIL','After failed attempts');
define('_CAPTCHA_ALWAYS','Always');
define('_CAPTCHA_DIFFICULTY','Difficulty');
define('_CAPTCHA_DIFFICULTY_HINT','Higher difficulty increases the proof-of-work cost for the visitor.');
define('_CAPTCHA_LOW','Low');
define('_CAPTCHA_NORMAL','Normal');
define('_CAPTCHA_HIGH','High');
define('_CAPTCHA_TTL','Challenge lifetime (seconds)');
define('_CAPTCHA_TTL_HINT','How long an issued challenge stays valid.');
define('_SECRET_REGEN','Regenerate site secret key');
define('_SECRET_REGEN_HINT','Generates a new site-wide secret on save and invalidates all CSRF tokens, captcha challenges and obfuscated field names.');
define('_CAPTCHA_STORE_WARN','The captcha storage directory storage/captcha is not writable. ALTCHA cannot generate a secret or verify solutions.');
define('_UPLOADS_SYSTEM','System files');
define('_UPLOADS_ROOT','Site');
define('_UPLOADS_TREE','Site tree');
define('_UPLOADS_PROPS','Properties');
define('_UPLOADS_UP','One level up');
define('_UPLOADS_FILTER','Filter in this directory');
define('_UPLOADS_TILES','Tiles');
define('_UPLOADS_PATH','Path');
define('_UPLOADS_FULL','Full path');
define('_UPLOADS_DIMS','Resolution');
define('_UPLOADS_OPEN','Open');
define('_UPLOADS_GRANTS','Available');
define('_UPLOADS_BROWSE','browsing');
define('_UPLOADS_PREVIEW','preview');
define('_UPLOADS_UPLOAD','upload');
define('_UPLOADS_DOWNLOAD','download');
define('_UPLOADS_EDIT','text editing');
define('_UPLOADS_CREATE','file creation');
define('_UPLOADS_MKDIR','directory creation');
define('_UPLOADS_RENAME','renaming');
define('_UPLOADS_COPY','copying');
define('_UPLOADS_MOVE','moving');
define('_UPLOADS_DELETE','deletion');
define('_UPLOADS_COMPRESS','archiving');
define('_UPLOADS_EMPTY','The directory is empty');
define('_UPLOADS_EMPTYTXT','There are no files and no subdirectories here yet.');
define('_UPLOADS_NOFIND','Nothing found');
define('_UPLOADS_NOFINDTXT','No object of this directory matches the filter.');
define('_UPLOADS_CRIT','The file takes part in the work of the site: an error in it stops the site.');
define('_UPLOADS_VERSION','Version');
define('_UPLOADS_LINES','Lines');
define('_UPLOADS_VERHINT','The version is issued when the file opens; on save the server compares it and refuses the write if someone else has changed the file');
define('_UPLOADS_CRITASK','The file takes part in the work of the site: an error in it stops the site. Save the changes? The operation goes into the journal.');
define('_UPLOADS_STALE','The file changed after it was opened, so nothing was written. Saving again overwrites the change of the other writer.');
define('_UPLOADS_NOEDIT','This file does not open in the source editor.');
define('_UPLOADS_LEAVE','The change was not saved. Leave the editor and lose it?');
define('_UPLOADS_TORENAME','Rename');
define('_UPLOADS_TOCOPY','Copy');
define('_UPLOADS_TOMOVE','Move');
define('_UPLOADS_TOZIP','Compress to ZIP');
define('_UPLOADS_NEWFILE','New file');
define('_UPLOADS_NEWDIR','New directory');
define('_UPLOADS_TAKEN','An object of this name already exists in this directory.');
define('_UPLOADS_FILLED','Only an empty directory is deleted: this one still holds objects.');
define('_UPLOADS_BADDEST','The destination lies inside the object itself.');
define('_UPLOADS_CRITDEL','The file takes part in the work of the site: after the deletion the site may stop. The operation goes into the journal.');
define('_UPLOADS_DIRS','Upload directories');
define('_UPLOADS_ADDR','Address');
define('_UPLOADS_PERMS','Permissions');
define('_UPLOADS_USER','Owner on the server');
define('_UPLOADS_OWNER','Owner');
define('_UPLOADS_OWNSITE','Administration');
define('_UPLOADS_LINKWARN','Renaming and moving do not update the links in already published materials: the old address stops opening.');
define('_UPLOADS_MARK','Mark the object');
define('_UPLOADS_MARKALL','Mark every object');
define('_UPLOADS_MARKED','Marked');
define('_UPLOADS_UNMARK','Drop the marks');
define('_UPLOADS_MANYDEL','Delete the marked objects?');
define('_UPLOADS_LOAD','Upload a file');
define('_UPLOADS_DROP','Drop files here or click');
define('_UPLOADS_QUEUE','Upload');
define('_UPLOADS_STOP','Cancel the upload');
define('_UPLOADS_TOPACK','Pack into one archive');
define('_UPLOADS_FAIL','The list did not arrive');
define('_UPLOADS_FAILTXT','The server did not answer the request for the directory. What was shown is still shown.');

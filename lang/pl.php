<?php
# Author: Eduard Laas
# 2005 - 2026 SLAED
# License: MIT
# Website: slaed.net

define('_ACCESSDENIED','Dostęp zabroniony');
define('_ACCOUNT','Profil');
define('_ACT','Aktywowane');
define('_ACTIVATE','Aktywacja');
define('_ACTIVATEPERSONAL','Aktywacja własne menu?');
define('_ACTIVEBUTNOTSEE','Aktywne ale niewiem');
define('_ADD','Dodaj');
define('_ADDITIONALYGRP','Moduł ten jest dostępny tylko dla grupy użytkowników');
define('_ADDMAIL','Prosimy pamiętać o tym, że użytkownik %1$s wyprodukował publikację nowego materiału w module«%2$s».<br>, Aby wyświetlić przydał, zaloguj się do panelu administratora systemu.');
define('_ADDMAILC','Prosimy pamiętać o tym, że użytkownik %1$s wyprodukował publikację nowego komentarza w module«%2$s».<br><br>Dla przeczytaniu wiadomości, przejdź do adresu: %3$s');
define('_ADMIN','Administrator');
define('_ADMINMENU','Panel administratora');
define('_ADMINS','Administracja');
define('_AHEIGHT','Maksymalna dopuszczalna wysokość obrazu');
define('_AIN',', w pikselach');
define('_ALBUM','Galeria');
define('_ALL','Wszystkie');
define('_ALLIN','Łączna liczba');
define('_ALLINC','kategoriach');
define('_ALLOWUSERS','Zezwalaj innym użytkownikom widzieć E-Mail?');
define('_ALPHABET','AĄBCĆDEĘFGHIJKLŁMNŃOÓPRSŚTUWYZŹŻ');
define('_AND','i');
define('_ANONYM','Gość');
define('_AQUERY_DB','Zapytania do bazy danych');
define('_AUTH_ERROR','Aby korzystać z tej funkcji, administrator systemu, należy zainstalować dodatkowe hasło i login do panelu administracji, dział: «Bezpieczeństwo». W razie pytań lub problemów, zalecamy skontaktować się z pomocy technicznej projektu SLAED pod adresem: https://slaed.net');
define('_AUTH_ERROR_IP','Dostęp w to dział z używanym Adresem IP jest zabronione!');
define('_AVARIABLES','Zmienne żądania');
define('_AVATAR','Aktualny obraz');
define('_AVERAGESCORE','Średnia ocena');
define('_AWIDTH','Maksymalna dopuszczalna szerokość obrazu');
define('_A_LINKS','Wymiana linków');
define('_BACK','Temu');
define('_BANIPSENDER','Zablokować Adres IP');
define('_BANN_INFO','Ci dostępu do witryny!');
define('_BANN_REAS','Powód blokady');
define('_BANN_TERM','Do końca blokady pozostało');
define('_BEST','Najlepsze');
define('_BIRTHDAY','Data urodzenia');
define('_BLOCKPROBLEM','Istnieje problem z tym blokiem!');
define('_BLOCKPROBLEM2','Nie ma zawartości dla tego bloku!');
define('_BMEM','Użytkowników');
define('_BOTS','Boty wyszukiwarek');
define('_BREG','Zarejestruj się lub zaloguj');
define('_BROWSER','Przeglądarka');
define('_BVIS','Gości');
define('_CATEGORIES','Kategorie');
define('_CATEGORY','Kategoria');
define('_CATVORH','Pokaż lub ukryj kategorie');
define('_CCLOSED','Kategoria zamknięte');
define('_CERROR','Pozostawić puste pole z tytułu/nazwy!');
define('_CERROR1','Pozostawić puste pole z tekstem!');
define('_CERROR10','Nie podałeś nazwy witryny!');
define('_CERROR11','Nie podałeś opis strony!');
define('_CERROR2','Wpisałeś zbyt długie słowo!');
define('_CERROR3','Nie podałeś swoje imię i nazwisko!');
define('_CERROR4','Nie podałeś link do strony!');
define('_CERROR5','Minimalny czas między publikacje wiadomości wynosi %1$s sekund.');
define('_CERROR6','Nie określono nazwy odbiorcy! Należy wpisać pełną nazwę użytkownika lub wybierz go z listy.');
define('_CERROR7','Odbiorca o podanej nazwie nie znaleziono! Sprawdź wprowadzoną nazwę użytkownika i spróbuj ponownie.');
define('_CERROR8','Zabronione wysyłanie wiadomości do siebie!');
define('_CERROR9','Umieszczanie linków w komentarzach zakaz palenia!');
define('_CHANGE','Ustawienia');
define('_CHANGELOG','Dziennik zmian');
define('_CHANGEFREQ','Częstotliwość odświeżania');
define('_CHARSET','utf-8');
define('_CHECKALL','Zaznacz wszystko');
define('_CHECKOP','Akcja');
define('_CHNGSTORY','Data publikacji');
define('_CLIENT','Klient');
define('_CLIENTS','Klienci');
define('_CLOSE','Zamknij');
define('_CLOSE_TEXT','Strona jest chwilowo niedostępna, są prowadzone prace konserwacyjne.');
define('_CONFIRM','Potwierdzenie');
define('_COPIED','Link skopiowany');
define('_COPYLINK','Kopiuj link');
define('_CODE','Kod');
define('_COMMENT','Komentarz');
define('_COMMENTREPLY','Zamieść komentarz');
define('_COMMENTS','Komentarze');
define('_COMMENTS_ADDED','Twój komentarz został opublikowany. Znajduje się na innej stronie dyskusji.');
define('_COMMENTS_REPLIES','Pokaż odpowiedzi');
define('_COMMENTS_REPS','Odpowiedzi pod jednym komentarzem');
define('_COMMENTS_MORE','Pokaż więcej komentarzy');
define('_COMMENTS_GONE','Ten komentarz został usunięty. Odpowiedzi pod nim zostają zachowane.');
define('_COMMENTS_PARENT','Tej odpowiedzi nie można przypisać do wskazanego komentarza.');
define('_COMMENTS_REPLAY','Komentarz został już zapisany pod tym kluczem i różni się od tego żądania.');
define('_COMMENTS_EDITED','Edytowano');
define('_COMMENTS_REPLY','Odpowiedz');
define('_CONTENT','Spis treści');
define('_COUNTRY','Kraj');
define('_C_12','Ilość publikacji na stronę');
define('_DATE','Data');
define('_DATESTRING','d.m.Y');
define('_DAYS','dni');
define('_DEACT','Zdezaktywowany');
define('_DEACTIVATE','Dezaktywować');
define('_DEBUGPANEL','Panel diagnostyczny');
define('_DELETE','Usuń');
define('_DOWNLOAD','Pobierz');
define('_DESCRIPTION','Opis');
define('_DOWN','Download');
define('_DOWNLLINK','Odwiedzenia strony internetowej');
define('_D_DEL','Usunięty plik');
define('_D_EDIT','Zmieniony plik');
define('_D_NEW','Dodano nowy plik');
define('_EAUTOTR','Automatyczne tłumaczenie tekstu');
define('_EDIT','Edytuj');
define('_EDITOR','Redaktor');
define('_EMAIL','E-Mail');
define('_END','Ukończony');
define('_ENDDATE','Data zakończenia');
define('_ENDTEXT','Zaawansowany tekst');
define('_ENGLISH','Angielski');
define('_EQUOTE','Cytat');
define('_ERROR','Błąd');
define('_TOKENMISS','Niezgodność tokena bezpieczeństwa!');
define('_ERROR1','Błąd, źle podany E-Mail!');
define('_ERROR2','Proszę używać standardowego formatu');
define('_ERROR3','Proszę, nie należy podawać na początku adresu');
define('_ERROR4','Proszę nie używaj spacji');
define('_ERROR_PAGE','Wystąpił błąd %1$s. Żądana strona jest niedostępna lub nie istnieje. Być może skorzystałeś z nieaktualnego linku, wpisałeś błędny adres lub wystąpił błąd serwera. Wróć na stronę główną lub skorzystaj z wyszukiwarki.');
define('_ERRORINVNICK','W loginie zawiera nie dopuszczalne do używania znaków!');
define('_ERRORTR','Nie wykryto tekst do tłumaczenia!');
define('_ERROR_ALL','Wypełnij wszystkie wymagane pola!');
define('_ERROR_BIG','Ładowany plik jest zbyt duży!');
define('_ERROR_DOWN','Wystąpił błąd podczas ładowania pliku źródłowego!');
define('_ERROR_EMAIL','Taki E-Mail jest już zarejestrowany!');
define('_ERROR_EXIST','Plik o tej nazwie już istnieje na serwerze!');
define('_ERROR_FILE','Nieprawidłowy format pliku!');
define('_ERRLOG','Błędy i ostrzeżenia');
define('_ERROR_PASS','Wprowadzone hasła nie są identyczne!');
define('_ERROR_SERV','Pliki tego typu nie mogą być teraz przetworzone na serwerze!');
define('_ERROR_SIZE','Rozmiar załadowanej grafiki przekracza normy!');
define('_ERROR_UP','Wystąpił błąd podczas zapisu pliku!');
define('_EATTACH','Załącznik');
define('_EDROPFILES','Przeciągnij pliki tutaj lub kliknij pole');
define('_EEMOJI','Emoji');
define('_EEMOJIEMPTY','Brak emoji');
define('_EEMOJINOTICE','Notatki');
define('_EEMOJIREACT','Reakcje');
define('_EEMOJIRECENT','Ostatnie');
define('_EEMOJISMILE','Emotikony');
define('_EEMOJISYMBOL','Symbole');
define('_EEXITFULL','Wyłącz pełny ekran');
define('_EEXPAND','Rozwiń okno');
define('_EFULLSCREEN','Wyświetl edytor na pełnym ekranie');
define('_EIMGURL','Adres obrazu');
define('_EINSOBJ','Wstaw miniaturę lub plik');
define('_EMODEEMBED','Osadź w tekście');
define('_EMODEEMBEDINFO','Obraz zostanie zapisany jako Base64 bezpośrednio w tekście. Tylko dla małych obrazów.');
define('_EMODEUPLOAD','Prześlij na serwer');
define('_EMODEUPLOADINFO','Pliki zostaną przesłane na serwer. Zaznacz te do wstawienia i kliknij «Wstaw».');
define('_EMOVEWIN','Przenieś okno');
define('_ENOEMBED','To pole nie może zawierać osadzonego obrazu — wstaw go przez adres.');
define('_ENOFILE','Brak pliku');
define('_ERESTORE','Przywróć okno');
define('_ETABS','Karty');
define('_ETEXTLONG','Tekst jest dłuższy, niż to pole może pomieścić — najwyżej %s.');
define('_EUPLOAD','Menedżer plików');
define('_EUSEHTML','Korzystać z kodu HTML');
define('_EUSEPHP','Kod wykorzystanie PHP');
define('_EDITOR_STORE','Magazyn');
define('_EDITOR_LINK','Adres obrazu');
define('_EDITOR_LINKNOTE','Plik nie jest przesyłany, ale obraz pozostaje dostępny pod podanym adresem.');
define('_EDITOR_LINKINFO','Obraz jest podłączany przez adres zewnętrzny, na serwer nic nie trafia.');
define('_EDITOR_LINKWARN','Plik pozostaje na obcym serwerze. Nie zajmuje miejsca w limicie modułu, ale jego trwałość również nie jest gwarantowana.');
define('_EDITOR_MY','Moje pliki');
define('_EDITOR_MYNOTE','Plików w module: %d');
define('_EDITOR_MYINFO','Już wgrane pliki modułu. Działania są w menu pliku, podgląd otwiera się przez obrazek.');
define('_EDITOR_NOUP','Ta grupa nie ma prawa wgrywania plików');
define('_EDITOR_NOMY','Katalog otwiera się tylko razem z prawem wgrywania');
define('_EDITOR_EMPTY','Katalog jest jeszcze pusty');
define('_EDITOR_EMPTYWHY','Wgrane pliki pojawią się tutaj zaraz po pierwszym wgraniu.');
define('_EDITOR_NONE','Nic nie znaleziono');
define('_EDITOR_NONEWHY','W tym katalogu nie ma plików pasujących do filtra.');
define('_EDITOR_FAIL','Lista nie dotarła');
define('_EDITOR_FAILWHY','Serwer nie odpowiedział na żądanie katalogu. Pliki są na miejscu, listy nie udało się odczytać.');
define('_EDITOR_WAIT','Katalog się ładuje');
define('_EDITOR_FILTER','Filtr według nazwy w tym katalogu');
define('_EDITOR_IMAGES','Obrazy');
define('_EDITOR_ASLIST','Listą');
define('_EDITOR_ASGRID','Kafelkami');
define('_EDITOR_FULL','Otwórz cały katalog');
define('_EDITOR_OTHERS','Pliki');
define('_EDITOR_PROPS','Właściwości pliku');
define('_EDITOR_NOTE','Pokazany jest adres względny wewnątrz dozwolonego katalogu modułu. Bezwzględna ścieżka na serwerze nigdy nie trafia do tego okna.');
define('_EDITOR_DIM','Rozdzielczość');
define('_EDITOR_PERMS','Prawa dostępu');
define('_EDITOR_OWNER','Właściciel');
define('_EDITOR_ADDR','Adres');
define('_EDITOR_MARK','Zaznacz');
define('_EDITOR_MARKALL','Zaznacz wszystkie pliki');
define('_EDITOR_MARKED','Zaznaczono');
define('_EDITOR_INSAS','Wstaw jako');
define('_EDITOR_INSERT','Wstaw');
define('_EDITOR_OPTS','Opcje wstawiania');
define('_EDITOR_ALIGN','Oblewanie tekstem');
define('_EDITOR_ALIGNNO','Bez oblewania');
define('_EDITOR_ALIGNLEFT','Po lewej stronie tekstu');
define('_EDITOR_ALIGNRIGHT','Po prawej stronie tekstu');
define('_EDITOR_CAPTION','Podpis');
define('_EDITOR_INSIMGS','Wstaw jako obrazy');
define('_EDITOR_INSOBJS','Wstaw jako załączniki');
define('_EDITOR_UNMARK','Zdejmij zaznaczenia');
define('_EDITOR_ZIP','Spakuj do ZIP');
define('_EDITOR_ASKDEL','Usunąć plik %s?');
define('_EDITOR_ASKDELS','Usunąć zaznaczone pliki: %d?');
define('_EDITOR_QUEUE','Wgrywanie pliku %1$d z %2$d');
define('_EDITOR_QUEUEEND','Wgranych plików: %1$d z %2$d');
define('_EDITOR_STOP','Anuluj wgrywanie');
define('_EDITOR_USED','Zajęte w module');
define('_EDITOR_ROOM','Miejsce we wpisie');
define('_EDITOR_MODULE','Moduł: %s');
define('_EDITOR_MORE','Pokazano %1$d ostatnich plików z %2$d.');
define('_EDITOR_PREV','Poprzedni');
define('_EDITOR_NEXT','Następny');
define('_FAQ','Pytania i odpowiedzi');
define('_FAVOR','Juz jest w ulubionych');
define('_FAVORITES','Ulubione');
define('_FAVOR_ADD','Dodaj do ulubionych');
define('_FAVOR_EXIT','Maksymalna dopuszczalna ilość w wysokości %1$s ulubionych już osiągnięta!');
define('_FAVOR_MAX','Maksymalna dopuszczalna ilość ulubionych %1$s, już dodane %2$s, można jeszcze dodać %3$s.');
define('_FAVOR_DONE','Dodano do ulubionych');
define('_FAVOR_MODS','Moduły');
define('_FAVOR_TOP','Najwięcej');
define('_FAVOR_LAST','Ostatni');
define('_FAVOR_FOLD','Kliknij nagłówek półki, aby zwinąć lub rozwinąć jej pozycje.');
define('_FAVOR_FREE','Wolne %1$s z %2$s');
define('_FAVOR_SEEK','Szukaj w ulubionych');
define('_FAVOR_SEEKNOTE','Tytuł lub moduł');
define('_FAVOR_NONE','Nic nie znaleziono. Wyczyść zapytanie lub wybierz moduł powyżej.');
define('_FEEDBACK','Feedback');
define('_FIELDS_REQ','To pole musi zostać wypełnione.');
define('_FIELDS_TYPE','Wartość ma nieprawidłową strukturę.');
define('_FIELDS_FORMAT','Wartość ma nieprawidłowy format.');
define('_FIELDS_CHOICE','Wartość nie należy do dozwolonych opcji.');
define('_FIELDS_MIN','Wartość jest poniżej dozwolonego minimum.');
define('_FIELDS_MAX','Wartość jest powyżej dozwolonego maksimum.');
define('_FILE','Plik');
define('_FILES','Katalog plików');
define('_FILEUP','Liczbę jednocześnie pobieranych plików');
define('_FILE_RENAMED','Plik przesłany i nazwany');
define('_FILE_SITE','Pobierz plik z linku');
define('_FIN',', w bajtach');
define('_FMODC','Zamknij na sprawdzanie');
define('_FOLLOWINGMEM','Informacje dla dostępu');
define('_FORADMINTESTS','Tylko dla admina');
define('_FORUM','Forum');
define('_FRENCH','Francuski');
define('_FRESET','Wyczyść filtr');
define('_FROM','Z');
define('_FSIZE','Maksymalny rozmiar pliku do pobrania');
define('_FSIZEALL','Maksymalny rozmiar przesłanych plików modułu');
define('_FTYPE','Dopuszczalne formaty plików');
define('_FULLEDIT','Pełna edycja');
define('_FUNCTIONS','Opcje');
define('_GENDER','Płeć');
define('_GENERATION','Generowanie');
define('_GENERATION_DB','zapytań do bazy danych za');
define('_PAGETIME','Czas generowania strony');
define('_GERMAN','Niemiecki');
define('_GOBACK','<input type="button" value="Wstecz" class="sl_but_blue" OnClick="javascript:window.history.go(-1);">');
define('_GROUP','Grupa');
define('_HACK','Próba włamania');
define('_HASH','Hash przeglądarki');
define('_HEIGHT','Wysokość');
define('_HELLO','Witam');
define('_HELP','Pomoc');
define('_HIDE','Ukryty tekst');
define('_HIDETEXT','Ukryty tekst widoczny tylko dla zarejestrowanych użytkowników, proszę <a href="index.php?name= " account">zarejestruj lub zaloguj się</a>.');
define('_HITS','Wejścia');
define('_HOME','Strona główna');
define('_HOMECAT','Na stronie głównej modułu');
define('_HOMEPROBLEMUSER','Nie znaleziono modułu używane na stronie głównej. Być może został on usunięty lub zmieniony. Polecamy wejść w dział konfiguracji panelu administratora systemu i zainstalować moduł ładowany na stronie głównej.');
define('_HOUR','godzina');
define('_ID','Nr');
define('_IMG','Obraz');
define('_INFO','Informacje');
define('_DOCS','Dokumentacja');
define('_INPUT','Wejście');
define('_INSERTIMG','Wstaw obraz');
define('_INTERESTS','Interesy');
define('_INTERVAL','Interwał');
define('_INVISIBLEMODULES','Niewidoczne moduły');
define('_IP','Adres IP');
define('_JOKES','Dowcipy');
define('_LANGUAGE','Język');
define('_LASTMOD','Ostatnia zmiana');
define('_LASTPOSTER','Ostatnio');
define('_LAST_VISIT','Ostatnia wizyta');
define('_LASTACTIVITY','Ostatnia aktywność');
define('_LINKEXIST','Strona z tym adresem znajduje się już w naszym katalogu!');
define('_LINKS','Katalog stron');
define('_LIST','Lista');
define('_LIVE','Na żywo');
define('_LOCALE','pl_PL');
define('_LOCALITYLANG','Miejsce zamieszkania');
define('_LOGIN','Zaloguj się');
define('_LOGININCOR','Błędny login! Spróbuj jeszcze...');
define('_LOGOUT','Wyjście');
define('_MAN','Mężczyzna');
define('_MEDIA','Katalog mediów');
define('_MEMUSAGE','Użycie i limit pamięci');
define('_DBQUERY','Łączna liczba zapytań do bazy danych');
define('_DBQTIME','Czas zapytań do bazy danych');
define('_MENUCONF','Własne menu');
define('_MENUFOR','Własne menu');
define('_MENUINFO','Własne menu będą widoczne w bloku z prawej lub z lewej strony na wszystkich strona witryny. Należy użyć kodu BB do określania adresu lub linki.');
define('_MESSAGE','Wiadomość');
define('_MESSAGES','Wiadomości');
define('_MIN','min');
define('_MODE_AUTO','System');
define('_MODE_LIGHT','Jasny');
define('_MODE_DARK','Ciemny');
define('_MODUL','Moduł');
define('_MODULESADMINS','Przepraszamy, ale ta część naszej strony jest tylko dla Administratorów.');
define('_MODULEUSERS','Przepraszamy, ale ta część naszej strony jest tylko dla zarejestrowanych użytkowników. Możesz zarejestrować się bezpłatnie <a href="index.php?name= " account">tutaj</a>, po czym można uzyskać dostęp do tej sekcji.');
define('_MONEY','Wymiana WebMoney');
define('_MONITOR','Monitoring');
define('_MOVETO','Przenieś do kategorii');
define('_MVADMIN','Tylko administratorzy');
define('_MVALL','Wszyscy odwiedzający');
define('_MVANON','Tylko анонимы');
define('_MVUSERS','Tylko użytkownicy');
define('_MODULES_DUPLICATE','Znaleziono zduplikowane moduły');
define('_MODULES_NEW','Znaleziono nowe moduły');
define('_MODULES_DELETED','Usunięte moduły');
define('_MORE','Więcej');
define('_NAME','Nazwa');
define('_NEW','Nowe');
define('_NEWLAST3DAYS','Dodany przez ostatnie 3 dni');
define('_NEWMONTH','Dodano w tym miesiącu');
define('_NEWNOW','Właśnie opublikowano');
define('_NEWS','Aktualności');
define('_NEWTHISWEEK','Dodano ostatni tydzień');
define('_NEWTODAY','Dodane dzisiaj');
define('_NEWTOPICS','Tematy');
define('_NEXT','Następny');
define('_NICKLONG','Login jest zbyt długi. Musi on być nie więcej niż 25 znaków!');
define('_NICKNAME','Nick');
define('_NO','Nie');
define('_NOACTIVEMODULES','Nieaktywne moduły');
define('_NOANONCOMMENTS','Nie możesz przesłać komentarz anonimowo, proszę <a href="index.php?name= " account">zarejestruj lub zaloguj się</a>.');
define('_NOCOMMENTS','Brak komentarzy. Dlaczego nie zostawić?');
define('_NODE_CAST','Obsada');
define('_NODE_COVER','Okładka');
define('_NODE_CREATOR','Twórca');
define('_NODE_DIRECTOR','Reżyser');
define('_NODE_MFORMAT','Format');
define('_NODE_PLAY','Odtwarzanie');
define('_NODE_POSTER','Plakat');
define('_NODE_QUALITY','Jakość');
define('_NODE_RELEASE','Wydanie');
define('_NODE_RUNTIME','Czas trwania');
define('_NODE_SUBTITLE','Tytuł dodatkowy');
define('_NODE_YEAR','Rok');
define('_NONE','Nie');
define('_NOPASS','Nie podałeś i nie powtarzać hasło!');
define('_NOTE','Uwaga');
define('_NO_INFO','Brak informacji');
define('_NUMOF','%1$s z %2$s');
define('_OAUTHWITH','Zaloguj się przez');
define('_OCCUPATION','Zawód');
define('_OK','OK');
define('_OFFLINE','Offline');
define('_ONLINE','Online');
define('_ONLINE_NOW','Teraz online');
define('_ONDELETE','Usuwanie obiektu');
define('_ONEDIT','Szybka edycja');
define('_OPMOD','Działania moderatora');
define('_ORDER','Formularz zamówienia');
define('_ORTYPEURL','Link do RSS/XML');
define('_OUTS','Wyjścia');
define('_OVERALL','Tylko');
define('_PADD','Dodano');
define('_PAGE','Strona');
define('_PAGEHOME','Początek');
define('_PAGES','Artykuły');
define('_PAGETOP','Na górę');
define('_PASSFOR','Zapomniane hasło?');
define('_PASSWORD','Hasło');
define('_PAUSE','Pauza');
define('_PAUSED','Wstrzymano');
define('_PBASKET','Koszyk');
define('_PCLOSED','Wiadomość zamknięte, wymaga sprawdzania!');
define('_PEDEND','Nie można już zmienić tę wiadomość. Okres czasu do edycji ograniczona jest do %1$s minut.');
define('_PERCENT','Odsetek');
define('_PERPAGE','na każdej stronie');
define('_PERSONAL','Odpowiedzieć sobie osobiście');
define('_PERSONALINFO','Dane osobowe');
define('_PLOAD1','Ta opcja spowalnia system od jednej do dwóch sekund!');
define('_POINTS','Punkty');
define('_POINTS_PUBLISH','Publikacja materiału lub tematu');
define('_POINTS_COMMENT','Komentarz lub odpowiedź');
define('_POINTS_VIEW','Zaliczone wyświetlenie');
define('_POINTS_DOWNLOAD','Zaliczone pobranie');
define('_POINTS_VISIT','Zaliczone przejście do zewnętrznego linku');
define('_POINTS_POLL','Głos w ankiecie');
define('_POINTS_ORDER','Potwierdzone zamówienie');
define('_POINTS_FAVORITE','Dodanie do ulubionych');
define('_POINTS_MESSAGE','Wiadomość lub zgłoszenie');
define('_POINTS_RECOMMEND','Polecenie strony');
define('_POINTS_REGISTER','Rejestracja');
define('_POINTS_LOGIN','Zaliczone logowanie');
define('_POINTS_REPORT','Potwierdzone przydatne zgłoszenie');
define('_POINTS_MODERATE','Zakończone działanie moderatora');
define('_POINTS_ADJUST','Korekta administratora');
define('_POINTS_PERIOD','Okres, s');
define('_POINTS_LIMIT','Nagrody w okresie');
define('_POLISH','Polski');
define('_POLLS','Inne ankiety');
define('_POP','Popularne');
define('_POSTEDBY','Umieścił');
define('_POSTER','Autor');
define('_POSTNOTE','Twoja wiadomość zostanie opublikowany dopiero po sprawdzeniu i uprawnienia administratora.');
define('_PRBACK','Do listy');
define('_PRBOXFULL','Skrzynka pełna');
define('_PRBOXNEAR','Skrzynka prawie pełna');
define('_PRBOXROOM','W skrzynce jest miejsce');
define('_PRBYMATE','Wg rozmówcy');
define('_PRBYNEW','Najpierw nowe');
define('_PRBYOLD','Najpierw stare');
define('_PRBYUNS','Najpierw nieprzeczytane');
define('_PRESENTATION','Prezentacja');
define('_PREVIEW','Podgląd');
define('_PRFILLED','%1$s zapełnione w %2$s procentach');
define('_PRFILTER','Filtr');
define('_PRFOCUS','Fokus');
define('_PRFOCUSN','Nieprzeczytane: %1$s');
define('_PRFORWARD','Przekaż dalej');
define('_PRFOUND','Znaleziono %1$s z %2$s');
define('_PRFREE','Wolne %1$s');
define('_PRFRESH','Z ostatniego tygodnia');
define('_PRFWD','Przekazane');
define('_PRIN','Przychodzące');
define('_PRINEXIT','Maksymalna dopuszczalna ilość w wysokości %1$s przychodzących wiadomości już została osiągnięta! Nie można już odbierać nowe wiadomości od użytkowników! Usuń przestarzałe lub wyczyść dział wiadomości przychodzących.');
define('_PRINNO','Nowe przychodzące');
define('_PRIORITY','Priorytet');
define('_PRIVAT','Prywatne wiadomości');
define('_PRIVAT_DELIN','Usunięte przez odbiorcę');
define('_PRIVAT_DELOUT','Usunięte przez nadawcę');
define('_PRIVAT_NEW','Oznacz jako nieprzeczytane');
define('_PRIVAT_NOSEL','Wybierz wiadomości, których dotyczy akcja.');
define('_PRIVAT_READ','Oznacz jako przeczytane');
define('_PRIVAT_UNSAVE','Usuń z zapisanych');
define('_PRLIMSUB','Tytuł, do %1$s znaków');
define('_PRLIMTO','Nazwa, do %1$s znaków');
define('_PRLIST','Lista wiadomości');
define('_PRMOVE','Zapisaną wiadomość');
define('_PRNEW','Nowa wiadomość');
define('_PRNEWMAIL','Prosimy pamiętać o tym, że nadeszła nowa prywatna wiadomość od użytkownika %1$s.<br><br>Dla przeczytaniu wiadomości, przejdź do adresu: %2$s<br><br><br>Masz to powiadomienie, tak jak w koncie, w którym podany Twój adres e-mail, aktywowana usługa «otrzymuj powiadomienia o nowych prywatnych wiadomościach».<br><br>Aby nie otrzymać takie uwagi w przyszłości, należy wyłączyć tę usługę w ustawieniach profilu.');
define('_PRNOHIT','Żadna wiadomość nie pasuje do filtrów');
define('_PRNOLIMIT','Bez ograniczeń');
define('_PRODUCTS','Produkty');
define('_PROLD','Przeczytana wiadomość');
define('_PROUT','Wychodzące');
define('_PROUTNEW','Nieprzeczytana wiadomość');
define('_PROUTNO','Nowe wychodzące');
define('_PRPICK','Nie wybrano wiadomości');
define('_PRPICKT','Wybierz wiadomość po lewej stronie, aby ją przeczytać i odpowiedzieć. Otwarcie wiadomości przychodzącej oznacza ją jako przeczytaną.');
define('_PRRE','Odbiorca');
define('_PRREP','Odpowiedź');
define('_PRRESET','Zresetuj wszystko');
define('_PRSAVE','Zapisane');
define('_PRSAVEEXIT','Maksymalna dopuszczalna ilość w wysokości %1$s zapisanych wiadomości już została osiągnięta! Nie można już zapisywać wiadomości od użytkowników! Usuń przestarzałe lub wyczyść dział zapisanych wiadomości.');
define('_PRSE','Nadawca');
define('_PRSEEK','Szukaj w temacie, treści i nazwie');
define('_PRSEEN','Przeczytane');
define('_PRSENDED','Wiadomość użytkownikowi %1$s został pomyślnie wysłany.');
define('_PRSENDOVER','Wiadomość użytkownikowi %1$s nie może być wysłany, z powodu jego przepełnienia skrzynki odbiorczej.');
define('_PRSHOWN','Pokazano %1$s-%2$s');
define('_PRSORT','Sortowanie');
define('_PRSTALE','Starsze niż miesiąc');
define('_PRUNSEEN','Nieprzeczytane');
define('_PRVIEW','Podgląd wiadomości');
define('_PRWAIT','Odstęp %1$s sek.');
define('_PRWIPE','Wyczyść formularz');
define('_PRWRITE','Napisz');
define('_PURCHASED','Pozostało');
define('_QRCODE','Kod QR');
define('_QUOTE','Cytat');
define('_RADIO','Radio internetowe');
define('_RANK','Ranga');
define('_RATE1','Źle');
define('_RATE5','Dobrze');
define('_RATING','Ocena użytkownika');
define('_RATINGS','Oceny');
define('_RATINGS_FORM','Żądanie oceny jest nieprawidłowe.');
define('_RATINGS_DENY','Nie można ocenić: oceny są wyłączone, zamknięte dla gości albo to Twój własny materiał.');
define('_RATINGS_GONE','Nie znaleziono ocenianego obiektu.');
define('_RATINGS_TWICE','To żądanie zostało już przyjęte z inną wartością.');
define('_RATINGS_WAIT','Już oceniono. Dni do następnej oceny: %d');
define('_RATINGS_FAIL','Ocena nie została zapisana, spróbuj ponownie.');
define('_READMORE','Więcej');
define('_READS','Przeczytaj');
define('_RECHN','Konto');
define('_RECHN_B','Wystawić rachunek');
define('_RECOMMEND','Polecam');
define('_REFERER','Przejście');
define('_REG','Rejestracja');
define('_REPLIES','Od.');
define('_RESUME','Wznów');
define('_RETRY','Spróbuj ponownie');
define('_RETYPEPASSWORD','Powtórz hasło');
define('_RNEWSLETTER','Otrzymywanie wiadomości?');
define('_RSS','RSS Kanały');
define('_RSS_FROM','Informacje ze strony internetowej');
define('_RSS_PROBLEM','Istnieje problem z nagłówkami na tej stronie!');
define('_RULES','Zasady rejestracji użytkowników');
define('_RUSSIAN','Rosyjski');
define('_SAVE','Zapisz');
define('_SAVECHANGES','Zapisz zmiany');
define('_SUCCSAVE','Zmiany zostały pomyślnie zapisane.');
define('_SUCCDELETE','Obiekt został pomyślnie usunięty.');
define('_SUCCCLEAR','Dane zostały pomyślnie wyczyszczone.');
define('_SUCCUPLOAD','Plik został pomyślnie przesłany.');
define('_SUCCFILESAVE','Plik został pomyślnie zapisany.');
define('_SUCCSTATUS','Status został pomyślnie zmieniony.');
define('_SEARCH','Szukaj');
define('_SEARCHLETMIN','Minimalna dozwolona ilość znaków w wyszukiwanych haseł');
define('_SEC','sek');
define('_SECCODEINCOR','Nie udało się przejść algorytmu sprawdzającego dla prawdziwego użytkownika, prosimy o kontakt z administracją projektu!');
define('_SECURITY','Bezpieczeństwo');
define('_SHARE','Udostępnij');
define('_SELECTASITE','Przeczytaj informacje');
define('_SEND','Wyślij');
define('_SENDEREMAIL','E-Mail nadawcy');
define('_SENDERNAME','Nazwa nadawcy');
define('_SENDMES','Wyślij prywatną wiadomość');
define('_SEROR1','Nie podałeś swój wariant odpowiedzi!');
define('_SEROR2','Twój głos nie został zaakceptowany! Można już głosować w tej ankiecie.');
define('_SHOP','Sklep');
define('_SHOW','Pokaż');
define('_SIGNATURE','Podpis');
define('_SIGNATURE_TEXT','Jest to tekst, który będzie dodać do obstawionych wiadomości.');
define('_SITE','Strona');
define('_SITEMAP','Mapa strony');
define('_SITEMAP_XML','Ta mapa serwisu przeznaczona jest dla wyszukiwarek, jest ona pozwala im powiedzieć o tym, jak zorganizowany treści na stronie internetowej. Roboty ogląda ten plik, aby dokładniej indeksowanie strony.');
define('_SITENAME','Nazwa strony');
define('_SITEURL','Adres strony internetowej');
define('_SIZE','Rozmiar');
define('_SMILIE','Uśmiech');
define('_SPEC','Specjalna');
define('_SPEC_GROUP','Specjalna grupa');
define('_STARTDATE','Data otwarcia strony');
define('_STATUS','Status');
define('_SUM','Suma');
define('_SYSTEM_INFO','Wydajność systemu');
define('_S_STARTSEITE','Zrobić domowej');
define('_TEXT','Tekst');
define('_THEME','Temat');
define('_TIMESTRING','d.m.Y H:i:s');
define('_TITLE','Tytuł');
define('_TODAY','Dzisiaj');
define('_TOPUSERS','Top użytkownicy');
define('_TYPE','Typ');
define('_UACESS','Sprawdzanie moderatora wiadomości użytkownika?');
define('_UKRAINIAN','Ukraiński');
define('_UNLIMITED','Bez limitu');
define('_UPDATE','Odśwież');
define('_UPDATED','Zaktualizowano');
define('_URANK','Tytuł');
define('_URL','Link');
define('_USER','Użytkownik');
define('_USERDEL','Użytkownik usunięty');
define('_USERNOEXIST','Użytkownika z określonymi parametrami nie istnieje!');
define('_USERPASSWORD','Hasło dla użytkownika');
define('_USERS','Użytkownicy');
define('_USER_GROUPS','Członek zespołów');
define('_UWARN','Ostrzeżenie');
define('_UWARNS','Ostrzeżenia');
define('_VCLINFO','Dziękujemy za udział w ankiecie! Ta ankieta jest zamknięta, wyniki są dostępne wyłącznie administracji projektu.');
define('_VCLOSE','Kryty ankieta');
define('_VERSION','Wersja');
define('_TPLMISS','Błąd ładowania szablonu! Szablon wskazany w funkcji nie istnieje w katalogu aktywnego motywu: %1$s. Zalecana nazwa pliku: %2$s.html');
define('_VIEW','Kto widzi');
define('_VIEWS','Itp.');
define('_VOPEN','Odkryty ankieta');
define('_VOTE','Głos');
define('_VOTES','Głosów');
define('_VOTING_ACT','Trwa głosowanie');
define('_VOTING_END','Głosowanie zakończone');
define('_VOTING','Ankiety');
define('_WAITINGCONT','Czekają na sprawdzenie');
define('_WARN','Zabronioną czynność');
define('_WELCOMETO','Witamy');
define('_WHO','Obserwacja');
define('_WHOIS','Skargi');
define('_WIDTH','Szerokość');
define('_WOMAN','Kobieta');
define('_YES','Tak');
define('_YESTERDAY','Wczoraj');
define('_YOUREMAIL','Twój E-Mail');
define('_YOURNAME','Imię i nazwisko');
define('_ZIP','Kompresji Zip');
define('_SQLERRORCON','Problem z nawiązaniem połączenia z bazą danych!');
define('_ERR_TYPE','Nieobsługiwany typ źródła');
define('_ERR_ZIPNA','ZIP niedostępny');
define('_ERR_ZOPEN','Nie udało się otworzyć ZIP');
define('_ERR_ZADD','Nie udało się dodać do ZIP');
define('_ERR_READ','Błąd odczytu');
define('_ERR_WRITE','Błąd zapisu');
define('_ERR_DELETE','Usuwanie nieudane');
define('_ERR_NOCOMP','Brak dostępnej metody kompresji');
define('_ERR_INVMODE','Nieprawidłowy tryb kompresji');
define('_ERR_GZNA','GZIP niedostępny');
define('_ERR_BZ2NA','BZIP2 niedostępny');
define('_ERR_DIR','Katalog docelowy nie został znaleziony');
define('_ERR_PARAM','Brak źródła lub nazwy archiwum');
define('_ERR_FILE','Źródło musi być plikiem dla gzip/bzip2');
define('_ERR_OPEN','Nie można otworzyć pliku źródłowego');
define('_ERR_GZIP','Nie można utworzyć archiwum gzip');
define('_ERR_BZIP','Nie można utworzyć archiwum bzip2');

# Captcha widget (ALTCHA)
define('_CAPTCHA_LABEL','Nie jestem robotem');
define('_CAPTCHA_VERIFYING','Weryfikacja…');
define('_CAPTCHA_VERIFIED','Weryfikacja zakończona pomyślnie!');
define('_CAPTCHA_ERROR','Weryfikacja nie powiodła się. Spróbuj ponownie.');
define('_CAPTCHA_EXPIRED','Weryfikacja wygasła. Zweryfikuj ponownie.');
define('_CAPTCHA_LOADING','Ładowanie…');
define('_CAPTCHA_REQUIRED','Wymagana weryfikacja!');
define('_CAPTCHA_WAIT','Trwa weryfikacja… proszę czekać.');

...
const DEMO_ADMIN = [
  {
    ...
    title: 'Фокус',
    note: 'Панель перестаёт быть каталогом. Первый экран отвечает на три вопроса по порядку: что ждёт меня сейчас — пять очередей крупными цифрами, пустые девять названы одной строкой; куда я хожу чаще всего — восемь закреплённых модулей; где всё остальное — 38 текстовых ссылок в пяти группах по смыслу. Сайдбара нет, модуль подаёт сигнал только когда отличается: чип очереди или замок доступа.',
    tags: ['без сайдбара', 'очереди', 'закреплённые', 'группы по смыслу'],
  },
  {
    file: 'adm-02-rail.html',
    title: 'Рельс',
    note: 'Оболочка приложения в три панели: рельс из шести мест, список выбранного места и инспектор. На экране никогда не больше двенадцати модулей, у каждого — одно предложение о том, что он делает. Веер из трёх точек раскрыт в инспекторе именованными действиями, статус — настоящий переключатель темы.',
    tags: ['app-shell', 'мастер-деталь', 'инспектор', 'sl-radio-switch'],
  },
  {
    file: 'adm-03-launcher.html',
    title: 'Лаунчер',
    note: 'Панель как домашний экран: первым стоит поле поиска, под ним модули — иконка, имя и бейдж, когда что-то ждёт. Тринадцать строк сайдбара растворяются в трёх бейджах на иконках. Описание модуля говорит строка-подсказка внизу — для одного модуля под курсором, а не для всех сразу. «/» или Ctrl+K — в поиск, Enter — открыть первый найденный.',
    tags: ['поиск первым', 'бейджи', 'клавиатура', 'строка-подсказка'],
  },
  {
    file: 'adm-04-cockpit.html',
    title: 'Кокпит',
    note: 'Лицо презентационной страницы, принесённое домой: пять приборов, окно журнала, маршрут запроса. Все эти цифры у живой панели уже есть — они лежат под подвалом четырьмя зелёными полосами. Здесь они на первом экране, а модули стали коммутатором из маленьких клавиш в пяти отсеках, размер отсека — по содержимому.',
    tags: ['бенто', 'приборы', 'журнал', 'клавиши'],
  },
  {
    file: 'adm-05-bridge.html',
    title: 'Мостик',
    note: 'Слияние лидеров. Скелет и порядок вопросов — «Фокуса»: что ждёт, куда хожу чаще всего, где всё остальное. Приборы «Кокпита» стоят рядом с приветствием, так что состояние системы читается тем же взглядом. Инспектор «Рельса» занял правую колонку: название открывает модуль одним кликом, как сегодня, а три точки вместо веера показывают модуль в инспекторе — именованные действия и переключатель статуса, которые ничего не закрывают.',
  ...
  },
  {
    ...
    title: 'Лидер',
    mode: 'dark',
    note: 'Живая страница как есть, на языке презентации. Ни один элемент не добавлен, не сдвинут и не убран — это разметка, которую отдаёт admin.php, со скриптами темы: панели сворачиваются, веер раскрывается, окно опций открывается. Оформление прочитано с presentation.css: линованный грунт и кольца угла, блоки сайдбара — окна с тремя лампами в шапке, моноширинные капс-подписи, заголовок раздела — номер в рамке, иконка в рамке и название над линейкой, плитка — карточка технологий с тонированным глифом и линией света по наведению, счётчик — цифра в рамке, скан-линия в шапке. Тёмная схема — глубокий синий лидера.',
    tags: ['структура 1:1', 'язык презентации', 'окна', 'моно-подписи'],
  },
  {
    file: 'adm-07-effects.html',
    title: 'Эффекты',
    note: 'Оригинал один в один и слой движения поверх. Ни цвет, ни градиент, ни шрифт, ни размер темы не изменены, разметка — исходник admin.php со скриптами темы. Страница появляется, блоки сайдбара въезжают, плитки поднимаются каскадом — и снова при разворачивании раздела. За курсором идёт мягкий свет; плитка под рукой наклоняется к курсору, несёт блик и свет, бегущий по рамке, её иконка подпрыгивает, а бокс заливается тоном. По сплошным полосам заголовков проходит блик, клик даёт рябь, счётчики сайдбара набегают, непустой счётчик несёт маячок, полосы отладки наполняются с бегущими штрихами. Всё стоит под prefers-reduced-motion и выключателем движения стенда.',

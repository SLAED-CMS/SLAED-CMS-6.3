...
// Last added files
$content .= '<tr><td>';
$result = $db->getSqlQuery('SELECT lid, title FROM '.PREFIX_DB."_files WHERE date <= now() AND status != '0' ORDER BY date DESC LIMIT 5");
...
// Last added media
$content .='</td><td>';
$result = $db->getSqlQuery('SELECT id, title, subtitle FROM '.PREFIX_DB."_media WHERE date <= now() AND status != '0' ORDER BY date DESC LIMIT 5");

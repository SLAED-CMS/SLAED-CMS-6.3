...
final class DatabaseBatchTest extends TestCase
{
    ...
    public function theShippedPatchSplitsIntoRealStatements(): void
    {
        ...
        $hit = array_values(array_filter($out['statements'], static fn(string $one): bool => str_contains($one, "'path'")));
        $this->assertNotEmpty($hit, 'The path column block is gone from the patch');

...
function getProbeGuard(): array {
    ...
    $half = intdiv($text, 2);
    return [
        'fit' => checkEditorTextRoom(getProbeText($text), 'products.intro'),
        'over' => checkEditorTextRoom(getProbeText($text + 1), 'products.intro'),
        'cyrfit' => checkEditorTextRoom(str_repeat('я', $half), 'products.intro'),
        'cyrover' => checkEditorTextRoom(str_repeat('я', $half + 1), 'products.intro'),
        'wide' => checkEditorTextRoom(getProbeText($text + 1), 'products.body'),
        'cfgover' => checkEditorTextRoom(getProbeText($text + 1), 'config'),
        'edge' => checkEditorTextRoom(getProbeData('image/png', Parser::EMBEDMAX), 'products.body'),
        'big' => checkEditorTextRoom(getProbeData('image/png', Parser::EMBEDMAX + 1), 'products.body'),
        'png' => checkEditorTextRoom(getProbeData('image/png', 512), 'products.body'),
        'small' => checkEditorTextRoom(getProbeData('image/png', 512), 'products.intro'),
        'link' => checkEditorTextRoom('![photo](https://files.example.net/photo.png)', 'products.intro'),
        'prose' => checkEditorTextRoom('plain prose that carries no image at all', 'products.intro'),
        'pdf' => checkEditorTextRoom(getProbeData('application/pdf', 512), 'products.body'),
        'svg' => checkEditorTextRoom(getProbeData('image/svg+xml', 512), 'products.body'),
        'odd' => checkEditorTextRoom(getProbeData('image/tiff', 512), 'products.body'),
        'tail' => checkEditorTextRoom('text '.getProbeData('image/png', 512).' more '.getProbeData('application/pdf', 512), 'products.body'),

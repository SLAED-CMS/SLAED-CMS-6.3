...
function main(): void {
    global $db;
     setHead(['title' => 'Простота Функциональность Эффективность Безопасность', 'descr' => 'Система управления содержимым сайта, простая в использовании и настройке, имеющая при этом высокий уровень безопасности, высокую скорость работы, а также практически неограниченный потенциал в решении вопроса расширения функциональности.']);
    ...
    $cont .= '<div class="wrp grid_1_2">
        <div class="grid col-block">
            ...
            while([$sid, $catid, $title, $time, $hometext, $ctitle, $cdesc] = $db->sql_fetchrow($result)) {
        ...
        <div class="grid col-block">
            ...
            while([$sid, $catid, $title, $hometext, $time, $ctitle, $cdesc] = $db->sql_fetchrow($result)) {
                $linkstrip = cutstr($title, 45);
                if (preg_match("#\[attach=(.*?)\s(.*?)\]#si", $hometext, $match)) {

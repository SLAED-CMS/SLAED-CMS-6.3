...
function users(): void {
    ...
    $count = ($num) ? $offset + 1 : 1;
        ...
        $cont .= setTemplateBasic('users-top-open', ['{%id%}' => _ID, '{%nickname%}' => _NICKNAME, '{%info_head%}' => $head, '{%gender_head%}' => _GENDER, '{%sort_head%}' => $sort, '{%points_head%}' => _POINTS]);
        while ([$id, $name, $site, $reg, $from, $last, $point, $ip, $gender, $votes, $total] = $db->getSqlRow($result)) {
            ...
            $cont .= setTemplateBasic('users-top-basic', [
                '{%row_id%}' => $count,
...
function rules(): void {
    ...
    $cont = setModuleNavi(['title' => _TOPUSERS, 'htitle' => _TOPUSERS, 'best_href' => getSeoUrl(['name' => $conf['name'], 'op' => 'rules']), 'btitle' => _TU_RULES, 'pop_href' => getSeoUrl(['name' => $conf['name'], 'op' => 'stats']), 'ptitle' => _TU_STATS, 'liste_href' => '', 'add_href' => '']);
    $cont .= setTemplateBasic('users-rules-open', ['{%id%}' => _ID, '{%type%}' => _TYPE, '{%description%}' => _DESCRIPTION, '{%points%}' => _POINTS]);
    ...
    for ($i = 0; $i < count($p); $i++) {
        $a = $i + 1;
        $cont .= setTemplateBasic('users-rules-basic', ['{%row_id%}' => $a, '{%type_text%}' => $p[$i], '{%description_text%}' => $d[$i], '{%points_text%}' => $points[$i]]);
    }
    $cont .= setTemplateBasic('table-close');
...
function stats(): void {
    ...
    if ($result) {
        $cont .= setTemplateBasic('users-stats-open', ['{%rank%}' => _RANK, '{%description%}' => _DESCRIPTION, '{%points%}' => _POINTS, '{%users_count%}' => _TU_USERSCOUNT, '{%spec%}' => cutstr(_SPEC, 4, 1)]);
        while ([$grid, $grname, $description, $points, $extra, $rank, $color] = $db->getSqlRow($result)) {
            ...
            $cont .= setTemplateBasic('users-stats-basic', [

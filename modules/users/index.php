...
function users(): void {
	...
	if ($db->sql_numrows($result) > 0) {
		$con = explode('|', $conf['rating']['account']);
		...
		while ([$user_id, $user_name, $user_website, $user_regdate, $user_from, $user_lastvisit, $user_points, $user_last_ip, $user_gender, $user_votes, $user_totalvotes] = $db->sql_fetchrow($result)) {
		...
		$cont .= '</tbody></table>';
		$cont .= setArticleNumbers('pagenum', $conf['name'], $unum, '', 'user_id', '_users', '', '', '5');

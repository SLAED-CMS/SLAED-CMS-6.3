...
function files() {
	global $db, $admin_file, $user, $conf, $confu, $conff, $home, $op;
	...
	$result = $db->sql_query("SELECT f.lid, f.cid, f.name, f.title, f.description, f.bodytext, f.date, f.counter, f.acomm, f.votes, f.totalvotes, f.totalcomments, f.hits, c.title, c.description, c.img, u.user_name FROM ".PREFIX_DB."_files AS f LEFT JOIN ".PREFIX_DB."_categories AS c ON (f.cid = c.id) LEFT JOIN ".PREFIX_DB."_users AS u ON (f.uid = u.user_id) ".$order." LIMIT ".$offset.", ".$unum);
...
function liste() {
	...
	$offset = intval($offset);
	$result = $db->sql_query("SELECT f.lid, f.cid, f.name, f.title, f.date, c.title, c.description, u.user_name FROM ".PREFIX_DB."_files AS f LEFT JOIN ".PREFIX_DB."_categories AS c ON (f.cid = c.id) LEFT JOIN ".PREFIX_DB."_users AS u ON (f.uid = u.user_id) ".$order." ".$cwhere." ORDER BY date DESC LIMIT ".$offset.", ".$listnum);
	head();
	$cont = navigate(_LIST);
	if ($db->sql_numrows($result) > 0) {
...
function view() {
	global $db, $admin_file, $conf, $confu, $conff;

...
function navigate($title, $cat='') {
	...
	$best = ($conff['rate']) ? '<a href="'.getSeoUrl(['name' => $conf['name']] + $cpar + ['op' => 'best']).'" title="'._BEST.'" class="sl_but_navi">'._BEST.'</a>' : '';
	$pop = ($conff['rate']) ? '<a href="'.getSeoUrl(['name' => $conf['name']] + $cpar + ['op' => 'pop']).'" title="'._POP.'" class="sl_but_navi">'._POP.'</a>' : '';
	$liste = '<a href="'.getSeoUrl(['name' => $conf['name'], 'op' => 'liste']).'" title="'._LIST.'" class="sl_but_navi">'._LIST.'</a>';
	$add = ((is_user() && $conff['add'] == 1) || (!is_user() && $conff['addquest'] == 1)) ? '<a href="'.getSeoUrl(['name' => $conf['name'], 'op' => 'add']).'" title="'._ADD.'" class="sl_but_navi">'._ADD.'</a>' : '';
...
function files() {
	global $db, $afile, $user, $conf, $confu, $conff, $home, $op;
	$cwhere = catmids($conf['name'], 'f.cid');
	$unum = user_news($user[3] ?? 0, $conff['num']);
	$ncat = getVar('get', 'cat', 'num');
	$params = [];
	...
	if (!$home || ($home && $conff['homcat'])) {
		...
		if ($ncat) $cont .= setTemplateBasic('cat-navi', ['{%crumbs%}' => catlink($conf['name'], $ncat, $conff['defis'], _FILES)]);
	...
	if ($db->sql_numrows($result) > 0) {
		while (list($id, $cid, $uname, $stitle, $description, $bodytext, $time, $counter, $acomm, $votes, $totalvotes, $comm, $hits, $ctitle, $cdesc, $cimg, $user_name) = $db->sql_fetchrow($result)) {
			...
			$post = ($conff['autor']) ? (($user_name) ? user_info($user_name) : (($uname) ? $uname : _ANONYM)) : '';
			$post = ($post) ? '<span title="'._POSTEDBY.'" class="sl_post">'.$post.'</span>' : '';
			$date = ($conff['date']) ? '<time datetime="'.date('c', strtotime($time)).'" title="'._CHNGSTORY.'" class="sl_date">'.format_time($time).'</time>' : '';
...
function view() {
	global $db, $afile, $conf, $confu, $conff;
	$id = getVar('get', 'id', 'num');
	$word = getVar('get', 'word', 'word');
	$cwhere = catmids($conf['name'], 'f.cid');
	$result = $db->sql_query('SELECT f.cid, f.name, f.title, f.url, f.description, f.bodytext, f.date, f.filesize, f.version, f.email, f.homepage, f.counter, f.acomm, f.votes, f.totalvotes, f.hits, f.status, c.title, c.description, c.img, u.user_name FROM '.PREFIX_DB.'_files AS f LEFT JOIN '.PREFIX_DB.'_categories AS c ON (f.cid = c.id) LEFT JOIN '.PREFIX_DB."_users AS u ON (f.uid = u.user_id) WHERE f.lid = '".$id."' AND f.date <= NOW() AND f.status != '0' ".$cwhere);
	if ($db->sql_numrows($result) == 1) {
		$db->sql_query('UPDATE '.PREFIX_DB."_files SET counter = counter+1 WHERE lid = '".$id."'");
		...
		if ($cid) $cont .= setTemplateBasic('cat-navi', ['{%crumbs%}' => catlink($conf['name'], $cid, $conff['defis'], _FILES)]);
		...
		$post = ($conff['autor']) ? (($user_name) ? user_info($user_name) : (($uname) ? $uname : _ANONYM)) : '';
		$post = ($post) ? '<span title="'._POSTEDBY.'" class="sl_post">'.$post.'</span>' : '';
		$date = ($conff['date']) ? '<time datetime="'.date('c', strtotime($date)).'" title="'._CHNGSTORY.'" class="sl_date">'.format_time($date).'</time>' : '';
		$reads = ($conff['read']) ? '<span title="'._READS.'" class="sl_views">'.$counter.'</span>' : '';
		$hits = ($conff['hits']) ? '<span title="'._FILEHITS.'" class="sl_down">'.$hits.'</span>' : '';
		...
		if ($conff['link']) {
			...
			list($count) = $db->sql_fetchrow($db->sql_query('SELECT COUNT(lid) FROM '.PREFIX_DB."_files WHERE cid = '".$cid."' AND lid != '".$id."' AND date <= NOW() AND status != '0'"));
			if ($count >= $limit) {
				$random = mt_rand(0, $count - $limit);
				$result = $db->sql_query('SELECT lid, title, description, bodytext, date FROM '.PREFIX_DB."_files WHERE cid = '".$cid."' AND lid != '".$id."' AND date <= NOW() AND status != '0' ORDER BY date DESC LIMIT ".$random.', '.$limit);
				$cont .= setTemplateBasic('assoc-open', ['{%title%}' => _CATASSOC]);
				while(list($aid, $title, $hometext, $bodytext, $time) = $db->sql_fetchrow($result)) {
					$date = ($conff['date']) ? '<time datetime="'.date('c', strtotime($time)).'" title="'._CHNGSTORY.'" class="sl_date">'._CHNGSTORY.': '.format_time($time).'</time>' : '';
...
function add() {
	global $db, $user, $conf, $confu, $conff, $stop;
...
function send() {
	global $db, $user, $conf, $conff, $stop;
	if ((is_user() && $conff['add'] == 1) || (!is_user() && $conff['addquest'] == 1)) {
		$title = getVar('post', 'title', 'title');
		$cid = getVar('post', 'cid', 'num');
		$description = getVar('post', 'description', 'text');
		...
		if ($db->sql_numrows($db->sql_query('SELECT title FROM '.PREFIX_DB."_files WHERE title = '".$title."'")) > 0) $stop[] = _MEDIAEXIST;
		...
		if (!$stop && getVar('post', 'posttype', 'var') == 'save') {
			...
			$db->sql_query('INSERT INTO '.PREFIX_DB."_files (lid, cid, uid, name, title, description, bodytext, url, date, filesize, version, email, homepage, ip_sender, status) VALUES (NULL, '".$cid."', '".$postid."', '".$uname."', '".$title."', '".$description."', '".$bodytext."', '".$url."', NOW(), '".$fsize."', '".$fversion."', '".$authormail."', '".$authorurl."', '".getIp()."', '0')");
			update_points(9);
...
function broken() {
	global $db, $conf, $conff;
	$id = getVar('get', 'id', 'num');
	if ($conff['broc'] == '1' && $id) {
		$db->sql_query('UPDATE '.PREFIX_DB."_files SET status = '2' WHERE lid = '".$id."' AND status != '0'");
		setHead();
...
function loading() {
	...
	if (($id && is_user()) || ($id && $conff['down'] == '1')) {
		$db->sql_query('UPDATE '.PREFIX_DB."_files SET hits = hits+1 WHERE lid = '".$id."'");
		list($stitle, $url) = $db->sql_fetchrow($db->sql_query('SELECT title, url FROM '.PREFIX_DB."_files WHERE lid = '".$id."'"));
		update_points(11);
		if ($conff['stream'] == 2) {

...
function news(): void {
    ...
    if ($db->getSqlRowCount($result) > 0) {
        ...
        $head = '<th>'._ID.'</th><th>'._TITLE.'</th><th>'._POSTEDBY.'</th><th class="{sorter: false}">'._STATUS.'</th><th class="{sorter: false}">'._FUNCTIONS.'</th><th class="{sorter: false}"><input type="checkbox" name="markcheck" id="markcheck" title="'._CHECKALL.'" OnClick="CheckBox(\'#markcheck\', \'.sl_check\')"></th>';
        $rows = '';
        while ([$id, $cid, $uname, $title, $time, $vote, $ip, $ctitle, $nick] = $db->getSqlRow($result)) {
            ...
            $acts = adminMenuItems([
                rtrim($view, '|'),
                rtrim($vote, '|'),
                '<a href="'.$afile.'.php?name=news&amp;op=add&amp;id='.$id.'" title="'._FULLEDIT.'">'._FULLEDIT.'</a>',
                '<a href="'.$afile.'.php?name=news&amp;op=actions&amp;typ=d&amp;id='.$id.$refer.'" OnClick="return DelCheck(this, \''._DELETE.' &quot;'.$title.'&quot;?\');" title="'._ONDELETE.'">'._ONDELETE.'</a>',
            ]);
            $cols = '<td>'.$id.'</td>'
            ...
            .'<td>'.$acts.'</td>'
            .'<td><input type="checkbox" name="id[]" class="sl_check" value="'.$id.'"></td>';
...
function add(): void {
    ...
    $rows .= getAdminFormRow(_CATEGORY.':', getcat('news', $cat, 'cat', 'sl_form', '<option value="">'._HOMECAT.'</option>'));
    ...
    if ($db->getSqlRowCount($result2) > 0) {
        ...
        $asso .= '</tr></table>';
        $rows .= getAdminFormRow(_ASSOTOPIC.':<div class="sl_small">'._ASSOTOPICI.'</div>', $asso);
    }
    $rows .= getAdminFormRow(_TEXT.':', textarea('1', 'hometext', $hometext, 'news', '5', _TEXT, '1'));
    $rows .= getAdminFormRow(_ENDTEXT.':', textarea('2', 'bodytext', $bodytext, 'news', '15', _ENDTEXT, '0'));
    $rows .= fields_in($field, 'news');
    $rows .= getAdminFormRow(_CHNGSTORY.':', datetime(1, 'time', $time, 16, 'sl_form'));
    $rows .= getAdminFormRow(_VOTING.':', add_voting('news', 'vote', $vote, 'sl_form'));
    $rows .= getAdminFormRow(_COMMENTS.':', com_access('acomm', $acomm, 'sl_form'));
    $rows .= getAdminFormRow(_PUBHOME, radio_form($ihome, 'ihome'));
    $rows .= getAdminFormRow(_FIXED.'?', radio_form($fix, 'fix'));

...
function news(): void {
    ...
    'ctitle' => $ctitle
        ...
        $cont .= '</table>';
        $cont .= setArticleNumbers('pagenum', $conf['name'], $unum, $field, 'id', '_news', 'cid', $onum, $conf['news']['nump']);
    } else {
...
function liste(): void {
    ...
    if ($db->getSqlRowCount($result) > 0) {
        ...
        $params = ($let) ? ['let' => $let.'%'] : [];
        $cont .= setArticleNumbers('pagenum', $conf['name'], $listnum, $field, 'id', '_news', 'cid', $onum, $conf['news']['nump'], $params);
    } else {
        $cont .= setTemplateWarning('warn', ['time' => '', 'url' => '', 'id' => 'info', 'text' => _NO_INFO]);
    }
    echo $cont;
...
function view(): void {
    ...
    if ($db->getSqlRowCount($result) == 1) {
        ...
        if ($conf['news']['assoc']) {
            ...
            if ($assocIn !== '') {
                [$count] = $db->getSqlRow($db->getSqlQuery('SELECT COUNT(id) FROM '.PREFIX_DB.'_news WHERE cid IN ('.$assocIn.') AND id != :id AND time <= NOW() AND status != \'0\'', ['id' => $id]));
            } else {
                $count = 0;
            }
            if ($count >= $limit) {

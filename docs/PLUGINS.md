...
> Статус: design note / proposal. Документ описывает текущее состояние
> расширяемости и рекомендуемое направление унификации. Он не является
> подтверждением, что общий реестр `plugins/*/manifest.json` или страница
> управления плагинами уже реализованы.
...
| Провайдер капчи | `core/classes/captcha.php`, assets в `plugins/altcha/` | провайдер выбирается из `config/security.php` | чистый паттерн «стратегия» |
...
## Рекомендация, не текущий контракт
...
Эти шаги описывают возможную реализацию, а не уже существующий runtime-контракт:

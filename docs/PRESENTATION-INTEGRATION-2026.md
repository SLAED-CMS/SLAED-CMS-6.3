...
Статус: закрыто A-E, следующего нет. Четырнадцать решений раздела 8 приняты владельцем 2026-09-10 и 2026-09-11 и вшиты в
...
12 якорей рельса ведут к секциям, ни один элемент не шире родителя, консоль пуста. Класс-контракт разметки — комментарии
над каждым блоком `presentation.css` (скелет партиала и фрагмента, тона через `data-sl-tone`, состояния через
`sl-is-*`, кольца на `.sl-knob`); `php tools/ui-audit.php --theme=lite` показывает `classes` 0 (решение 13) и в конце C
выполняется `php tools/ui-audit.php --store`.
...
13. **Классы без разметки между B и C (сессия B, 2026-09-11).** `presentation.css` объявляет 222 класса `sl-pres-*`, а
    разметка приходит только в C, поэтому счётчик `classes` lite растёт 0 → 222 и `--store` отказывается писать линию.
    Владелец принял: в батче B `lite.classes: 222` вписывается в `tools/ui-audit-baseline.json` вручную и линия
    пересохраняется штатно; приёмка C обязана вернуть `classes` к 0 и выполнить `php tools/ui-audit.php --store`.
    Побочный факт той же сессии: `--store` не имеет принудительного режима, а сайтовый вход рельса
    (`.sl-block-login`) не срабатывает — auth-страницы сайта в паре `before/after` отсутствуют, E это учитывает.
14. **Листание в окне (сессия D, 2026-09-11).** Стрелки `[data-sl-shot-step]` и клавиши сайтового просмотрщика остаются
    инертными, как до плана (F17); плагин только открывает окно по `a[data-sl-shot-open]` через лайтбокс `slaed.js`.
    Появление секций решено в B средствами CSS (`animation-timeline: view()` в `presentation.css`), плагин его не дублирует.

...
| `updated` | `tid, status, pinned, updated, id` | список типа по дате изменения |
| `views` | `tid, status, pinned, views, published, id` | список типа по числу просмотров |
| `title` | `tid, status, pinned, title, id` | буквенный фильтр и сортировка типа по заголовку |
| `expires` | `tid, status, expires` | ближайшее окончание видимости для временной границы HTML-кеша списка |
| `tree` | `tid, status, id` | партии дерева документов `getNodeTree()` по курсору ID без сортировки остатка типа (решение пользователя 2026-09-26, S20.4) |
...
- Отдельные индексы средней оценки и каждого сочетания категории с необязательной сортировкой не создаются: они умножили бы стоимость записи. Сортировку типа по просмотрам обслуживает индекс `views`, по дате изменения — `updated`, буквенный фильтр и сортировку по заголовку — индекс `title`, а средняя оценка вычисляется из `score` и `ratings`.
- Упорядоченное чтение делит каждую ветвь типа на две непересекающиеся: `pinned <> 0` и `pinned = 0` (решение пользователя 2026-09-25, S19.6). В каждой ветви `pinned` постоянен, поэтому индекс с `pinned` после `tid, status` отдаёт строки в порядке любой сортировки и в обе стороны; ветви объединяются `UNION ALL` с пределом конца страницы, как смешанная выборка типов. Так закрепление первым не требует сортировки всех строк типа, а тип без `features.pinned` пользуется теми же индексами. Административный список сортируется как публичный по умолчанию и обслуживается `pub`. Профиль S19.6 (tools/node-profile.php, MariaDB 11.7, 100000 материалов, чтения страницы news по `Handler_read`): по заголовку — 9049 до разбиения и 195 после, по дате изменения — 9049 и 100; ветвь `pinned <> 0` сортирует все закреплённые строки типа.
...
- Индекс `expires` служит только временной границе HTML-кеша (`getNodeDeadline()`, решение пользователя 2026-09-25, S19.6): список задаёт диапазон через `published`, а граница ищет ближайшее будущее `published` по ветвям `pinned` индекса `pub` и ближайшее будущее `expires` по этому индексу. Профиль S19.6: 9021 чтение всех опубликованных строк типа до изменения, 8 после.
...
- Публичная выборка категории объединяет основной индекс `_nodes.cat` и дополнительный `_node_categories.cat`, возвращая каждый материал один раз: ветвь `cid = X` читает `cat` в порядке сортировки, ветвь `cid <> X` с материалом из `_node_categories.cat` читает связи категории; ветви не пересекаются по построению, поэтому `UNION ALL` и сумма счётчиков не удваивают материал даже при нарушении правила «основная не дублируется». Основная ветвь читает строк порядка конца страницы, дополнительная — все дополнительные связи категории: порядок сортировки хранится в `_nodes`, а не в связи (профиль S19.6: счётчик плотной категории — 2505 чтений вместо 18722; ручной замер на той же базе с категорией из 20 материалов — страница 83 чтения вместо 8222).

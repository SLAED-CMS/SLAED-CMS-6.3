...
- **The crawl is red on this stand before you touch anything, and it is coverage and not violations.** The baseline stores 366 pages and the tree renders 365: three route variants no longer answer - `?word=` twice and `?author=1` once - and two of them are record-bound, so the report also names two forms it could not reach. It was proved to predate the work rather than argued: the whole of one batch was stashed and the same audit run against the committed tree gave an identical loss, the same three routes and the same 366 to 365. Read the violation list, which is what your change can move; a loss that names none of your selectors is this one. Do not `--store` over it - that is how a real regression gets adopted as the new floor.

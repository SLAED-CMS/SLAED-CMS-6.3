...
| 9 | 2 | `format` column in all three channels | done | Committed in `cfa91cb8`. `setup/sql/table.sql` (`format VARCHAR(20) NOT NULL DEFAULT ''` as the last column of `_privat`, after `delout`, which is where `addcol` appends it in both upgrade channels), `setup/sql/table_update6_3.sql` (the `addcol` in Batch K after the three state columns and before the backfill, so the column order of an upgraded table is the column order of a fresh one), `setup/sql/update6_3_patch.sql` (its own section 6, because an installation that already applied section 5 never re-runs it, plus the two header paragraphs that enumerate what the file changes and what it never does), `tests/Support/privat_probe.php` and `tests/Unit/PrivatMigrationTest.php` (state E and the `PROBENEW` set). The column ships empty on purpose and no statement backfills it: the verdict is per body, not per column, and `tools/privat-migrate.php` of step 10 is what computes it. The deployment gate the plan already carries — no row outside `plain`/`markdown` — is what makes `''` a state the release cannot end in. Passed: `php -l` on the two changed PHP files, `composer analyse`, `composer test` (683 tests, 7679 assertions, 6 pre-existing skips), `php-cs-fixer --dry-run` (nothing to fix), the probe over both upgrade channels times states A through E (all 10 converge on the fresh definition byte for byte, all safe to run twice, no message row touched), and section 6 run against the migrated dev database through the shipped splitter: 1472 rows in, 1472 out, 1472 out again on a second run, the table now carrying the eight columns and seven keys of the fresh schema, every row `format = ''` and none classified. A smoke check of `/` and `/index.php?name=account` over HTTPS answered 200 and left `error_php.log`, `error_sql.log` and `error_site.log` byte for byte unchanged. One gap this step found and closed rather than left: the probe's states C and D build their table from `table.sql`, so the moment `format` entered the fresh schema they stopped describing the installation section 6 is written for — a table carrying every stage 1 column and key and only `format` missing. That is now state E, seeded from the fresh schema with `format` dropped. It is not the only state the column is added in — A and B start from the legacy shape and never carried it either — but it is the only one that starts from the table stage 1 really shipped, which is the only shape the patch channel's section 6 will ever meet in the field. `moved` was generalized with it: it used to assert only that state D starts on the wrong definition, and now asserts that every state except the already-final C really differs from the final definition before the run. One amendment to the specification itself, made with explicit approval on 2026-08-05 because the shipped column is wider than the sentence that described it: the storage model now records `format` as `VARCHAR(20) NOT NULL DEFAULT ''` and names `''` as the unclassified state that exists only between the schema section and the gate, and the content contract now says in as many words that the `''` of the storage model is a migration state the gate has already cleared rather than a third format runtime has to render. Neither sentence changes what is built; they close the one reading under which the two sections looked like they disagreed. Not done here, because the plan gives them their own rows: no runtime code reads or writes `format` yet (step 11), and the `docs/VERSIONS.md` entry with the stage 2 deployment sequence is step 12. |
| 10 | 2 | `tools/privat-migrate.php` with report/classify/convert/sample/title, ledger, rehearsal options | done | Committed in `cfa91cb8`. `tools/privat-migrate.php` (the five modes the plan names and no sixth; `report` reads and prints only, `classify` writes `format` and the ledger `storage/migrate/privat-format.json` and rewrites nothing, `convert` and `title` are two independent ledger-driven passes that store the value they replaced, mark every row they finish and rewrite nothing twice, `sample` prints both fields before and after per class; `--default=`, `--size=`, `--num=`, `--out=`, `--db=`, `--prefix=` and `--force=1`), `tests/Support/privat_format_probe.php` and `tests/Unit/PrivatFormatTest.php` (11 tests, 59 assertions over 10 fixture messages in a disposable schema, driven through `--db` and `--prefix`, which is the rehearsal contract itself). The body classifier is the one `tools/comment-migrate.php` carries, because the writer is literally the same `filterHtml()`; the tool shares that discipline and none of its code. The title needs a verdict of its own and has no column to keep it in, so the ledger keeps it: a title was written by the html branch when its own row's body was, because both fields of one message went through one editor mode in one request, and a `&#034;` in the title alone still settles it because the other branch escapes a quote as `&quot;` and can never produce that entity. A self review of the step found and fixed three defects it had introduced, and all three are part of this row. The refusal read the completion stamp alone, and a pass stamps the ledger only when it finishes: an interrupted `convert` leaves finished rows behind and no stamp at all, so `classify` would have accepted that ledger, reset every mark and handed those rows to a second reversal — the one operation no restore short of the dump can undo. The refusal now counts the rows a pass really finished and never reads a stamp. The row verdict and the body decode also disagreed: the ledger's `html` flag knew the whole request had gone through the html branch whenever the title carried a `&#034;` the other branch cannot produce, while `getConvertBody()` re-derived the branch from the class of the body alone, so one row could be read as written by two different editors. The body now follows the row verdict wherever its own class carries no evidence — classes 4 and 5 have neither a tag nor a break — with class 2 the one exception, because a break with the line ending behind it is written by `nl2br()` and by nothing else, and `nl2br()` runs in no branch but the plain one. And `sample` dereferenced the result of a read that can answer no row; it now skips that id. What the review did not change is the branch structure, and that is deliberate and load-bearing: the tag map runs on the legacy branch alone, because the plain entity map turns an authored `&lt;a&gt;` back into a real tag, and a tag map running after it would silently promote text the author typed as escaped characters into a live Markdown link. Row 14 of the dev table is exactly that shape and must stay text. Passed: `php -l` on the three new files, `composer analyse` (292 files, no errors), `composer test` (694 tests, 7738 assertions, the same 6 pre-existing skips), `php-cs-fixer --dry-run` (nothing to fix), and a full rehearsal, re-run on a fresh `mysqldump`/restore copy of the real 1472-row table after the three fixes: `report` classified 1138 rows as `plain` and 334 as `markdown` with no row left over, `classify` wrote every format, `convert` and `title` rewrote 1472 rows each, and the table ended with 1472 rows, 0 outside `plain`/`markdown`, 0 bodies still carrying a `<br>` and 0 titles still carrying a writer entity. Running both passes again rewrote 0 rows and `classify` refused the finished ledger; `--force=1` accepted it. The site table was read by nothing and ended the run exactly as it started: 1472 rows, all `format = ''`, 1138 still carrying `<br>`. The rehearsal schema and its ledger were removed. `error_php.log` and `error_sql.log` were last written at 10:24 and 09:33, before the run; `error_site.log` carries only the scheduler's standing sendmail failure, 174 entries spread evenly over every hour of the day. Four things this step decided and records rather than improvised silently: the html branch is seeded as stored bytes instead of written, because every content editor a user may pick answers `plain` or `markdown` and only the admin editors answer `html`, so no user request on this installation can reach that branch; `convert` and `title` each exit non-zero only on a defect of their own — a row without a final format or a row count that moved — while the gate is printed as a readout ending in `clean` or `not clean yet`, so a successful `convert` does not abort a scripted deployment merely because `title` has not run yet; `classify` refuses a ledger whose rows a pass has already rewritten before it scans the table, so a converted table is never described by a table of verdicts that no longer mean anything; and the report counts the titles carrying a writer entity and the titles carrying markup separately, because a plain title keeps markup the map does not name as its own characters. Not done here, because the plan gives them their own rows: no runtime code reads `format` yet (step 11) and the `docs/VERSIONS.md` entry with the stage 2 deployment sequence is step 12. The real conversion is not this step's to run: it belongs in the maintenance window of the deployment sequence, behind the closed site and the rehearsed restore. |
| 11 | 2 | Safe rendering for body and title, write-time encoding and `getDecodedText()` compensation removed | done | Not committed yet. `core/classes/privat.php` (`format` joins the field list and every row a read answers, `addMessage()` normalizes both fields and resolves the format once before the first attempt so a retry rewrites nothing, `addMessageRow()` stores the format with the row, and the two new private methods are the ones `Comment` already carries: `getBodyFormat()`, which refuses an html editor and keeps the body as markdown source, and `filterMessageText()`, which keeps what changes the text itself — the trusted-html tokens a visitor may not open, the autolinker on the body alone and the censor list — and drops only the escape), `core/user.php` (the detail view renders `filterContent($body, true, ..., $view['format'])`, the send hands both fields over as the author submitted them, and the compose form refills them unescaped because escaping source into an editor hands the author back an escape they never wrote), `core/admin.php` (`getDecodedText()` is gone with the contract it compensated for, and the popover renders the same safe call the recipient reads through), `tests/Support/privat_class_probe.php` and `tests/Unit/PrivatClassTest.php` (the probe now builds the class from the whole configuration, because the normalization reads the site settings, plus two scenarios and four tests: what a send stores per editor format, what the renderer both adapters call answers for it, what the template does to the title, and what the writer still filters). One amendment to the class comments rather than to the plan: step 2 had written that the editor normalization stays outside the class, and it now lives in the class exactly as it does in `Comment`, because what is left after the escape is dropped is per-message content policy and no caller may restate it. Passed: `php -l` on the four changed PHP files, `composer analyse` (292 files, no errors), `composer test` (698 tests, 7692 assertions; the skips are the environment ones the suite already had — OAuth JWT, symlink, config permissions, the unused-constant and setup-file audits — which the summary counts as 14 and `--display-skipped` names as 13, and none of the 47 Privat tests is among them), `php-cs-fixer --dry-run` (nothing to fix), and 19 authenticated HTTPS checks in a real browser against the migrated stand. One defect found on the way and fixed with explicit approval, in a stage 1 file this step does not otherwise touch: `setup/sql/table_update6_3.sql` ended on a bare `ALTER TABLE {prefix}_privat MODIFY id/title/body`, which rebuilds a table that already carries the final definition, so `up62:c` of the migration probe had never been quiet; the three columns are now reconciled through `modcol()`, the helper stage 1 added for exactly that, and `PrivatMigrationTest` and `SchemaUpdateValidationTest` pass together (10 tests, 85 assertions). The stand itself was migrated for the run, in the order the deployment sequence fixes: a `mysqldump` of the whole database, a rehearsed restore into a copy of its own, the privat section of the 6.3 patch channel and the whole tool run against that copy first, then the same against the stand — 1482 rows in and out, the table on the eight columns and seven keys of the fresh schema, `classify` writing every format, `convert` and `title` rewriting 1482 rows each and the gate reading clean. The route checks then ran on fixture accounts with converted rows of both formats carrying everything an escape used to hide: the mailbox list prints a title carrying `<img src=x onerror=alert(1)>` as its own characters, the markdown detail escapes the script and the link the author wrote while `[b]` still renders as `<strong>`, the plain detail turns its own line ending into a `<br>` and keeps a stored `<br />` as text, the administrator popover renders the same body through the same call and the chip escapes the title in the text and in the attribute alike, and no page in the run answers a dialog, an injected image or an `onerror` attribute. The one check that did not pass is a false positive recorded rather than waved away: the admin page carries a single `href="javascript:OpenWindow(...)"`, the panel's own Sypex Dumper block link, which stands on the admin home just as well and comes from no message; every message-borne link in the run is escaped text. A send through the real form stored source and nothing else: `format = markdown`, the title as typed, the body with its markup intact, read back as escaped text by its recipient. `error_php.log` and `error_sql.log` were untouched by the whole run and `hack.log` still ends on its July entry. Three things the run established that are not this step's to fix: the site request filter refuses a POST body carrying `<script>` or `javascript:` before any module sees it, so that payload can only enter the table the way a converted legacy row does; the Toast editor does not mount inside the htmx-swapped compose form of this page, so the send was driven through the field it wraps, with the same token and the same route; and the stand carries a body-stripped dump, so 1481 of its 1482 messages have an empty body and the conversion could only be measured on the 24 titles that carried a writer entity. |
| 12 | 2 | `docs/VERSIONS.md` entry with deployment sequence, stage 2 verification matrix | done | Not committed yet, and it lands with step 11, which is not committed either. `docs/VERSIONS.md` (dated stage 2 entry above the stage 1 one, because the file reads newest first and this release depends on that one: what the release changes, which of the three channels carries the column, that `tools/privat-migrate.php` is mandatory and needs a maintenance window with the site closed and a rehearsed restore, the eight-step deployment sequence with the gate as step 6 and the runtime code only after it, what `report` and `sample` are for, and every behavior change the release carries down to the one thing the conversion cannot repair). No other file needed a change: the column, the tool and the runtime code landed complete in steps 9, 10 and 11, and this step proved it rather than rewriting it. Passed: `composer analyse` (292 files, no errors), `composer test` (698 tests, 7692 assertions, the same 13 environment skips), `php-cs-fixer --dry-run` (0 of 497 files), the 47 Privat tests on their own (`PrivatClass` 29, `PrivatFormat` 11, `PrivatMigration` 7 — both upgrade channels times states A through E, the gate reading clean after both passes, and a ledger refused twice: finished, and interrupted) and `SchemaUpdateValidationTest` (3 tests). `php -l` was not run because this step changed no PHP file. Source proofs, each `rg` over the tree: `_privat` appears outside `core/classes/privat.php` only in the two probes, `tools/privat-migrate.php` and the compiled template cache; `deletePrivatCounts`, `setPrivateMessageSaved`, `deletePrivateMessage` and `deleteAdminPrivate` are gone; `getDecodedText` no longer appears in the private-message path of `core/admin.php`; `filterHtml` no longer appears in the private-message path of `core/user.php`; `fsmail` survives only in the forum and the profile form; and no `ORDER BY id DESC LIMIT 1` runs against `_privat`. Route matrix: 48 authenticated HTTPS checks in one run in a real browser over the final tree, against the migrated and converted stand, on two fixture accounts and five seeded rows carrying everything an escape used to hide. Every read is a GET without a token — the mailbox page, the two mailboxes fetched on demand, the dashboard preview; every mutation is a POST with the token in the body. Both formats were opened through the real htmx action and asserted at the DOM rather than by string match: the markdown detail carries no `script` element, no `img` element and no `javascript:` link inside the rendered body, prints all three as their own characters, still renders `[b]` and `**` as `<strong>`, and hands the author's `&lt;a&gt;` back as text; the plain detail turns its own line ending into a break and keeps a stored `<br />` as text. The whole page and the whole admin page carry zero `[onerror]` attributes and zero `img[src=x]`, no dialog was opened anywhere in the run, and no page error was raised. Eleven refusals each changed nothing: a GET on the read route, a missing token, a wrong token, a foreign id, a GET on the mutation route, a missing and a wrong token on it, a batch carrying one foreign id, a second send inside the interval, a send without a token, and the removed `go=5`. Persistent outcome, read back row by row: the two opened rows and the bulk-read row are `viewed = 1`, the row four delete refusals named is still `delin = 0`, the saved row is untouched, the foreign row is untouched, and the run stored exactly one message — `format = markdown`, the title and the body as the author submitted them with `<b>` intact and `&lt;esc&gt;` still escaped, which is source and not what the old writer would have stored — and the queue grew by exactly one, a notification to the recipient address whose link carries that id, so neither of the two refused sends queued anything. The administrator panel renders the same body through the same safe call, escapes the title in the text and in the attribute alike, and the one `javascript:` link on that page is named rather than waved away: `javascript:OpenWindow('plugins/sxd/index.php', ...)`, the panel's own Sypex Dumper block link, which comes from no message. Recorded because it cost a round: the first pass of this matrix reported five failures and all five were the assertions, not the code. Three matched escaped text or the compose form on the whole page instead of the rendered body, and the investigation behind them established one thing worth keeping — the unescaped compose refill step 11 introduced reaches the Toast editor bootstrap as a JavaScript string, and what keeps it inside that string is `json_encode()` at `plugins/editors/toastui/driver.php:39`, whose default slash escaping turns `</script>` into `<\/script>` so no literal `</` can appear at all; the textarea the editor wraps is escaped by its template. The fourth read a refusal out of the word `TOKEN` appearing in an accepted answer, which is not an inference: `core/helpers.php:590` writes `hx-headers='{"X-CSRF-TOKEN": ...}'` onto every row action, so the rebuilt list the accepted bulk answered with carries that literal once per row. The fifth was the admin `javascript:` link above. All five now assert at the DOM or against the database. The run created its own two accounts, five rows and one send, and removed all of it again: the database ended holding exactly what it held before — 1482 messages, 11845 users, 11 queued mails, the gate at 0 — and its 14 session rows were removed with it. `error_php.log` and `error_sql.log` are 0 bytes and were last written the day before the run; `hack.log` still ends on its July entry; `error_site.log` carries only the scheduler's standing `mail()` failure against the missing local SMTP server, one entry of which is this run's own notification being drained. Named rather than left to be inferred, because this matrix is the stage 2 one and not a second stage 1 run: it does not exercise unread, save and unsave, either user-deletion path, the admin delete, or a recipient at `psmail = 0`, and it drove the send through the compose route with the token the page carried rather than through the form's own submit, because step 11 established the Toast editor does not mount inside the htmx-swapped compose form. Every one of those is a stage 1 row and was exercised where it belongs — save and unsave and the bulk set in steps 3 and 4, both deletion paths and the admin delete in steps 6 and 8, the `psmail` gate with two distinct accounts and two distinct addresses in step 7 — and the step 11 diff touches none of their code, which is why they were not run again. What does cover them here is the unit suite, which ran green in this step and carries a test for each: independent read and saved states, a bounded batch refusing one foreign id, a delete touching one side only, a delete only a participant may run, a deleted account leaving the counterpart intact, and an admin delete erasing the row. |

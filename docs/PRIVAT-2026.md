...
- `format` (stage 2): `plain` or `markdown`.
...
- `body` is source. `format` is `plain` or `markdown` and nothing else. An HTML
  editor is not a trust grant: its submitted markup is treated as Markdown source
  and escaped. Frontend and admin both render it through
  `Parser::filterContent()` with `safe = true` and the stored format.
